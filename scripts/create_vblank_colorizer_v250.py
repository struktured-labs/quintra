#!/usr/bin/env python3
"""
v2.50: BG Anti-Flicker + 48 Tiles/Frame

CHANGES over v2.36:
1. INCREASED BG TILES: 48 tiles/frame (was 32)
   - Full tilemap refresh every 1024/48 ≈ 21 frames (~0.35s)
   - 48 tiles × ~75 M = 3600 M + DMA 160 M = 3760 M (under 4560 M VBlank budget)
2. 0xFF ANTI-FLICKER FILTER: Skip comparison chain if tile read is 0xFF
   - PPU mode 3 can return 0xFF garbage when reading VRAM
   - Without filter, 0xFF falls through to palette 0, overwriting correct attributes
   - With filter, garbage reads are simply skipped (attribute preserved)

INHERITED from v2.36:
- Input debounce: 1 d-pad read + 8 button reads via loop
...
def main():
    ...
    print("=== Penta Dragon DX v2.50 ===")
    print("48-Tile BG + Anti-Flicker Filter")
    print()
    print("CHANGES in v2.50:")
    print("  1. BG colorizer: 48 tiles/frame (full refresh in ~0.35s)")
    print("  2. 0xFF anti-flicker filter: skip garbage tile reads")
    print()
    print("INHERITED from v2.36:")
    print("  - Input debounce: 1+8 reads via loop (proper debounce)")
