#!/usr/bin/env python3
"""
v1.87: Minimal BG colorizer - just VBK manipulation test

- Tests if VBK save/restore alone causes Sara corruption
- No actual tile colorization, just VBK read/write
"""
import sys
import yaml
...
def create_bg_row_colorizer_lazy(lookup_table_addr: int) -> bytes:
    """Minimal VBK test - just switch VBK 0->1->0 and return.

    Tests if VBK manipulation alone causes Sara corruption.
    """
    code = bytearray()
    ...
    code.append(0x47)                   # LD B, A (save VBK in B)
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v187.gb")
    ...
    print("\n=== v1.87: VBK manipulation test ===")
    print("  Minimal BG colorizer: just VBK 0->1->0 switch")
    print("  Tests if VBK switching alone causes Sara corruption")
    print()
