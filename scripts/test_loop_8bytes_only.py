#!/usr/bin/env python3
"""
Test: Hardcoded all palettes, then loop overwrite ONLY palette 4 (8 bytes).
This is exactly what OVERWRITE did, which worked.
"""
import sys
import yaml
...
def load_palette4_from_yaml(yaml_path: Path) -> bytes:
    """Load just palette 4 (Hornets) from YAML."""
    ...
    # Just Hornets palette
    hornets = data.get('obj_palettes', {}).get('Hornets', {})
    if hornets:
        return pal_to_bytes(hornets['colors'])
    return pal_to_bytes(["0000", "03FF", "00DF", "0000"])  # Default yellow


def create_loader_with_8byte_loop(pal4_data_addr: int) -> bytes:
    """
    1. Hardcoded BG palettes (grayscale)
    2. Hardcoded OBJ palettes (grayscale)
    3. Loop overwrite ONLY palette 4 (8 bytes)
    """
    code = bytearray()

    # BG palettes - grayscale
    code.extend([0x3E, 0x80])
    code.extend([0xE0, 0x68])
    for _ in range(8):  # 8 palettes
        code.extend([0x3E, 0xFF]); code.extend([0xE0, 0x69])
        code.extend([0x3E, 0x7F]); code.extend([0xE0, 0x69])
        code.extend([0x3E, 0x94]); code.extend([0xE0, 0x69])
        code.extend([0x3E, 0x52]); code.extend([0xE0, 0x69])
        code.extend([0x3E, 0x08]); code.extend([0xE0, 0x69])
        code.extend([0x3E, 0x21]); code.extend([0xE0, 0x69])
    ...
    # Loop overwrite ONLY palette 4 (8 bytes)
    code.extend([0x21, pal4_data_addr & 0xFF, (pal4_data_addr >> 8) & 0xFF])
    code.extend([0x3E, 0xA0])  # OCPS = 0xA0 (palette 4 start with auto-inc)
    code.extend([0xE0, 0x6A])
    code.extend([0x0E, 0x08])  # LD C, 8 (only 8 bytes)
    loop_start = len(code)
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_LOOP8.gb")
    palette_path = Path("palettes/penta_palettes_v097.yaml")

    print("=== Test: Hardcoded init + 8-byte loop for palette 4 only ===")

    pal4_data = load_palette4_from_yaml(palette_path)
    print(f"Palette 4 data: {pal4_data.hex()}")
    ...
    palette_loader = create_loader_with_8byte_loop(data_addr)
    ...
    print()
    print("If YELLOW: 8-byte loop works (like OVERWRITE)")
    print("If GRAY: loop failed")
