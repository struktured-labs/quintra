#!/usr/bin/env python3
"""
v1.61: v1.09 + BG Code (but DON'T call it)

Test to isolate: BG code is in ROM but combined doesn't call it.
If this works on fresh boot, the CALL to bg_colorizer is the problem.
"""
import sys
import yaml
...
def create_bg_row_colorizer_lazy(lookup_table_addr: int) -> bytes:
    """BG row colorizer with lazy init and LCD check."""
    WRAM_BUFFER = 0xDFF0
    code = bytearray()
    ...
    # Increment counter (wrap at 64)
    code.append(0x78)
    code.append(0x3C)
    code.extend([0xE6, 0x3F])
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v161.gb")
    ...
    print("\n=== v1.61: v1.09 + BG Code (NOT CALLED) ===")
    print("  v1.09 sprite colorization only")
    print("  BG code is in ROM but NOT called - testing isolation")
    print()
    ...
    combined = create_combined_with_dma(palette_loader_addr, shadow_main_addr)  # NO bg_colorizer call
