#!/usr/bin/env python3
"""
v1.65: TEST - VBK switching with warmup delay

BG colorizer waits 128 frames before doing any VBK switching.
Tests if delayed VBK switching avoids the startup crash.
"""
import sys
import yaml
...
def create_bg_row_colorizer_lazy(lookup_table_addr: int) -> bytes:
    """TEST: LCD check + warmup delay + VBK switching."""
    code = bytearray()
    ...
    # Counter >= 128: do VBK switching
    # Switch to VBK=1, then back to VBK=0
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v165.gb")
    ...
    print("\n=== v1.65: TEST - VBK with Warmup ===")
    print("  BG colorizer: 128 frame warmup before VBK switching")
    print("  Testing if delayed VBK switching avoids crash")
    print()
