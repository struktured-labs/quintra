#!/usr/bin/env python3
"""2 rows unrolled (no outer loop, no JR NZ row_loop): two tile_loops + DE+=8.

If this PASSES → outer loop backward JR is the freezing factor.
If this FREEZES → something about doing tile_loop twice in a row, or DE+=8.
"""
import sys
...
def create_2rows_unroll(bg_table_addr):
    ...
    # Second tile_loop (24 tiles, manually unrolled — no outer counter)
    code.extend([0x3E, 0x18])
    tl2 = len(code)
    code.extend([0xF5, 0x2A, 0x4F, 0x0A, 0x12, 0x13, 0xF1, 0x3D])
...
def build():
    ...
    rom[0x14D] = chk
    out = Path("rom/working/penta_dragon_dx_v301_2rows_unroll.gb")
    out.write_bytes(rom)
    print(f"Wrote {out}")
