#!/usr/bin/env python3
"""
v1.80: BG Colorizer - VRAM WRITE test

- Tests if writing to VRAM bank 1 causes freeze
- Writes just ONE byte to 0x9800 with VBK=1
"""
import sys
import yaml
...
def create_bg_row_colorizer_lazy(lookup_table_addr: int) -> bytes:
    """BG colorizer - test VBK SWITCH only (no write)."""
    HRAM_COUNTER = 0xC4
    WARMUP_FRAMES = 60
    TOTAL_STEPS = 128
    code = bytearray()
    ...
    # After warmup, do VBK switch test
    ...
    # Write ONE byte to VRAM bank 1 (attribute at 0x9800)
    code.extend([0x21, 0x00, 0x98])     # LD HL, 0x9800
    code.extend([0x36, 0x00])           # LD [HL], 0  (write palette 0)
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v180.gb")
    ...
    print("\n=== v1.80: BG Colorizer - VRAM WRITE test ===")
    print("  Writes 1 byte to VRAM bank 1")
    print()
