#!/usr/bin/env python3
"""Diagnostic: attr_comp with only 1 row (24 tiles), single DI window, single bank-2 write batch.

If gameplay works -> issue is in the 24-row loop machinery / cumulative effect.
If still broken -> even one batch of attr writes disrupts game state.
"""
import sys
...
def create_attr_1row_only(bg_table_addr):
    """Single row, single DI window. Writes 24 attrs to 0xD000-0xD017 in bank 2."""
...
def build():
    ...
    w(A['sw'], bytes(sw))
    w(A['ac'], create_attr_1row_only(A['bt']))
    ...
    w(A['cz'], bytes(code))
    print(f"  colorize handler (1-row attr only): {len(code)} bytes")
    ...
    rom[0x14D] = chk
    out = Path("rom/working/penta_dragon_dx_v301_attr_1row_only.gb")
    out.write_bytes(rom)
    print(f"Wrote {out}")
