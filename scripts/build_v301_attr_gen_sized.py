#!/usr/bin/env python3
"""attr_comp + general-mode GDMA with byte count matching attr_comp output.

HDMA5 value = (bytes/16)-1.  attr_comp writes n_rows × 32 bytes (24 tiles
per row + 8 stride padding).  So HDMA5 = (n_rows*32/16)-1 = (n_rows*2)-1.

For n_rows = 18, this copies 576 bytes — covers 18 rows of attrs without
clobbering bg_sweep's per-frame VRAM writes for rows 18+.
"""
import sys
...
def create_gdma_general_sized(n_rows: int) -> bytes:
    """Atomic GDMA with byte count = n_rows × 32."""
    bytes_to_copy = n_rows * 32
    hdma5 = (bytes_to_copy // 16) - 1
    assert 0 <= hdma5 <= 0x7F, f"hdma5 out of range: {hdma5}"
    ...
    code.extend([0x3E, hdma5, 0xE0, 0x55])    # general-mode start
...
def build(n_rows: int):
    ...
    w(A['gd'], create_gdma_general_sized(n_rows))
    ...
    rom[0x14D] = chk
    out = Path(f"rom/working/penta_dragon_dx_v301_gens_R{n_rows}.gb")
    out.write_bytes(rom)
    print(f"Wrote {out} (GDMA bytes = {n_rows*32})")
