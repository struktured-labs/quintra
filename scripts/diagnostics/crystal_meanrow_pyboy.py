"""crystal_meanrow_pyboy.py — Reach Crystal Dragon arena (D880=0x0E, FFBA=2) via
the PROVEN PyBoy teleport mechanism (docs/boss_teleport_breakthrough.md:
map bank3, IE=0, simulate CALL 0x1A2B), then sample the active tilemap for ~300
frames. For each NON-ZERO tile id: count, sum-of-screen-row (0-17), min/max row,
distinct rows, distinct cols, and current attr-palette histogram (low 3 bits of
VRAM bank1). Output: /tmp/crystal/crystal_meanrow_pyboy.log

Run: uv run python scripts/diagnostics/crystal_meanrow_pyboy.py
"""
import sys, os
from pyboy import PyBoy

ROM = "rom/working/penta_dragon_dx_teleport.gb"
BOSS = 2  # Crystal Dragon -> D880 0x0E
OUTDIR = "/tmp/crystal"
os.makedirs(OUTDIR, exist_ok=True)
LOG = os.path.join(OUTDIR, "crystal_meanrow_pyboy.log")
logf = open(LOG, "w")
def log(m):
    logf.write(m + "\n"); logf.flush()

log(f"crystal meanrow pyboy ROM={ROM} BOSS={BOSS} (D880 target=0x{0x0C+BOSS:02X})")
...
# --- boot to title + auto-start (same schedule as teleport_mechanism_pyboy.py) ---
...
if m[0xD880]!=expect:
    log("FAIL: arena did not load"); pb.screen.image.save(OUTDIR+"/crystal_fail.png"); pb.stop(save=False); logf.close(); sys.exit(1)

# settle: let the arena's per-frame loop + colorizer paint the tilemap. The
# arena needs many frames after CALL to actually draw (D880 flips first, VRAM
# fills later). Tick until the active tilemap has substantial non-zero content.
def base():
...
pb.screen.image.save(OUTDIR+"/crystal_arena.png")
