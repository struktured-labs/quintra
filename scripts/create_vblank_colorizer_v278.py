#!/usr/bin/env python3
"""
v2.78: Hook-First Hybrid — 24-row sweep coverage

Based on v2.77 (VBK safety fix). Expands sweep from 18 rows to 24 rows to match
the game's tilemap copy area (24 cols x 24 rows at 0x42A7).

v2.77 bug: Sweep only covered rows 0-17. For idle states where the hook doesn't fire,
rows 18-23 kept palette 0 (uncolored). This caused 3-7 visible misses on states
like gargoyle boss and mid-level.

Fix: Phase thresholds changed from 18/36 to 24/48.
  - Phase 1: FFA5 = 0-23, 2 rows/frame → 12 frames (was 9)
  - Phase 2: FFA5 = 24-47, 1 row/frame, 24-frame maintenance cycle (was 18)
  - Hook sets FFA5 = 24 (Phase 2 start)

HRAM assignments:
  FF91: Hook flag (0x5A = hook ran, cleared by VBlank)
  FFA5: Sweep state counter (0-23=Phase1 initial, 24-47=Phase2 maintenance)
...
def create_enhanced_tilemap_copy(bg_table_addr: int, base_addr: int,
    ...
    Same as v2.75, but at the end sets:
      ...
      FFA5 = 24   (skip sweep Phase 1, go straight to maintenance)
...
if __name__ == "__main__":
    rom_path = build_v278()
    print(f"\nBuilt v2.78: {rom_path}")
    print(f"Hook-first hybrid with 24-row sweep coverage")
    print(f"Fix: Sweep expanded from 18 to 24 rows to match game's tilemap copy area")
    print(f"VBlank handler: [flag check] → optional BG sweep (24 rows) + CondPalette + OBJ + DMA")
