#!/usr/bin/env python3
"""v3.01 PRODUCTION: 24 single-row DI windows for attr buffer build.

Each DI window writes 24 tiles to bank 2 (~2000T DI), well under the
empirical safe budget of ~2000-3000T (per-DI freeze boundary). After each
row, EI lets Timer ISR service in the gap (~50T).

Total: 24 rows × ~2000T DI + 24 × ~50T EI = ~50K T-cycles per attr build.
Same total work as the original 8-chunk version, but split into safe DI sizes.

Per VBlank handler: build attr buffer → GDMA → done. Total handler time
~52K T, vastly exceeds VBlank (~4560T), so the handler overruns into
active rendering. That's fine for WRAM writes; GDMA needs LCD off or
H/V blank, so we still need to gate GDMA appropriately.

Output: rom/working/penta_dragon_dx_v301_per_row.gb
"""
import sys
...
from build_v301_gdma import (
    BG_TABLE_BYTES, WRAM_BG_TABLE, create_inline_tile_copy_tileonly,
    create_gdma_transfer,
)


def create_attr_per_row(bg_table_addr: int) -> bytes:
    """24 rows, each in its own DI window. ~2000T DI each, ~50T EI gap.

    Outer row counter lives in HRAM scratch (FFE0) to avoid DI-internal
    PUSH/POP nesting, which compounds with the inner tile-counter PUSH AF.

    Layout per row:
        DI
        FF70 = 2
        LD B, bg_table_hi
        LD A, 24 (tile counter)
        tile_loop: PUSH AF; LD A,[HL+]; LD C,A; LD A,[BC]; LD [DE],A; INC DE;
                   POP AF; DEC A; JR NZ
        FF70 = 1
        EI
    ...
    code.extend([0x21, 0xA0, 0xC1])               # LD HL, 0xC1A0 (tile src)
    code.extend([0x11, 0x00, 0xD0])               # LD DE, 0xD000 (attr dst, bank 2)
    code.extend([0x3E, 0x18])                     # LD A, 24 (row counter)
    code.extend([0xE0, 0xE0])                     # LDH [FFE0], A  (HRAM scratch row counter)
    ...
    code.extend([0x2A])                           # LD A, [HL+]   tile id
    code.extend([0x4F])                           # LD C, A
    code.extend([0x0A])                           # LD A, [BC]    bg_table[id]
    code.extend([0x12])                           # LD [DE], A    bank 2 write
    ...
    # DE += 8 (skip padding) — outside DI, safe
    code.extend([0x7B, 0xC6, 0x08, 0x5F, 0x30, 0x01, 0x14])

    # Row counter from HRAM
...
def build():
    ...
    w(A['sw'], bytes(sw))
    w(A['gd'], create_gdma_transfer())
    attr_code = create_attr_per_row(A['bt'])
    w(A['ac'], attr_code)
    print(f"  attr per-row: {len(attr_code)} bytes")
    ...
    code[gd_skip] = (len(code) - gd_skip - 1) & 0xFF
    ...
    code.extend([0x3E, 0x01, 0xEA, 0x03, 0xDF])   # DF03=1 (GDMA ready next frame)
    ...
    rom[0x14D] = chk
    out = Path("rom/working/penta_dragon_dx_v301_per_row.gb")
    out.write_bytes(rom)
    print(f"Wrote {out}")
