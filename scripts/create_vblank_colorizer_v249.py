#!/usr/bin/env python3
"""
v2.49: Faster BG via ROM Table (minimal change from v2.36 STABLE)

CHANGES from v2.36:
1. ROM LOOKUP TABLE at 0x6E00: 256-byte tile→palette table replaces
   inline CP/JR comparison chain. Constant-time lookup vs variable-time.
2. 0xFF ANTI-FLICKER FILTER: VRAM reads during PPU mode 3 return 0xFF.
   Without filter, this writes palette 0 over correct attributes = flicker.
   With filter, garbage reads are skipped. Attributes only go wrong→correct.
3. REGISTER SAVE (D/C) instead of PUSH/POP in loop. No stack writes.
4. TILE COUNT: 64 tiles/frame (up from 32). Full refresh in ~16 frames
   (~0.27s). 0xFF filter prevents flickering despite higher count.

IDENTICAL to v2.36 (proven STABLE):
- Dual tilemap: processes BOTH 0x9800 and 0x9C00 at each position
- FFEA/FFEB for position counter
- Same input hook (1 d-pad read + 8 button reads via loop)
- Same combined function order (BG → Palette → OBJ → DMA)
- Same shadow colorizer (both C000 and C100 buffers)
- Same palette loader (bosses, powerups, jets)
- Same OBJ tile colorizer
"""
import sys
import yaml
...
def create_bg_colorizer_rom_table(bg_table_hi: int) -> bytes:
    """
    BG colorizer with ROM table lookup + 0xFF filter - v2.49.

    KEY FIX: 0xFF anti-flicker filter. VRAM reads during PPU mode 3
    return 0xFF garbage. Without the filter, this maps to palette 0 and
    OVERWRITES correct attributes, causing visible flickering. The filter
    skips writes when tile=0xFF, so attributes only go wrong→correct,
    never correct→wrong.

    Uses D/C registers to save/restore HL (no PUSH/POP in loop).
    Processes both 0x9800 and 0x9C00 tilemaps at each position.
    64 tiles/frame → full refresh in ~16 frames (~0.27s).
    """
    TILES_PER_FRAME = 64
    ...
    code.extend([0x06, TILES_PER_FRAME])  # LD B, tiles
    ...
    code.extend([0x7E])         # LD A, [HL] (read tile from 9800)
    code.extend([0xFE, 0xFF])   # CP 0xFF (mode 3 garbage?)
    skip_9800_pos = len(code)
    code.extend([0x28, 0x00])   # JR Z, skip_9800 (placeholder)

    # ROM table lookup using D/C to save/restore HL
    code.extend([0x54])         # LD D, H (save VRAM high)
    code.extend([0x4D])         # LD C, L (save VRAM low)
    code.extend([0x26, bg_table_hi])  # LD H, table_hi
    code.extend([0x6F])         # LD L, A (L = tile_id)
    code.extend([0x5E])         # LD E, [HL] (E = palette from ROM table)
    code.extend([0x62])         # LD H, D (restore H)
    code.extend([0x69])         # LD L, C (restore L)
    ...
    code[skip_9800_pos + 1] = skip_9800_target - (skip_9800_pos + 2)
    ...
    code.extend([0xAF])         # XOR A
    code.extend([0xE0, 0x4F])   # LDH [0xFF4F], A  (VRAM bank 0)
    code.extend([0x7E])         # LD A, [HL] (read tile from 9C00)
    code.extend([0xFE, 0xFF])   # CP 0xFF
    skip_9c00_pos = len(code)
    code.extend([0x28, 0x00])   # JR Z, skip_9C00 (placeholder)

    # ROM table lookup for 0x9C00
    code.extend([0x54])         # LD D, H
    code.extend([0x4D])         # LD C, L
    code.extend([0x26, bg_table_hi])  # LD H, table_hi
    code.extend([0x6F])         # LD L, A
    code.extend([0x5E])         # LD E, [HL]
    code.extend([0x62])         # LD H, D
    code.extend([0x69])         # LD L, C
    code.extend([0x3E, 0x01])   # LD A, 1
    code.extend([0xE0, 0x4F])   # LDH [0xFF4F], A  (VRAM bank 1)
    code.extend([0x73])         # LD [HL], E

    skip_9c00_target = len(code)
    code[skip_9c00_pos + 1] = skip_9c00_target - (skip_9c00_pos + 2)
...
def create_bg_tile_table() -> bytes:
    ...
    Same categorization as v2.36's inline comparison, stored as a
    constant-time ROM table at 0x6E00.
    """
    table = bytearray(256)
    for tile in range(256):
        if tile <= 0x3F:
            table[tile] = 0    # Floor/edges/platforms
        elif tile <= 0x5F:
            table[tile] = 6    # Wall fill blocks
        elif tile <= 0x87:
...
def main():
    ...
    print("=== Penta Dragon DX v2.49 ===")
    print("Faster BG via ROM Table (minimal change from v2.36 STABLE)")
    print()
    print("Changes from v2.36:")
    print("  1. ROM table at 0x6E00: constant-time tile→palette lookup")
    print("  2. 0xFF anti-flicker filter: skip writes on mode 3 garbage reads")
    print("  3. 64 tiles/frame (up from 32): full refresh in 0.27s")
    print()
    print("Identical to v2.36:")
    print("  - Dual tilemap (9800 + 9C00)")
    print("  - Input debounce (1 d-pad + 8 button reads)")
    print("  - Both-buffer shadow OBJ colorization")
    print()
    ...
    lookup_table_addr = 0x6B00  # OBJ tile lookup (v2.36 legacy)
    bg_colorizer_addr = 0x6C00
    combined_addr = 0x6D00
    bg_tile_table_addr = 0x6E00  # NEW: 256-byte ROM table for BG palettes
    ...
    bg_tile_table = create_bg_tile_table()
    bg_table_hi = (bg_tile_table_addr >> 8) & 0xFF  # 0x6E
    bg_colorizer = create_bg_colorizer_rom_table(bg_table_hi)
    ...
    print(f"BG tile table: {len(bg_tile_table)} bytes at 0x{bg_tile_table_addr:04X} (ends 0x{bg_tile_table_addr + len(bg_tile_table):04X})")
    ...
    regions = [
        ...
        ('bg_tile_table', bg_tile_table_addr, len(bg_tile_table)),
    ]
    ...
    write_bank13(bg_tile_table_addr, bg_tile_table)
    ...
    # Verify BG tile table distribution
    pal_counts = [0] * 8
    for p in bg_tile_table:
        pal_counts[p] += 1
    ...
    print(f"\nROM patched successfully (v2.49)")
