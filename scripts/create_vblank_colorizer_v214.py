#!/usr/bin/env python3
"""
v2.14: Full-Screen BG Colorization with Stage Title Detection

Fixes v2.13's "STAGE ??" crash by checking 0xFFC2:
- 0xFFC1 = 0: Menu/title screen -> Skip BG colorization
- 0xFFC1 = 1 AND 0xFFC2 = 1: Stage title ("STAGE 01") -> Skip
- 0xFFC1 = 1 AND 0xFFC2 = 0: Actual gameplay -> Run colorization
"""
import sys
import yaml
...
def create_bg_colorizer_with_stage_check(lookup_table_addr: int) -> bytes:
    """
    Tile-based BG colorization with menu AND stage title detection.

    Check 0xFFC1 and 0xFFC2:
    - FFC1 = 0: Menu/title -> Return immediately
    - FFC1 = 1 AND FFC2 = 1: Stage title ("STAGE 01") -> Return
    ...
    code.extend([0xC8])                    # RET Z (return if A == 0)

    # Check 0xFFC2 - if 1, skip BG colorization (stage title)
    code.extend([0xF0, 0xC2])              # LDH A, [0xC2]
    code.append(0xB7)                      # OR A (set flags)
    code.extend([0xC0])                    # RET NZ (return if A != 0)

    # === Actual gameplay: Run BG colorization ===
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v214.gb")
    ...
    print("\n=== v2.14: BG Colorization with Stage Title Detection ===")
    print("  Checks 0xFFC1 AND 0xFFC2:")
    print("    FFC1=0           -> Skip (menu)")
    print("    FFC1=1, FFC2=1   -> Skip (stage title)")
    print("    FFC1=1, FFC2=0   -> Colorize (gameplay)")
    print()
    ...
    bg_colorizer = create_bg_colorizer_with_stage_check(lookup_table_addr)
