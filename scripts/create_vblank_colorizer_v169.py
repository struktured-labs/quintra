#!/usr/bin/env python3
"""
v1.69: BG Colorizer with stack-based VBK save

60 frame warmup, VBK saved on stack (not HRAM).
Avoids potential HRAM conflicts with game.
"""
import sys
import yaml
...
def create_bg_row_colorizer_lazy(lookup_table_addr: int) -> bytes:
    """BG colorizer with 60-frame warmup, VBK saved on stack."""
    WRAM_BUFFER = 0xDFF0
    WARMUP_FRAMES = 60  # ~1 second warmup
    code = bytearray()
    ...
    # If counter < WARMUP_FRAMES, just increment and return
    code.extend([0xFE, WARMUP_FRAMES])  # CP warmup
    skip_to_warmup = len(code)
    code.extend([0x38, 0x00])           # JR C, warmup (placeholder)

    # ===== MAIN COLORIZATION =====

    # Calculate row = (counter - WARMUP) & 0x1F
    ...
    # Save original VBK on stack (use C since we're done with row calc)
    ...
    # Restore original VBK from stack
    code.append(0xF1)                   # POP AF
    code.extend([0xE0, 0x4F])           # LDH [0xFF4F], A

    # warmup: increment counter (wraps at WARMUP + 32)
    ...
    wrap_val = WARMUP_FRAMES + 32
    code.extend([0xFE, wrap_val])       # CP wrap_val
    code.extend([0x38, 0x02])           # JR C, +2 (skip reset)
    code.extend([0x3E, WARMUP_FRAMES])  # LD A, warmup (wrap back)
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v169.gb")
    ...
    print("\n=== v1.69: BG Colorizer (Stack VBK) ===")
    print("  60 frame warmup, VBK saved on stack (not HRAM)")
    print()
