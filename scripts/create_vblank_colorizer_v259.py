#!/usr/bin/env python3
"""
v2.59: Interleaved dual-tilemap BG colorizer with SET/RES optimization

Uses INTERLEAVED read→write pattern: read 9800 tile → write 9800 attr →
read 9C00 tile → write 9C00 attr. This is CRITICAL because VRAM writes
during PPU mode 3 are silently dropped, and interleaved read→write has
only ~30M gap (writes succeed) vs batched ~65M gap (writes dropped).

Key changes over v2.58:
...
- Hardcoded wrap at CP 0x9C (no HRAM read needed in loop)
...
def main():
    ...
    print("=== Penta Dragon DX v2.59 ===")
    print("Interleaved dual-tilemap + SET/RES + ROM table + 109 tiles/frame + v2.36 hook")
    print()
