#!/usr/bin/env python3
"""
v1.95: Incremental address (no calculation)

- Store address in HRAM, increment by 8 each frame
- No complex bit shifting calculations
- Wrap when reaching 0x9C00
"""
import sys
import yaml
...
def create_bg_row_colorizer_lazy(lookup_table_addr: int) -> bytes:
    """v1.95: Incremental address stored in HRAM.

    Uses HRAM 0xFFC5 (low) and 0xFFC6 (high) for 16-bit address.
    Increments by 8 each frame, wraps at 0x9C00 back to 0x9800.
    NO complex bit shifting - just simple 16-bit increment.
    """
    HRAM_COUNTER = 0xC4   # Frame counter for warmup
    HRAM_ADDR_LO = 0xC5   # Address low byte
    HRAM_ADDR_HI = 0xC6   # Address high byte
    WARMUP_FRAMES = 60
    TILES_PER_FRAME = 8
    code = bytearray()
    ...
    # Read frame counter for warmup check
    code.extend([0xF0, HRAM_COUNTER])   # LDH A, [0xFFC4]
    code.append(0x47)                   # LD B, A (save counter)

    # If counter < WARMUP, skip to increment
    code.extend([0xFE, WARMUP_FRAMES])  # CP 60
    skip_to_inc = len(code)
    code.extend([0x38, 0x00])           # JR C, just_increment (placeholder)

    # Load address from HRAM into HL
    code.extend([0xF0, HRAM_ADDR_LO])   # LDH A, [0xFFC5]
    code.append(0x6F)                   # LD L, A
    code.extend([0xF0, HRAM_ADDR_HI])   # LDH A, [0xFFC6]
    code.append(0x67)                   # LD H, A

    # If H < 0x98, initialize to 0x9800
    code.extend([0xFE, 0x98])           # CP 0x98
    code.extend([0x30, 0x04])           # JR NC, +4 (skip init if >= 0x98)
    ...
    # Restore VRAM pos, write palette
    ...
    # After loop, HL points to next address. Save it.
    # Check if H >= 0x9C (reached end of tilemap)
    code.append(0x7C)                   # LD A, H
    code.extend([0xFE, 0x9C])           # CP 0x9C
    code.extend([0x38, 0x03])           # JR C, +3 (skip wrap if < 0x9C)
    code.extend([0x21, 0x00, 0x98])     # LD HL, 0x9800 (wrap)

    # Save address to HRAM
    code.append(0x7D)                   # LD A, L
    code.extend([0xE0, HRAM_ADDR_LO])   # LDH [0xFFC5], A
    code.append(0x7C)                   # LD A, H
    code.extend([0xE0, HRAM_ADDR_HI])   # LDH [0xFFC6], A

    # just_increment: increment frame counter
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v195.gb")
    ...
    print("\n=== v1.95: Incremental address (no calc) ===")
    print("  Address stored in HRAM, increments by 8 each frame")
    print("  NO complex bit shifting - simple 16-bit increment")
    print()
