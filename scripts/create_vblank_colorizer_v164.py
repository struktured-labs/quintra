#!/usr/bin/env python3
"""
v1.64: TEST - VBK switching

BG colorizer does LCD check + counter + VBK switch.
Tests if VBK switching during VBlank breaks things.
"""
import sys
import yaml
...
def create_bg_row_colorizer_lazy(lookup_table_addr: int) -> bytes:
    """TEST: LCD check + counter + VBK switching. No VRAM read/write."""
    code = bytearray()
    ...
    # Read counter, increment, wrap, write back
    code.extend([0xF0, 0xC0])           # LDH A, [0xFFC0]
    code.append(0x3C)                   # INC A
    code.extend([0xE6, 0x3F])           # AND 0x3F
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v164.gb")
    ...
    print("\n=== v1.64: TEST - VBK Switching ===")
    print("  BG colorizer: LCD check + counter + VBK switch")
    print("  Testing if VBK switching during VBlank breaks things")
    print()
