#!/usr/bin/env python3
"""Penta Dragon DX v2.96 — v295 base + empirically-calibrated bg_tile_table.

The v295 architecture (no trampoline, cond_pal before FFC1) keeps phantom
sound clean but used a NAIVE bg_tile_table (raw tile-ID range buckets) that
gave wall-edge artifacts and inconsistent dungeon BG colorization.

v296 substitutes a CALIBRATED bg_table built from 16 real level-1 dungeon
save states. The agent run that produced it dumped tile pixel patterns,
clustered by visual content, and confirmed against the YAML semantics:
  pal0 = floor/empty, pal1 = items (88 IDs), pal2 = decorative corners,
  pal5 = hazards/spikes, pal6 = walls/stone.

Net effect: wall slate color (pal6) is consistent across the full
wall-tile range; spike hazards (0x2A-0x2E, 0x3A-0x3D, 0x47, 0x57)
render in red/orange (pal5) instead of slate; items use pal1.
"""
import sys
...
# Calibrated table from /tmp/calibrated_bg_table.py (agent run 2026-05-12).
# 256 bytes — tile_id -> palette index (0..7) | 0xFF sentinel.
BG_TABLE_BYTES = bytes([
...
def build_v296():
    ...
    # *** v296 change: use the calibrated bg_table ***
    ...
    # Colorize handler — same as v295 (cond_pal before FFC1, else gameplay-gated)
    code = bytearray()
