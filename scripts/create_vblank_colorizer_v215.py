#!/usr/bin/env python3
"""
v2.15: Incremental BG Colorization (VBlank-Safe)

FIXES v2.13/v2.14 freeze by processing only 32 tiles per frame instead of 576.
- 576 tiles at ~124 cycles/tile = 71,424 cycles (way over VBlank's ~4,560)
- 32 tiles at ~100 cycles/tile = 3,200 cycles (fits in VBlank)

Approach:
- Use HRAM counter at 0xFFD0 to track current row (0-17)
- Each VBlank, colorize one row (32 tiles)
- After 18 frames, full screen is colorized
- Repeat continuously

Game state checks (from v2.14):
- FFC1 = 0: Menu/title -> Skip BG colorization
- FFC1 = 1 AND FFC2 = 1: Stage title ("STAGE 01") -> Skip
- FFC1 = 1 AND FFC2 = 0: Actual gameplay -> Run colorization
"""
import sys
import yaml
...
def create_bg_colorizer_incremental(lookup_table_addr: int, row_counter_addr: int = 0xD0) -> bytes:
    """
    Incremental BG colorization - process ONE ROW (32 tiles) per VBlank.

    Uses HRAM at row_counter_addr (0xFFD0) to track current row.
    Row cycles from 0-17, wrapping around.

    ~32 tiles × 100 cycles = 3,200 cycles per frame (fits in VBlank)
    Full screen colorized every 18 frames (~0.3 seconds at 60fps)
    """
    code = bytearray()

    # Check 0xFFC1 - if 0, skip BG colorization (menu)
    code.extend([0xF0, 0xC1])              # LDH A, [0xC1]
    code.append(0xB7)                      # OR A
    code.extend([0xC8])                    # RET Z

    # Check 0xFFC2 - if 1, skip BG colorization (stage title)
    code.extend([0xF0, 0xC2])              # LDH A, [0xC2]
    code.append(0xB7)                      # OR A
    code.extend([0xC0])                    # RET NZ

    # === Actual gameplay: Process ONE row ===

    # Get current row from HRAM
    code.extend([0xF0, row_counter_addr])  # LDH A, [row_counter]

    # Calculate VRAM address: 0x9800 + (row * 32)
    # row * 32 = row << 5
    # HL = 0x9800 + A*32
    code.append(0x5F)                      # LD E, A (save row)
    code.append(0x87)                      # ADD A, A  (A*2)
    code.append(0x87)                      # ADD A, A  (A*4)
    code.append(0x87)                      # ADD A, A  (A*8)
    code.append(0x87)                      # ADD A, A  (A*16)
    code.append(0x87)                      # ADD A, A  (A*32)
    code.append(0x6F)                      # LD L, A
    code.extend([0x26, 0x98])              # LD H, 0x98
    # Now HL = 0x9800 + row*32 (assuming no overflow into H, which works for rows 0-7)
    # For rows 8-17, we need to handle the carry
    # Actually let's do it properly with 16-bit math

    # Restart with proper calculation
    code.clear()
    ...
    # Use DE for the multiplication
    code.append(0x5F)                      # LD E, A
    code.extend([0x16, 0x00])              # LD D, 0
    # DE = row
    # Shift left 5 times for *32
    code.append(0xCB)                      # SLA E (shift left)
    code.append(0x23)
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v215.gb")
    ...
    print("\n=== v2.15: Incremental BG Colorization ===")
    print("  Processes ONE ROW (32 tiles) per VBlank")
    print("  ~3,200 cycles/frame (fits in VBlank)")
    print("  Full screen colorized every 18 frames (~0.3s)")
    print()
    ...
    bg_colorizer = create_bg_colorizer_incremental(lookup_table_addr)
