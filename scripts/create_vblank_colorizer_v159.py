#!/usr/bin/env python3
"""
v1.59: Lazy VRAM Bank 1 Init with LCD Check

The trampoline approach (patching 0x4003) crashes on fresh boot.
Instead, do VRAM bank 1 init lazily during VBlank:

1. Keep original startup init at 0x4003 (no patching!)
2. Use HRAM 0xFFC0 as init counter (0-31 = initializing, 0xFF = done)
3. During VBlank, if counter < 32, init one row and increment
4. After 32 rows, set counter to 0xFF and switch to continuous colorization
5. During continuous mode, update rows round-robin for scroll handling

This is slower (32 frames to init) but avoids the startup crash.
"""
import sys
import yaml
...
def create_bg_row_colorizer_lazy(lookup_table_addr: int) -> bytes:
    """
    BG row colorizer with lazy init.

    0xFFC0 = row counter / state:
      0-31: initializing (process row N, increment)
      32+: continuous mode (process row N % 32, increment, wrap at 64)

    Uses WRAM buffer at 0xDFF0 for 2-phase approach.

    IMPORTANT: Skips if LCD is off (early boot) to prevent crashes.
    """
    WRAM_BUFFER = 0xDFF0

    code = bytearray()

    # Check if LCD is on (LCDC bit 7). Skip BG colorization during early boot.
    code.extend([0xF0, 0x40])           # LDH A, [0x40] ; Read LCDC
    code.extend([0xCB, 0x7F])           # BIT 7, A     ; Check LCD enable bit
    code.append(0xC8)                   # RET Z        ; Return if LCD off

    code.extend([0xF5, 0xC5, 0xD5, 0xE5])

    # Get counter from 0xFFC0
    code.extend([0xF0, 0xC0])           # LDH A, [0xFFC0]
    code.append(0x47)                   # LD B, A (save full counter)

    # Calculate row = counter & 0x1F
    code.extend([0xE6, 0x1F])           # AND 0x1F
    code.append(0x4F)                   # LD C, A (save row number)

    # Ensure VBK = 0 for reading tiles
    code.append(0xAF)
    code.extend([0xE0, 0x4F])

    # Calculate VRAM address for this row: 0x9800 + row * 32
    ...
    # Phase 1: Read 32 tiles, lookup palettes, store in WRAM
    ...
    # Phase 2: Switch to VBK=1 and copy buffer
    ...
    # Increment counter (wrap at 64 for continuous mode)
    code.append(0x78)                   # LD A, B (original counter)
    code.append(0x3C)                   # INC A
    code.extend([0xE6, 0x3F])           # AND 0x3F (wrap at 64)
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v159.gb")
    ...
    print("\n=== v1.59: Lazy VRAM Bank 1 Init with LCD Check ===")
    print("  NO patching of 0x4003 - keeps original startup intact")
    print("  Skips BG colorization when LCD is off (early boot)")
    print("  BG colorization happens lazily during VBlank")
    print("  Full init in ~0.5 seconds, then continuous update")
    print()

    rom = bytearray(input_rom.read_bytes())
    apply_all_display_patches(rom)

    # NO startup hook! Keep 0x4003 original.

    # Lookup table and code in bank 13
    ...
    print(f"BG row colorizer (lazy): {len(bg_colorizer)} bytes at 0x{bg_colorizer_addr:04X}")
