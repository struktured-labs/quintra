#!/usr/bin/env python3
"""
v1.17: Multi-Color BG Item Colorization

Extends v1.16 with different colors for different item types:
  - Dragon powerup (0xC0-0xDF): BG Palette 1 (gold/yellow)
  - Flash item (0xAA-0xBF): BG Palette 4 (cyan/teal)
  - Other items (0x88-0xA9): BG Palette 5 (red/orange)
  - Normal tiles: BG Palette 0

Uses GBC HDMA hardware for efficient attribute copying.
"""
import sys
import yaml
...
def create_attr_buffer_prep() -> bytes:
    ...
    # Check if tile < 0x88 (not an item)
    code.extend([0xFE, 0x88])        # CP 0x88
    jumps_to_fix.append((len(code), 'not_item'))
    code.extend([0x38, 0x00])        # JR C, not_item

    # Check if tile >= 0xE0 (not an item)
    code.extend([0xFE, 0xE0])        # CP 0xE0
    jumps_to_fix.append((len(code), 'not_item'))
    code.extend([0x30, 0x00])        # JR NC, not_item

    # Now we know 0x88 <= tile < 0xE0 (it's an item)
    # Check for Dragon powerup (0xC0-0xDF) → Palette 1
    code.extend([0xFE, 0xC0])        # CP 0xC0
    jumps_to_fix.append((len(code), 'dragon_powerup'))
    code.extend([0x30, 0x00])        # JR NC, dragon_powerup

    # Check for Flash item (0xAA-0xBF) → Palette 2
    code.extend([0xFE, 0xAA])        # CP 0xAA
    jumps_to_fix.append((len(code), 'flash_item'))
    code.extend([0x30, 0x00])        # JR NC, flash_item
    ...
    labels['flash_item'] = len(code)
    code.extend([0x3E, 0x04])        # LD A, 4 (cyan for flash)
    jumps_to_fix.append((len(code), 'write_attr'))
    code.extend([0x18, 0x00])        # JR write_attr

    labels['dragon_powerup'] = len(code)
    code.extend([0x3E, 0x01])        # LD A, 1 (gold for dragon)
    jumps_to_fix.append((len(code), 'write_attr'))
    code.extend([0x18, 0x00])        # JR write_attr
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v117.gb")
