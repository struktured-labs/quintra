#!/usr/bin/env python3
"""
Test: Copy palette data to WRAM first, then load palettes from WRAM.
This tests if the issue is ROM reads during VBlank.
"""
import sys
import yaml
...
def create_wram_copy_then_load(rom_addr: int, wram_addr: int) -> bytes:
    """
    Step 1: Copy 64 bytes from ROM to WRAM
    Step 2: Load palettes from WRAM
    """
    code = bytearray()

    # Step 1: Copy ROM -> WRAM (64 bytes)
    code.extend([0x21, rom_addr & 0xFF, (rom_addr >> 8) & 0xFF])  # LD HL, rom_addr
    code.extend([0x11, wram_addr & 0xFF, (wram_addr >> 8) & 0xFF])  # LD DE, wram_addr
    code.extend([0x06, 0x40])  # LD B, 64
    copy_loop = len(code)
    code.append(0x2A)  # LD A, [HL+]
    code.append(0x12)  # LD [DE], A
    code.append(0x13)  # INC DE
    code.append(0x05)  # DEC B
    copy_offset = copy_loop - len(code) - 2
    code.extend([0x20, copy_offset & 0xFF])

    # Step 2: Load OBJ palettes from WRAM
    code.extend([0x21, wram_addr & 0xFF, (wram_addr >> 8) & 0xFF])  # LD HL, wram_addr
    ...
    pal_offset = pal_loop - len(code) - 2
    code.extend([0x20, pal_offset & 0xFF])
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_WRAMLOOP.gb")
    palette_path = Path("palettes/penta_palettes_v097.yaml")

    print("=== Loop from WRAM (copy ROM->WRAM first) ===")
    ...
    rom_data_addr = 0x6800  # Palette data in ROM bank 13
    wram_addr = 0xD800      # WRAM buffer for palettes
    palette_loader_addr = 0x6850
    colorizer_addr = 0x68B0
    shadow_main_addr = 0x68D0
    combined_addr = 0x6900

    palette_loader = create_wram_copy_then_load(rom_data_addr, wram_addr)
    ...
    write_to_bank13(rom_data_addr, obj_data)
