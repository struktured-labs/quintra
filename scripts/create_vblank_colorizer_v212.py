#!/usr/bin/env python3
"""
v2.12: Full-Screen Tile-Based BG Colorization

Improvements over v2.10:
1. Process 576 tiles (18 rows × 32 columns) to cover entire visible area
2. Optimized tile mappings based on observed game tiles

Tile categories observed:
- 0x00-0x1F (0-31): Floor/platform base tiles -> Palette 0 (blue)
- 0x20-0x3F (32-63): Wall/decoration tiles -> Palette 2 (purple)
- 0x40-0x7F (64-127): More walls/decorations -> Palette 2 (purple)
- 0x80-0xBF (128-191): Items/special tiles -> Palette 1 (gold)
- 0xC0-0xDF (192-223): More items -> Palette 1 (gold)
- 0xE0-0xFE (224-254): Border/edge tiles -> Palette 2 (purple)
- 0xFF: Void/empty -> Palette 0
"""
import sys
import yaml
...
def create_tile_palette_lookup() -> bytes:
    ...
    for i in range(256):
        if i < 0x20:
            # Floor tiles (0x00-0x1F) -> Palette 0 (blue dungeon)
            lookup[i] = 0
        elif i < 0x80:
            # Wall/decoration tiles (0x20-0x7F) -> Palette 2 (purple)
            lookup[i] = 2
        elif i < 0xE0:
            # Item tiles (0x80-0xDF) -> Palette 1 (gold)
            lookup[i] = 1
        elif i == 0xFF:
            # Void -> Palette 0
            lookup[i] = 0
        else:
            # Border tiles (0xE0-0xFE) -> Palette 2 (purple)
            lookup[i] = 2
...
def create_bg_colorizer(lookup_table_addr: int) -> bytes:
    """
    Tile-based BG colorization - full screen version.

    Process 576 tiles (18 rows × 32 columns).
    Uses optimized loop with E register for value.
    """
    code = bytearray()

    # Process 0x9800-0x9A3F (576 bytes = 18 rows × 32 columns)
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v212.gb")
    ...
    print("\n=== v2.12: Full-Screen Tile-Based BG Colorization ===")
    print("  Process 576 tiles (18 rows × 32 columns)")
    print("  Tile-based palette assignment:")
