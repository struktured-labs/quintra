...
CRITICAL FIX over v2.36:
- BG position counter moved from FFEA/FFEB to FF91/FFA5
- FFEA has 4 reads + 13 writes in game code (countdown timer in VBlank!)
- FFEB has 9 reads + 6 writes in game code (toggle flag, branching)
- FF91 and FFA5 have ZERO references in the entire original ROM
- This eliminates mutual state corruption between our colorizer and the game

Everything else identical to v2.36 (known stable):
- 1 d-pad read + 8 button reads via loop
- 32 tiles/frame BG colorizer (no 0xFF filter)
- Both-buffer OBJ colorizer
- Inline tile->palette comparison chain
"""
import sys
import yaml
...
def main():
    ...
    print("=== Penta Dragon DX v2.52 ===")
    print("Safe HRAM Fix (based on v2.36)")
    print()
    print("FIX in v2.52:")
    print("  BG counter: FF91/FFA5 (was FFEA/FFEB - game conflicts!)")
    print("  FFEA: 4 reads + 13 writes in game (countdown timer)")
    print("  FFEB: 9 reads + 6 writes in game (toggle flag)")
    print("  FF91/FFA5: 0 references in game (truly free)")
    print()
