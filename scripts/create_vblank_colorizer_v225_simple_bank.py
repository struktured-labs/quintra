#!/usr/bin/env python3
"""
v2.23: Fix Input Handler - Proper Delays and Return Value

CRITICAL FIX: Input handler now matches original timing:
- 8 delay reads for d-pad (was 2, causing unreliable input)
- Returns input value in A register (game expects this)
- Stores to B register before returning

Plus v2.22 fixes:
- Palette loader runs only once (not every frame)
...
def create_palette_loader(palette_data_addr: int, gargoyle_addr: int, spider_addr: int) -> bytes:
    """
    Palette loader - ONE-SHOT version (loads only once).
    Uses HRAM 0xFFE1 as "loaded" flag to prevent reloading every frame.
    """
    code = bytearray()

    # Check if already loaded
    code.extend([0xF0, 0xE1])              # LDH A, [0xFFE1]
    code.extend([0xB7])                    # OR A
    code.extend([0xC0])                    # RET NZ (return if already loaded)
    ...
    code.extend([0xE0, 0xE1])              # LDH [0xFFE1], A
...
def create_combined_with_dma(palette_loader_addr: int, obj_main_addr: int, bg_colorizer_addr: int) -> bytes:
    ...
    # BG colorizer DISABLED for v2.24 - it causes display corruption
...
def create_vblank_hook_with_input(combined_func_addr: int) -> bytes:
    ...
    # Simple bank switching like v1.09 (no 0x4000 write)
    hook_code = bytearray([
...
def main():
    ...
    output_rom = project_root / "rom/working/penta_dragon_dx_v225_simple_bank.gb"
    ...
    print("\n=== v2.25: Simple Bank Switching (v1.09 Style) ===")
    print("  FIX: Use v1.09's simple bank switching (no 0x4000 write)")
    print("  Input handler: Proper timing with 8 delays")
    print("  Palette loader: One-shot (runs once only)")
    print("  OBJ: Tile-based colorization")
    print("  BG: DISABLED for this test")
    print()
    ...
    print("\n=== v2.25 (Simple Bank) Build Complete ===")
