#!/usr/bin/env python3
"""
v1.63: TEST - Counter access at 0xFFC0

BG colorizer does LCD check + counter read/increment.
Tests if 0xFFC0 conflicts with game.
"""
import sys
import yaml
...
def create_bg_row_colorizer_lazy(lookup_table_addr: int) -> bytes:
    """TEST: LCD check + counter read/increment only. No VRAM."""
    code = bytearray()
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v163.gb")
    ...
    print("\n=== v1.63: TEST - Counter at 0xFFC0 ===")
    print("  BG colorizer: LCD check + counter read/increment")
    print("  Testing if 0xFFC0 conflicts with game")
    print()
