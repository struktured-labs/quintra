#!/usr/bin/env python3
"""
v1.67: Full BG Colorizer with VBK save/restore

Full BG tile colorization with:
- LCD check (skip if LCD off)
- 128 frame warmup before starting
- VBK save/restore to avoid corruption
- 2-phase: read tiles (VBK=0) then write attrs (VBK=1)
"""
import sys
import yaml
...
def create_bg_row_colorizer_lazy(lookup_table_addr: int) -> bytes:
    """Full BG colorizer with LCD check, warmup, VBK save/restore."""
    WRAM_BUFFER = 0xDFF0
    code = bytearray()

    # Check if LCD is on. Skip if off (early boot).
    ...
    # If counter < 128, just increment and return (warmup period)
    code.extend([0xFE, 0x80])           # CP 128
    skip_to_warmup = len(code)
    code.extend([0x38, 0x00])           # JR C, warmup (placeholder)

    # ===== MAIN COLORIZATION (counter >= 128) =====

    # Calculate row = (counter - 128) & 0x1F
    code.extend([0xD6, 0x80])           # SUB 128
    code.extend([0xE6, 0x1F])           # AND 0x1F
    code.append(0x4F)                   # LD C, A (row number 0-31)
    ...
    code.extend([0xE6, 0x18])           # AND 0x18 (high bits for rows 8-31)
    code.append(0x0F)                   # RRCA
    code.append(0x0F)                   # RRCA
    code.append(0x0F)                   # RRCA
    code.append(0x84)                   # ADD H
    code.append(0x67)                   # LD H, A (HL = VRAM row address)

    code.append(0xE5)                   # PUSH HL (save VRAM addr)

    # Phase 1: Read 32 tiles from VBK=0, lookup palettes, store in WRAM
    ...
    code.extend([0x26, (lookup_table_addr >> 8) & 0xFF])  # LD H, lookup_hi
    code.append(0x7E)                   # LD A, [HL] (lookup palette)
    code.append(0xE1)                   # POP HL
    code.append(0x12)                   # LD [DE], A (store in buffer)
    ...
    code.extend([0xE0, 0xC1])           # LDH [0xFFC1], A (temp save)

    # Phase 2: Switch to VBK=1 and write attributes
    ...
    # Restore original VBK
    code.extend([0xF0, 0xC1])           # LDH A, [0xFFC1]
    code.extend([0xE0, 0x4F])           # LDH [0xFF4F], A

    # warmup: increment counter (wraps at 160 = 128 + 32 rows)
    warmup_label = len(code)
    code[skip_to_warmup + 1] = (warmup_label - skip_to_warmup - 2) & 0xFF

    code.append(0x78)                   # LD A, B (original counter)
    code.append(0x3C)                   # INC A
    code.extend([0xFE, 0xA0])           # CP 160 (128 + 32)
    code.extend([0x38, 0x02])           # JR C, +2 (skip reset)
    code.extend([0x3E, 0x80])           # LD A, 128 (wrap back to start of colorization)
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v167.gb")
    ...
    print("\n=== v1.67: Full BG Colorizer ===")
    print("  LCD check + 128 frame warmup + VBK save/restore")
    print("  2-phase: read tiles (VBK=0) then write attrs (VBK=1)")
    print()
