#!/usr/bin/env python3
"""2 rows in 1 DI window, but row counter in HRAM scratch instead of stack.

If this PASSES → outer PUSH AF (nested stack inside DI) was the freezing factor.
If this FREEZES → something else in row machinery (DE += 8, LD B reset, etc.).
"""
import sys
...
def create_2rows_hram(bg_table_addr):
    bg_table_hi = (bg_table_addr >> 8) & 0xFF
    code = bytearray()
    code.extend([0xC5, 0xD5, 0xE5, 0xF5])
    code.extend([0x21, 0xA0, 0xC1])               # HL = 0xC1A0
    code.extend([0x11, 0x00, 0xD0])               # DE = 0xD000
    code.extend([0x3E, 0x02])                     # row count = 2
    code.extend([0xE0, 0xE0])                     # [FFE0] = A
    code.extend([0xF3])                           # DI
    ...
    # Row counter via HRAM (inside DI — but FFE0 access is fast LDH)
    code.extend([0xF0, 0xE0, 0x3D, 0xE0, 0xE0])
...
def build():
    ...
    rom[0x14D] = chk
    out = Path("rom/working/penta_dragon_dx_v301_2rows_hram.gb")
    out.write_bytes(rom)
    print(f"Wrote {out}")
