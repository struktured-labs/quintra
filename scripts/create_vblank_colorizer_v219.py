#!/usr/bin/env python3
"""
v2.19: Restore Working OBJ Colorization (v1.09 baseline)

PURPOSE: Fix the OBJ regression by removing BG colorization entirely.
Based on v1.09 which was confirmed working by the user.

KEY CHANGE: No BG colorization at all. This is to verify that v2.17/v2.18's
BG colorizer (with VRAM bank switching) was causing the sprite color issues.

OBJ Colorization (from v1.09):
- Uses 0xFFBE to determine Sara palette (D register):
  - 0xFFBE = 0: Sara Witch -> Palette 2
  - 0xFFBE != 0: Sara Dragon -> Palette 1
- Tile-based colorization for enemies
- Boss detection via 0xFFBF with dynamic palette loading

Tile ranges (OBJ):
- 0x00-0x0F: Effects/projectiles -> Palette 0 (white/gray)
- 0x10-0x1F: More effects -> Palette 0
- 0x20-0x27: Sara Witch -> Palette from D register (2)
- 0x28-0x2F: Sara Dragon -> Palette from D register (1)
- 0x30-0x3F: Crow -> Palette 4 (default enemy)
...
Boss override (0xFFBF):
- 0xFFBF = 1 -> Gargoyle mode, all enemies palette 6
- 0xFFBF = 2 -> Spider mode, all enemies palette 7
...
def create_tile_based_colorizer() -> bytes:
    ...
    RESTORED FROM v1.09 - proven working.

    Input: HL = pointer to flags byte, D = Sara palette, E = boss flag (0=normal, 6/7=boss mode)

    Logic:
    - Slots 0-3: Sara (palette D)
    - Tile < 0x10: Projectile (palette 0)
    - If E != 0: Boss mode -> use E as palette for all enemies
    - Otherwise tile-based coloring
    """
    code = bytearray()
    labels = {}
    jumps_to_fix = []

    # LD B, 40 (loop counter for 40 sprites)
    ...
    code.extend([0x38, 0x00])        # JR C, sara_palette (if slot < 4)

    # Read tile (at HL-1, since HL points to flags byte)
    ...
    # boss_palette: (boss/miniboss mode - all enemies use E palette)
    ...
    code.append(0x7E)                # LD A, [HL] (read current flags)
    ...
    # Next sprite: HL += 4 to move to next sprite's flags byte
...
def create_shadow_colorizer_main(colorizer_addr: int) -> bytes:
    """
    Main routine that colorizes BOTH shadow buffers (0xC000 and 0xC100).
    RESTORED FROM v1.09 - uses 0xFFBE for Sara form detection.

    Sets up:
    - D = Sara palette (2 for Witch, 1 for Dragon based on 0xFFBE)
    - E = Boss override palette (0=none, 6=gargoyle, 7=spider based on 0xFFBF)
    """
    code = bytearray()
    code.extend([0xF5, 0xC5, 0xD5, 0xE5])  # PUSH AF, BC, DE, HL

    # Determine Sara palette (D) from 0xFFBE
    # 0xFFBE = 0 -> Witch (palette 2)
    # 0xFFBE != 0 -> Dragon (palette 1)
    ...
    # HL points to flags byte of first sprite (Y, X, Tile, Flags = 4 bytes, so Flags at +3)
...
def create_combined_with_dma(palette_loader_addr: int, shadow_main_addr: int) -> bytes:
    ...
    NO BG colorization in this version - just OBJ.
    """
    code = bytearray()
...
def create_vblank_hook_with_input(combined_func_addr: int) -> bytes:
    ...
    Includes simplified input reader followed by bank switch + call.
    """
    # Simplified input reader (from original v1.09)
    ...
    # Bank switch to 13, call combined function, switch back to 1
    hook_code = bytearray([
        0x3E, 0x0D, 0xEA, 0x00, 0x20,  # LD A, 13; LD [0x2000], A (switch to bank 13)
        0xCD, combined_func_addr & 0xFF, combined_func_addr >> 8,  # CALL combined
        0x3E, 0x01, 0xEA, 0x00, 0x20,  # LD A, 1; LD [0x2000], A (switch back to bank 1)
        0xC9,  # RET
    ])
...
def main():
    ...
    output_rom = project_root / "rom/working/penta_dragon_dx_v219.gb"
    ...
    print("\n=== v2.19: Restore Working OBJ Colorization ===")
    print("  Based on v1.09 (confirmed working)")
    print("  PURPOSE: Verify OBJ regression was caused by BG colorizer")
    print()
    print("  OBJ colorization (from v1.09):")
    print("    - 0xFFBE -> Sara palette (D register)")
    print("    - 0xFFBF -> Boss override (E register)")
    print("    - Tile-based coloring for enemies")
    print()
    print("  NO BG colorization (removed)")
    print()
    ...
    print("\n=== v2.19 Build Complete ===")
    print("Test this version to verify OBJ colorization works.")
    print("If Sara colors are correct, the BG colorizer was the regression cause.")
