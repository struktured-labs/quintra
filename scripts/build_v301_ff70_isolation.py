#!/usr/bin/env python3
"""v3.01 FF70 ISOLATION TEST — does the FF70 op alone freeze, or is attr_comp the culprit?

Per docs/v301_gdma_freeze_diagnosis.md "Suggested debugging path" step 1:
> Build a v3.01 that writes FF70=2, immediately FF70=1 (no other work)
> in the warm path, and check if THAT freezes. Isolates the FF70 op itself.

This script is identical to build_v301_gdma.py EXCEPT the colorize handler
adds a minimal `DI; FF70=2; FF70=1; EI` (9 bytes) inside the FFC1 gate.
No attr_computation, no bank-2 writes, no GDMA. Just the bank-switch op pair.

Outcomes:
- Freezes at FFC1=0→1 transition  → FF70 op alone is the problem.
  Next: try alternative bank, or abandon FF70 path entirely.
- Boots and runs through gameplay → attr_computation logic is the culprit.
  Next: instrument attr_computation to find the exact byte that freezes.

Output: rom/working/penta_dragon_dx_v301_ff70_test.gb
"""
import sys
...
from build_v301_gdma import (
    BG_TABLE_BYTES, WRAM_BG_TABLE, WRAM_BG_TABLE_HI,
...
def build():
    ...
    output_path = Path("rom/working/penta_dragon_dx_v301_ff70_test.gb")
    ...
    # ============================================================
    # COLORIZE HANDLER with FF70 isolation test inside FFC1 gate
    # ============================================================
    code = bytearray()
    ...
    # COLD-BOOT PATH (identical to build_v301_gdma)
    ...
    # WARM PATH
    code.extend([0xCD, cond_pal_addr & 0xFF, (cond_pal_addr >> 8) & 0xFF])  # cond_pal
    code.extend([0xCD, bg_sweep_addr & 0xFF, (bg_sweep_addr >> 8) & 0xFF])  # bg_sweep

    # FFC1 gate: includes OAM DMA, shadow colorizer, AND the FF70 isolation test
    ...
    # ─── FF70 ISOLATION TEST: 9 bytes ───
    # Identical to the wrapping used by attr_computation in build_v301_gdma:
    # DI; LD A,2; LDH [70],A; LD A,1; LDH [70],A; EI
    code.extend([0xF3])                       # DI
    ...
    print(f"  (FF70 isolation test active: DI; FF70=2; FF70=1; EI inside FFC1 gate)")

    # VBLANK HOOK at 0x0824 (identical)
    hook = bytearray([
        0xF0, 0x99, 0xF5,
