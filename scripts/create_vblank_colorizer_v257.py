#!/usr/bin/env python3
"""
v2.57: Alternating tilemap BG colorizer (same speed as v2.57)

Based on v2.57 with one key change:
- Instead of following LCDC bit 3 (active tilemap only), ALTERNATES between
  0x9800 and 0x9C00 each frame via XOR 0x04 toggle on FFA9.
- Both tilemaps stay colorized, eliminating accuracy drops on tilemap flips.
- Same CPU cost as v2.57 (253 tiles/frame from one tilemap).
- When a tilemap flip happens, the newly-active tilemap was colorized just
  1 frame ago → effectively no accuracy drop.

v2.56 proved dual-tilemap processing gives 98.5-99.6% during scrolling,
but was too slow (181 tiles × 2 tilemaps = 362 VRAM ops/frame).
v2.57 gives similar accuracy at v2.57's speed.
"""
import sys
import yaml
...
def create_bg_colorizer(bg_table_addr: int) -> bytes:
    """BG colorizer: ALTERNATING tilemap + ROM table lookup + 253 tiles/frame.

    Alternates between 0x9800 and 0x9C00 each frame via XOR toggle.
    Both tilemaps stay colorized, so tilemap flips don't cause accuracy drops.
    Same CPU cost as v2.57's single-tilemap approach.

    Safe HRAM: FF91=counter_lo, FFA5=counter_hi, FFA9=tilemap toggle (0x98/0x9C)
    """
    ...
    # === TOGGLE TILEMAP (alternate 0x9800 / 0x9C00 each frame) ===
    # Self-correcting: handles ANY initial FFA9 value (uninitialized HRAM)
    # AND 0x04 extracts just the tilemap-select bit, XOR toggles it,
    # OR 0x98 reconstructs the valid base (always produces 0x98 or 0x9C)
    code.extend([0xF0, 0xA9])        # LDH A, [FFA9]     ; current state
    code.extend([0xE6, 0x04])        # AND 0x04           ; extract tilemap bit
    code.extend([0xEE, 0x04])        # XOR 0x04           ; toggle
    code.extend([0xF6, 0x98])        # OR 0x98            ; reconstruct base
    code.extend([0xE0, 0xA9])        # LDH [FFA9], A      ; save (0x98 or 0x9C)
...
def main():
    ...
    print("=== Penta Dragon DX v2.57 ===")
    print("Alternating tilemap + ROM table + 253 tiles/frame + v2.36 hook")
    print()
