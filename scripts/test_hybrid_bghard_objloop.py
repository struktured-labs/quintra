#!/usr/bin/env python3
"""
Test: Hardcoded BG palettes + Loop-based OBJ palettes.
Tests if BG palettes must be written first.
"""
import sys
import yaml
...
def create_hybrid_loader(obj_data_addr: int) -> bytes:
    """
    Hardcoded BG palettes + Loop-based OBJ palettes.
    """
    code = bytearray()

    # HARDCODED BG palettes first (like FULLHARD does)
    code.extend([0x3E, 0x80])  # LD A, 0x80
    code.extend([0xE0, 0x68])  # LDH [BCPS], A

    for _ in range(8):  # 8 BG palettes - grayscale
        # White
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_HYBRID.gb")
    palette_path = Path("palettes/penta_palettes_v097.yaml")

    print("=== Hybrid: Hardcoded BG + Loop OBJ ===")
    ...
    # Put OBJ palette data in bank 0
    bank0_data_addr = 0x0200
    rom[bank0_data_addr:bank0_data_addr + len(obj_data)] = obj_data

    palette_loader_addr = 0x6800  # In bank 13
    colorizer_addr = 0x6A00      # After palette loader
    shadow_main_addr = 0x6A20
    combined_addr = 0x6A50

    palette_loader = create_hybrid_loader(bank0_data_addr)
    ...
    print(f"Hybrid palette loader: {len(palette_loader)} bytes")
