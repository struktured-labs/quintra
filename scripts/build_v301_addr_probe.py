#!/usr/bin/env python3
"""Address-probe: vary the start address and count of bank-2 writes.

usage: build_v301_addr_probe.py <start_lo> <count>
  start_lo: 0x00..0xFF — start address in bank-2 page (0xD000 + lo)
  count:    number of writes (each writes 1 byte)

Layout:
    DI; FF70=2; LD HL, 0xD000+lo; <count> writes via LD [HL+], A; FF70=1; EI
Each `LD [HL+], A` is 8T. count=48 ≈ 384T (compact).
"""
import sys
...
def create_addr_probe(start_lo: int, count: int) -> bytes:
    """DI; FF70=2; LD A, 0x42; LD HL, 0xD000+lo; count × LD [HL+],A; FF70=1; EI."""
    code = bytearray()
    code.extend([0xF3])                                  # DI
    code.extend([0x3E, 0x02, 0xE0, 0x70])                # FF70 = 2
    code.extend([0x3E, 0x42])                            # LD A, 0x42 (sentinel value)
    code.extend([0x21, start_lo & 0xFF, 0xD0])           # LD HL, 0xD000 + start_lo
    for _ in range(count):
        code.extend([0x22])                              # LD [HL+], A   (8T)
...
def build(start_lo: int, count: int):
    ...
    w(A['sw'], bytes(sw))
    w(A['ac'], create_addr_probe(start_lo, count))
    ...
    rom[0x14D] = chk
    out = Path(f"rom/working/penta_dragon_dx_v301_addr_{start_lo:02x}_n{count}.gb")
    out.write_bytes(rom)
    print(f"Wrote {out}  (writes 0xD0{start_lo:02x}..0xD0{(start_lo+count-1)&0xFF:02x})")


if __name__ == "__main__":
    start_lo = int(sys.argv[1], 0)
    count = int(sys.argv[2], 0)
    build(start_lo, count)
