#!/usr/bin/env python3
"""Diagnostic v3.01 — same as production but attr_comp + GDMA SKIPPED.

Tests whether the STAGE-LOAD-stuck regression is caused by attr_computation
or by GDMA. If gameplay works with both disabled but reverts to slow
bg_sweep coverage, the regression is somewhere in the attr/GDMA path.

Output: rom/working/penta_dragon_dx_v301_no_attr.gb
"""
import sys
...
from build_v301_gdma import (BG_TABLE_BYTES, WRAM_BG_TABLE,
    create_inline_tile_copy_tileonly, create_gdma_transfer, create_attr_computation)
...
def build():
    ...
    code.extend([0xCD, A['cp'] & 0xFF, (A['cp'] >> 8) & 0xFF])  # cond_pal
    code.extend([0xCD, A['sw'] & 0xFF, (A['sw'] >> 8) & 0xFF])  # bg_sweep

    # NO GDMA, NO attr_computation. FFC1 gate just does OAM DMA + shadow.
    ...
    code.extend([0xC9])
    w(A['cz'], bytes(code))
    print(f"  colorize handler (no attr/GDMA): {len(code)} bytes")
    ...
    rom[0x14D] = chk
    out = Path("rom/working/penta_dragon_dx_v301_no_attr.gb")
    out.write_bytes(rom)
    print(f"Wrote {out}")
