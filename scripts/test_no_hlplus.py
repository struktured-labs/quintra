#!/usr/bin/env python3
"""
Test: Use LD A, [HL] + INC HL instead of LD A, [HL+].
Maybe the [HL+] auto-increment has a bug in this context.
"""
import sys
import yaml
...
def create_no_hlplus_loader(rom_addr: int) -> bytes:
    """Use LD A, [HL] + INC HL instead of LD A, [HL+]."""
    code = bytearray()

    # Point HL to ROM data
    code.extend([0x21, rom_addr & 0xFF, (rom_addr >> 8) & 0xFF])

    # Set OCPS to 0x80
    code.extend([0x3E, 0x80])
    code.extend([0xE0, 0x6A])

    # Loop 64 bytes using LD A, [HL] + INC HL
    code.extend([0x0E, 0x40])  # LD C, 64
    loop_start = len(code)
    code.append(0x7E)          # LD A, [HL] (NOT [HL+])
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_NOHLPLUS.gb")
    palette_path = Path("palettes/penta_palettes_v097.yaml")

    print("=== No [HL+] test: use LD A,[HL] + INC HL ===")
    ...
    palette_loader = create_no_hlplus_loader(bank0_data_addr)
