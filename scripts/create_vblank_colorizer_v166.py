#!/usr/bin/env python3
"""
v1.66: TEST - VBK save/restore

BG colorizer saves original VBK, switches to 1, then restores.
Tests if preserving original VBK state avoids corruption.
"""
import sys
import yaml
...
def create_bg_row_colorizer_lazy(lookup_table_addr: int) -> bytes:
    """TEST: LCD check + warmup + VBK save/restore."""
    code = bytearray()
    ...
    # Counter >= 128: do VBK switching with save/restore
    # Save original VBK
    code.extend([0xF0, 0x4F])           # LDH A, [0xFF4F]
    code.append(0x47)                   # LD B, A (save original VBK)
    ...
    # Re-read counter for increment
    code.extend([0xF0, 0xC0])           # LDH A, [0xFFC0]

    # warmup: increment counter (no wrap - let it saturate at 255)
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v166.gb")
    ...
    print("\n=== v1.66: TEST - VBK Save/Restore ===")
    print("  BG colorizer: saves original VBK, switches to 1, restores")
    print("  Testing if preserving original VBK avoids corruption")
    print()
    ...
    combined_addr = 0x6A98  # Moved slightly to fit bg_colorizer
