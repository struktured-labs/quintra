#!/usr/bin/env python3
"""
v1.94: Minimal BG work - only 2 tiles per frame

- Only 2 tiles/frame to minimize VBlank time
- BG colorizer called first
- Tests if minimal work fixes Sara corruption
"""
import sys
import yaml
...
def create_bg_row_colorizer_lazy(lookup_table_addr: int) -> bytes:
    """v1.94: Minimal - only 2 tiles per frame.

    Tests if absolute minimal work fixes Sara corruption.
    """
    HRAM_COUNTER = 0xC4
    WARMUP_FRAMES = 60
    WRAP_VALUE = 188
    TILES_PER_FRAME = 2  # Reduced to absolute minimum
    code = bytearray()
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v194.gb")
    ...
    print("\n=== v1.94: Minimal - 2 tiles/frame ===")
    print("  Only 2 tiles per frame (absolute minimum)")
    print("  Tests if minimal work fixes Sara corruption")
    print()
