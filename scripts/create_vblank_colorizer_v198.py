#!/usr/bin/env python3
"""
v1.98: Faster buffer updating for scroll support

FINDINGS FROM v1.97:
- Sara complete (GDMA + buffer builder work!)
- BG colors work in initial area
- Scrolling reveals rainbow stripes (buffer not keeping up with scroll)

THIS VERSION:
- Increases tiles per frame from 8 to 32 (one full row)
- Completes full buffer in 32 frames instead of 128
- Should handle scrolling much better
- Still uses GDMA for safe instant copy
"""
import sys
import yaml
...
def create_fast_buffer_builder(lookup_table_addr: int) -> bytes:
    """
    v1.98: Faster buffer builder - 32 tiles per frame (one full row).

    Completes full 1024-tile buffer in 32 frames instead of 128.
    Better handles scrolling since buffer updates faster.

    Uses HRAM 0xFFC4 as row counter (0-31).
    """
    ATTR_BUFFER = 0xD000
    TILES_PER_FRAME = 32  # One full row
    TOTAL_ROWS = 32
    HRAM_ROW = 0xC4
    WARMUP = 30  # Reduced warmup since we're faster
    ...
    # Read row counter
    code.extend([0xF0, HRAM_ROW])         # LDH A, [0xFFC4]
    code.append(0x47)                      # LD B, A (save row)

    # If row < WARMUP, just increment and return
    code.extend([0xFE, WARMUP])            # CP warmup
    skip_to_end = len(code)
    code.extend([0x38, 0x00])              # JR C, skip_work (placeholder)

    # Calculate actual row (row - WARMUP), range 0-31
    code.extend([0xD6, WARMUP])            # SUB warmup
    code.append(0x4F)                      # LD C, A (working row 0-31)

    # Calculate VRAM address: 0x9800 + row * 32
    # row * 32 = row << 5
    code.append(0x87)                      # ADD A (x2)
    code.append(0x87)                      # ADD A (x4)
    code.append(0x87)                      # ADD A (x8)
    code.append(0x87)                      # ADD A (x16)
    code.append(0x87)                      # ADD A (x32) - low byte
    code.append(0x6F)                      # LD L, A
    # High byte: 0x98 + (row >> 3)
    code.append(0x79)                      # LD A, C (row)
    code.extend([0xCB, 0x3F])              # SRL A
    code.extend([0xCB, 0x3F])              # SRL A
    code.extend([0xCB, 0x3F])              # SRL A -> 0-3
    code.extend([0xC6, 0x98])              # ADD 0x98
    code.append(0x67)                      # LD H, A
    # HL = VRAM address

    # Calculate buffer address: 0xD000 + row * 32
    code.append(0x79)                      # LD A, C (row)
    code.append(0x87)                      # x2
    code.append(0x87)                      # x4
    code.append(0x87)                      # x8
    ...
    # VBK = 0 for reading tiles
    code.extend([0xAF])                    # XOR A
    code.extend([0xE0, 0x4F])              # LDH [VBK], A

    # Loop: read 32 tiles, lookup palettes, write to buffer
    code.extend([0x0E, TILES_PER_FRAME])   # LD C, 32
    ...
    # End: increment row counter, wrap at warmup+32
    end_label = len(code)
    code[skip_to_end + 1] = (end_label - skip_to_end - 2) & 0xFF

    code.append(0x78)                      # LD A, B (original row)
    code.append(0x3C)                      # INC A
    wrap_val = WARMUP + TOTAL_ROWS         # 30+32=62
    code.extend([0xFE, wrap_val])          # CP 62
    code.extend([0x38, 0x02])              # JR C, +2 (no wrap)
    code.extend([0x3E, WARMUP])            # LD A, WARMUP (wrap to 30)
    code.extend([0xE0, HRAM_ROW])          # LDH [0xFFC4], A
...
def create_combined_func(palette_loader_addr: int, shadow_main_addr: int,
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v198.gb")
    ...
    print("\n=== v1.98: Fast buffer builder (32 tiles/frame) ===")
    print("  One full row per frame instead of 8 tiles")
    print("  Full buffer in 32 frames instead of 128")
    print("  Should handle scrolling much better")
    print()
    ...
    buffer_builder = create_fast_buffer_builder(lookup_table_addr)
