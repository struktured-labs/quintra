#!/usr/bin/env python3
"""
v2.17: Fix Sara Color Flashing

FIXES from v2.16:
- Sara color flashing: Removed 0xFFBE dependency entirely
- Sara palette now determined by tile ID directly:
  - Tiles 0x20-0x27: Sara Witch -> Palette 2 (hardcoded)
  - Tiles 0x28-0x2F: Sara Dragon -> Palette 1 (hardcoded)
- No more race condition from asynchronous 0xFFBE updates

OBJ tile ranges (purely tile-based, no external flags):
- 0x00-0x1F: Effects/projectiles -> Palette 0 (white/gray)
- 0x20-0x27: Sara Witch -> Palette 2 (skin/pink) [HARDCODED]
- 0x28-0x2F: Sara Dragon -> Palette 1 (green) [HARDCODED]
...
def create_tile_based_colorizer_v217() -> bytes:
    ...
    v2.17: Sara palettes are HARDCODED based on tile ID:
    - Tiles 0x20-0x27: Sara Witch -> palette 2 (no D register!)
    - Tiles 0x28-0x2F: Sara Dragon -> palette 1 (no D register!)

    E register still used for boss override.

    Tile ranges:
    - 0x00-0x1F: Effects -> palette 0
    - 0x20-0x27: Sara Witch -> palette 2 (HARDCODED)
    - 0x28-0x2F: Sara Dragon -> palette 1 (HARDCODED)
    ...
    # Sara Witch: HARDCODED palette 2 (no D register!)
    labels['sara_witch_palette'] = len(code)
    code.extend([0x3E, 0x02])              # LD A, 2
    jumps_to_fix.append((len(code), 'apply_palette'))
    code.extend([0x18, 0x00])              # JR apply_palette

    # Sara Dragon: HARDCODED palette 1 (no D register!)
...
def create_shadow_colorizer_main_v217(colorizer_addr: int) -> bytes:
    """
    Main routine that sets up E register for boss override and calls colorizer.

    v2.17: D register no longer used for Sara palette!
    Sara palette is determined by tile ID in the colorizer itself.
    ...
    # v2.17: NO 0xFFBE reading - Sara palette is determined by tile ID
    # D register is not used anymore for Sara
...
def create_bg_colorizer_3rows(lookup_table_addr: int, row_counter_addr: int = 0xD0) -> bytes:
    ...
    ~96 tiles x 35 cycles = 3,360 cycles (still fits in VBlank)
    """
    code = bytearray()
...
def main():
    ...
    output_path = project_root / 'rom' / 'working' / 'penta_dragon_dx_v217.gb'
    ...
    print("\n=== v2.17: Fix Sara Color Flashing ===")
    print("  FIX: Sara palette now determined by tile ID (no 0xFFBE race condition)")
    print("  - Tiles 0x20-0x27: Sara Witch -> Palette 2 (hardcoded)")
    print("  - Tiles 0x28-0x2F: Sara Dragon -> Palette 1 (hardcoded)")
    print()
    ...
    # Addresses (same layout as v2.16)
    palette_data_addr = 0x6800
    gargoyle_addr = 0x6880
    spider_addr = 0x6888
    obj_colorizer_addr = 0x6900  # Tile-based colorizer
    obj_main_addr = 0x6980       # Shadow colorizer main (v2.17: no D register)
    ...
    print(f"OBJ colorizer (tile-based v2.17): {len(obj_colorizer)} bytes at 0x{obj_colorizer_addr:04X}")
    print(f"OBJ colorizer main (no 0xFFBE): {len(obj_main)} bytes at 0x{obj_main_addr:04X}")
    ...
    # Patch entry points (same as v2.16)
    ...
    print("\n=== v2.17 Build Complete ===")
    print("FIX: Sara color flashing eliminated")
    print("  - Removed 0xFFBE reading (race condition source)")
    print("  - Sara palette now 100% tile-based:")
    print("    - Tiles 0x20-0x27: Sara Witch -> Palette 2")
    print("    - Tiles 0x28-0x2F: Sara Dragon -> Palette 1")
