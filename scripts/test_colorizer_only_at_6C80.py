#!/usr/bin/env python3
"""
Test: Keep combined at 0x6C50 and colorizer_main at 0x6C20 (working addresses),
but move ONLY the colorizer routine to 0x6C80.
This isolates whether the colorizer address specifically matters.
"""
import sys
import yaml
...
def create_test_loader(data_addr: int) -> bytes:
    code = bytearray()
    code.extend([0x3E, 0x80]); code.extend([0xE0, 0x68])
    for _ in range(64):
        code.extend([0x3E, 0x00]); code.extend([0xE0, 0x69])
    code.extend([0x3E, 0x80]); code.extend([0xE0, 0x6A])
    for _ in range(8):
    ...
    code.extend([0x3E, 0xA0]); code.extend([0xE0, 0x6A])
    code.extend([0x0E, 0x08])
    loop_start = len(code)
    code.append(0x7E); code.extend([0xE0, 0x6B]); code.append(0x23); code.append(0x0D)
...
def create_force_palette4_colorizer() -> bytes:
    ...
    loop_start = len(code)
    code.append(0x7E); code.extend([0xE6, 0xF8]); code.extend([0xF6, 0x04]); code.append(0x77)
    code.extend([0x23, 0x23, 0x23, 0x23]); code.append(0x05)
    code.extend([0x20, (loop_start - len(code) - 2) & 0xFF])
    code.append(0xC9)
    return bytes(code)
...
def create_shadow_colorizer_main(colorizer_addr: int) -> bytes:
    ...
    code.extend([0x21, 0x03, 0xC0]); code.extend([0xCD, colorizer_addr & 0xFF, colorizer_addr >> 8])
    code.extend([0x21, 0x03, 0xC1]); code.extend([0xCD, colorizer_addr & 0xFF, colorizer_addr >> 8])
    code.extend([0xE1, 0xD1, 0xC1, 0xF1]); code.append(0xC9)
    return bytes(code)
...
def create_combined(palette_loader: int, colorizer_main: int) -> bytes:
    ...
    code.extend([0xCD, 0x80, 0xFF]); code.append(0xC9)
    return bytes(code)

def create_vblank_hook(combined_addr: int) -> bytes:
    input_code = bytearray([0x3E, 0x20, 0xE0, 0x00, 0xF0, 0x00, 0x2F, 0xE6, 0x0F, 0xCB, 0x37, 0x47,
        0x3E, 0x10, 0xE0, 0x00, 0xF0, 0x00, 0xF0, 0x00, 0x2F, 0xE6, 0x0F, 0xB0,
        0xE0, 0x93, 0x3E, 0x30, 0xE0, 0x00])
    hook_code = bytearray([0x3E, 0x0D, 0xEA, 0x00, 0x20, 0xCD, combined_addr & 0xFF, combined_addr >> 8,
        0x3E, 0x01, 0xEA, 0x00, 0x20, 0xC9])
    return bytes(input_code + hook_code)

def main():
    input_rom = Path("rom/Penta Dragon (J).gb")
    output_rom = Path("rom/working/penta_dragon_dx_COLORIZER_6C80.gb")
    ...
    # Keep combined and colorizer_main at WORKING addresses
    palette_loader_addr = 0x6800
    colorizer_addr = 0x6C80       # ONLY THIS CHANGES
    colorizer_main_addr = 0x6C20  # Same as ZEROBG_GRAYOBJ
    combined_addr = 0x6C50        # Same as ZEROBG_GRAYOBJ

    palette_loader = create_test_loader(data_addr)
    colorizer = create_force_palette4_colorizer()
    colorizer_main = create_shadow_colorizer_main(colorizer_addr)  # This will call 0x6C80
    combined = create_combined(palette_loader_addr, colorizer_main_addr)
    vblank_hook = create_vblank_hook(combined_addr)

    print(f"Combined at: 0x{combined_addr:04X} (working address)")
    print(f"Colorizer_main at: 0x{colorizer_main_addr:04X} (working address)")
    print(f"Colorizer at: 0x{colorizer_addr:04X} (TEST ADDRESS)")
