#!/usr/bin/env python3
"""
v1.38: HDMA-Based BG Colorization with Corrected Tile Ranges

Based on actual BG tile analysis:
- 0x01-0x06: Floor pattern tiles (palette 0)
- 0x10-0x3F: Wall/structure tiles (palette 2 - purple/blue)
- 0xC0-0xDF: Item tiles (palette 1 - gold)
- 0xE0-0xFF: Wall edges/borders (palette 2)
- Everything else: Default (palette 0)

HDMA transfers 256 bytes per frame (8 rows), full screen in 4 frames.
"""
import sys
import yaml
...
def create_tile_palette_lookup() -> bytes:
    ...
    Based on actual BG tile analysis from level 1:
    - 0x00-0x0F: Floor tiles (palette 0 - main dungeon)
    - 0x10-0x3F: Wall/structure tiles (palette 2 - purple/blue walls)
    - 0x40-0x7F: More structure (palette 2)
    - 0x80-0xBF: Hazard/special tiles (palette 3 - green)
    - 0xC0-0xDF: Item tiles (palette 1 - gold)
    ...
    for i in range(256):
        if i < 0x10:
            lookup[i] = 0  # Floor - palette 0
        elif i < 0x40:
            lookup[i] = 2  # Walls - palette 2 (purple)
        elif i < 0x80:
            lookup[i] = 2  # More walls - palette 2
        elif i < 0xC0:
            lookup[i] = 3  # Hazards - palette 3 (green)
        elif i < 0xE0:
            lookup[i] = 1  # Items - palette 1 (gold)
        else:
            lookup[i] = 2  # Wall edges - palette 2
    return bytes(lookup)
...
def create_hdma_bg_colorizer(lookup_table_addr: int, attr_buffer_addr: int,
    ...
    """
    HDMA-based BG colorizer - processes 256 tiles per frame.
    """
    code = bytearray()
    ...
    code.extend([0x28, 0x03])  # JR Z, skip
    code.append(0xF5)  # PUSH AF
    code.extend([0x18, (outer_loop - len(code) - 2) & 0xFF])

    # Recalculate HDMA destination
    ...
    # Start HDMA: 256 bytes = 16 blocks, HBlank mode
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v138.gb")
    ...
    print("\n=== v1.38: HDMA with Corrected Tile Ranges ===")
    print("  Tile ranges based on actual BG tile analysis:")
    print("    0x00-0x0F: Floor (palette 0)")
    print("    0x10-0x7F: Walls (palette 2)")
    print("    0x80-0xBF: Hazards (palette 3)")
    print("    0xC0-0xDF: Items (palette 1 - gold)")
    print("    0xE0-0xFF: Wall edges (palette 2)")
    print()
    ...
    if hdma_end > shadow_main_addr:
        print(f"ERROR: HDMA colorizer overlaps shadow_main!")
        return
