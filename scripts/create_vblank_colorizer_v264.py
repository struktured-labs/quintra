#!/usr/bin/env python3
"""
v2.64: LY-gated BG writes - ZERO VBK flicker on MiSTer FPGA

Root cause of ALL flicker on MiSTer: VBK bank switching (FF4F) during
rendering causes the PPU to briefly read tile/sprite data from the wrong
VRAM bank, producing visual glitches on every scanline where a VBK switch
occurs. Previous versions did 100-300+ VBK switches per frame during
rendering. Sara W flickered because her sprite tiles were read from bank 1
during a VBK switch meant for BG attributes.

Fix: LY-gated writes. Check LY register before every VBK switch:
- LY >= 144 (VBlank): safe to switch VBK, read tile, write attribute
- LY < 144 (rendering): skip the write entirely, advance counter only
Combined with 0xFF anti-flicker filter for bad reads during rendering.

Result: ZERO VBK switches during rendering. All writes happen in VBlank.
~25-30 tiles written per frame (whatever fits in VBlank). Counter advances
157 positions/frame for tilemap coverage distribution. Full tilemap coverage
in ~40 frames (0.67s) but no flicker at any point during settling.

Key features:
- LY-gated VBK writes (ZERO bank switches during rendering)
- 0xFF anti-flicker filter (bad mode 3 reads → skip)
- 157 tiles/frame sweep (25-30 written, rest read-only)
...
def create_bg_colorizer(bg_table_addr: int) -> bytes:
    """BG colorizer: LY-gated writes for ZERO VBK flicker on MiSTer.

    Every VBK bank switch (FF4F) during rendering causes the PPU to briefly
    read tile/sprite data from the wrong VRAM bank. This is the root cause
    of ALL flicker on MiSTer FPGA - including Sara W sprite flickering.

    Fix: Check LY before every VBK switch. Only write during VBlank (LY>=144).
    During rendering, read tiles + lookup palettes but skip the VBK switch.
    Counter advances regardless for tilemap coverage distribution.

    ~25-30 tiles written per frame (VBlank only). Zero rendering artifacts.
    Safe HRAM: FF91=counter_lo, FFA5=counter_hi
    """
    TILES_PER_VBLANK = 157  # prime, coprime with 1024
    # Counter advances 157/frame for coverage distribution
    # Only ~25-30 tiles actually WRITTEN (those during VBlank)
    # Full tilemap coverage: ~40 frames (0.67s)
    # CPU: ~31% (reads during rendering are cheap, no VBK switch)
    ...
    # LY gate: only write during VBlank (LY >= 144 = 0x90)
    emit([0x4F])              # LD C, A             ; 4T  save palette
    emit([0xF0, 0x44])        # LDH A, [FF44]      ; 12T read LY
    emit([0xFE, 0x90])        # CP 144              ; 8T
    emit_jr(0x38, 'skip_write')  # JR C, skip_write ; 12T taken (rendering) / 8T not (VBlank)

    # VBlank confirmed: safe to switch VBK and write
...
def main():
    ...
    print("=== Penta Dragon DX v2.64 ===")
    print("LY-gated BG writes - ZERO VBK flicker + 0xFF filter (MiSTer-optimized)")
    print()
