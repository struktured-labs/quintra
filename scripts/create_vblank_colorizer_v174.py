#!/usr/bin/env python3
"""
v1.74: BG Colorizer DISABLED for testing

- BG colorizer call is commented out to isolate the freeze bug
- Everything else same as v1.73
"""
import sys
import yaml
...
def create_combined_with_dma_and_bg(palette_loader_addr: int, shadow_main_addr: int, bg_colorizer_addr: int) -> bytes:
    """Combined: palettes, sprite colorize, (BG DISABLED), DMA."""
    code = bytearray()
    code.extend([0xCD, palette_loader_addr & 0xFF, palette_loader_addr >> 8])
    code.extend([0xCD, shadow_main_addr & 0xFF, shadow_main_addr >> 8])
    # BG colorizer DISABLED for testing - skip the call
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v174.gb")
    ...
    print("\n=== v1.74: BG Colorizer DISABLED ===")
    print("  Testing if freeze is caused by BG colorizer")
    print()
