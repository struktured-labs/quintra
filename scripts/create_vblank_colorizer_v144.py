#!/usr/bin/env python3
"""
v1.44: Full BG Tile Colorization

Based on v1.43 (stable) but processes ALL 576 tiles across 18 frames:
- Frame 0: tiles 0-31
- Frame 1: tiles 32-63
- Frame 2: tiles 64-95
- ...
- Frame 17: tiles 544-575

Uses frame counter at 0xFFC2 to track which chunk to process.
Full tilemap colored every 18 frames (~300ms at 60fps).
32 tiles per frame reduces VBlank pressure to prevent flickering.
"""
import sys
import yaml
...
def create_bg_colorizer_chunked(lookup_table_addr: int, frame_counter_addr: int) -> bytes:
    """
    BG colorizer that processes 64 tiles per frame, cycling through 9 chunks.

    Uses frame counter at 0xFFC2:
    - Reads counter (0-8)
    - Calculates offset = counter * 64
    - Processes tiles at that offset
    - Increments counter (wraps at 9)

    Full 576 tiles updated every 9 frames.

    NOTE: Game Boy CPU (LR35902) does NOT have EX DE,HL instruction.
    We use PUSH/POP to swap registers instead.
    """
    code = bytearray()
    ...
    # Result fits in 16 bits (max 17*32=544)
    ...
    # Move HL to DE (no EX DE,HL on GB!)
    ...
    # Increment frame counter (wrap at 18 for 32-tile chunks)
    code.extend([0xF0, frame_counter_addr & 0xFF])  # LDH A, [counter]
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v144.gb")
    ...
    print("\n=== v1.44: Full BG Colorization (18-frame cycle) ===")
    print("  Processes 32 tiles per frame (reduced VBlank pressure)")
    print("  Full 576 tiles colored every 18 frames")
    print("  Uses frame counter at 0xFFC2")
    print()
    ...
    bg_colorizer = create_bg_colorizer_chunked(lookup_table_addr, frame_counter_addr)
