#!/usr/bin/env python3
"""
v2.56: Dual-Tilemap BG Colorizer

Root cause fix for tilemap-flip accuracy drops during scrolling.
Diagnostic confirmed: game flips LCDC bit 3 every ~28 frames.
After each flip, the newly-active tilemap has stale palettes → 89-94% accuracy.

Fix: Process BOTH 0x9800 and 0x9C00 at each sweep position.
Uses XOR D, 0x04 to toggle between tilemaps (they're 0x400 apart).
Both tilemaps stay current → no stale palettes after flip.

Architecture:
- v2.54's ROM lookup table + 0xFF anti-flicker filter + safe HRAM
- v2.36's VBlank hook (1 d-pad read + explicit bank restore, no ship mode)
- 127 tiles/frame per tilemap (prime, coprime with 1024)
- L register as palette temp (frees C, since no base_hi needed)
- DE always in 0x98xx range, XOR 0x04 for 0x9Cxx access
"""
import sys
import yaml
...
def create_bg_colorizer(bg_table_addr: int) -> bytes:
    """BG colorizer: DUAL tilemap + ROM table lookup + 127 positions/frame.

    Processes BOTH 0x9800 and 0x9C00 at each position to eliminate
    tilemap-flip accuracy drops. DE always in 0x98xx range; XOR D, 0x04
    toggles to 0x9Cxx for the second tilemap.

    Uses L as palette temp (not C), since C is unused.
    Safe HRAM: FF91=counter_lo, FFA5=counter_hi (FFA9 no longer needed).
    """
    # Prime, coprime with 1024. Each tilemap gets 181 tiles/frame.
    # Full sweep per tilemap: 1024/181 ≈ 5.7 frames.
    # Flips happen every ~20-28 frames → ~3.5-5 sweeps between flips → both current.
    TILES_PER_VBLANK = 181
    ...
    code.extend([0x1A])              # LD A, [DE]        ; read tile from 0x98xx
    code.extend([0x6F])              # LD L, A           ; lookup index
    code.extend([0x7E])              # LD A, [HL]        ; palette from ROM table
    code.extend([0xFE, 0xFF])        # CP 0xFF           ; garbage filter
    skip_9800_pos = len(code)
    code.extend([0x28, 0x00])        # JR Z, skip_9800
    code.extend([0x6F])              # LD L, A           ; save palette in L
    code.extend([0x3E, 0x01])        # LD A, 1
    code.extend([0xE0, 0x4F])        # LDH [FF4F], A     ; VBK=1
    code.extend([0x7D])              # LD A, L           ; restore palette
    ...
    # --- Process 0x9C00 tilemap ---
    code.extend([0x1A])              # LD A, [DE]        ; read tile from 0x9Cxx
    code.extend([0x6F])              # LD L, A           ; lookup index
    code.extend([0x7E])              # LD A, [HL]        ; palette from ROM table
    code.extend([0xFE, 0xFF])        # CP 0xFF           ; garbage filter
    skip_9C00_pos = len(code)
    code.extend([0x28, 0x00])        # JR Z, skip_9C00
    code.extend([0x6F])              # LD L, A           ; save palette in L
    code.extend([0x3E, 0x01])        # LD A, 1
    code.extend([0xE0, 0x4F])        # LDH [FF4F], A     ; VBK=1
    code.extend([0x7D])              # LD A, L           ; restore palette
    ...
    code.extend([0x7A])              # LD A, D
    code.extend([0xEE, 0x04])        # XOR 0x04          ; 0x9C->0x98
    code.extend([0x57])              # LD D, A

    # --- Advance position ---
    code.extend([0x13])              # INC DE

    # --- Wrap check (within 0x9800-0x9BFF) ---
    code.extend([0x7A])              # LD A, D
    code.extend([0xFE, 0x9C])        # CP 0x9C           ; past 0x9BFF?
    no_wrap_pos = len(code)
    code.extend([0x20, 0x00])        # JR NZ, no_wrap
    code.extend([0x16, 0x98])        # LD D, 0x98        ; reset
...
def main():
    ...
    print("=== Penta Dragon DX v2.56 ===")
    print("Dual-tilemap BG (both 9800+9C00) + 181 tiles/frame (coprime) + v2.36 hook")
    print()
