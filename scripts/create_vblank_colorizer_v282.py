#!/usr/bin/env python3
"""
v2.82: Hook-First Hybrid — Per-row interleaved + aggressive dual-tilemap sweep

Builds on v2.81's per-row interleaving. Remaining ~1% artifact cause:
The hook only writes palettes to the ACTIVE tilemap (H from caller). The game
alternates tilemaps each frame (DC0B toggle). The OTHER tilemap has stale palette
attributes until the VBlank sweep catches up. With Phase 2 sweep (1 row/frame),
full coverage takes 24 frames (~0.4s) → visible stale palettes during scrolling.

Fix: After hook completes, set FFA5=0 instead of FFA5=24. This restarts the
sweep in Phase 1 mode (2 rows/frame = 12 frames for full coverage). The sweep
writes to BOTH tilemaps (D^=0x04 trick), so the other tilemap gets re-colored
much faster. During active scrolling, the hook triggers frequently, keeping the
sweep perpetually in fast mode.

HRAM assignments:
  FF91: Hook flag (0x5A = copy active, checked but NOT cleared by VBlank)
  FFA5: Sweep state counter (0-23=Phase1, 24-47=Phase2, reset to 0 by hook)
...
def create_enhanced_tilemap_copy(bg_table_addr: int, base_addr: int,
    ...
    emit([0xAF])               # XOR A (A = 0)
    emit([0xE0, 0xA5])        # LDH [FFA5], A       ; restart Phase 1 fast sweep
...
def build_v282():
    """Build v2.82 ROM — per-row interleaved + aggressive dual-tilemap sweep."""
...
if __name__ == "__main__":
    rom_path = build_v282()
    print(f"\nBuilt v2.82: {rom_path}")
    print(f"Hook-first hybrid with per-row interleaved copy + aggressive sweep")
    print(f"Each row writes palette+tiles atomically (DI-protected transition)")
    print(f"Hook resets FFA5=0 → fast Phase 1 sweep re-colors OTHER tilemap (2 rows/frame)")
