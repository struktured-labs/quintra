#!/usr/bin/env python3
"""
v1.91: Full address calculation test

- Counter + warmup + FULL ADDRESS CALCULATION + write
- This is essentially v1.85 - tests if address calc causes corruption
"""
import sys
import yaml
...
def create_bg_row_colorizer_lazy(lookup_table_addr: int) -> bytes:
    """Full v1.85-style colorizer with address calculation.

    Tests if the address calculation causes Sara corruption.
    """
    HRAM_COUNTER = 0xC4
    WARMUP_FRAMES = 60
    WRAP_VALUE = 188
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v191.gb")
    ...
    print("\n=== v1.91: Full address calculation test ===")
    print("  Counter + FULL ADDRESS CALCULATION + write")
    print("  Tests if address calc causes Sara corruption")
    print()
