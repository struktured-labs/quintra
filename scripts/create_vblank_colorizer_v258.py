#!/usr/bin/env python3
"""
v2.58: Dual-tilemap BG colorizer with SET/RES bit-toggle optimization

Reads tile from BOTH 0x9800 and 0x9C00 at each position, writes palettes
to both, using only ONE bank switch pair (VBK=1 → write both → VBK=0).

Key optimizations over v2.56:
- SET 2, D / RES 2, D for 0x9800↔0x9C00 toggle (8cy vs 16cy XOR approach)
- Single VBK switch pair per position (2 switches vs 4 in v2.56)
- 0xFF filter only on first read (both tilemaps share PPU mode timing)
- Hardcoded wrap at 0x9C (no FFA9 reads in loop)
- 191 tiles/frame (prime, coprime with 1024)
- ~50% CPU (vs v2.56's ~64%) = game at full speed with both tilemaps current
"""
import sys
import yaml
...
def create_bg_colorizer(bg_table_addr: int) -> bytes:
    """BG colorizer: DUAL tilemap at each position with SET/RES optimization.

    Reads tiles from BOTH 0x9800 and 0x9C00 at each sweep position.
    Uses SET 2, D / RES 2, D for fast tilemap switching (8cy vs 16cy XOR).
    Only ONE VBK switch pair per position (vs 4 in v2.56).
    DE always stays in 0x9800 range; SET/RES temporarily accesses 0x9C00.

    Safe HRAM: FF91=counter_lo, FFA5=counter_hi (FFA9 freed)
    """
    # Prime, coprime with 1024 (GCD=1). Both tilemaps at each position.
    # Full sweep per tilemap: 1024/191 ≈ 5.4 frames.
    # ~164 cycles/position × 191 = ~31K cycles ≈ 50% CPU.
    TILES_PER_VBLANK = 191
    ...
    code.extend([0xE6, 0x03])        # AND 0x03           ; 0-3
    code.extend([0xC6, 0x98])        # ADD A, 0x98        ; D = 0x98-0x9B
    ...
    # --- Read tile from 0x9800 tilemap (VRAM bank 0) ---
    code.extend([0x1A])              # LD A, [DE]         ; read 9800 tile
    code.extend([0x6F])              # LD L, A            ; lookup index
    code.extend([0x7E])              # LD A, [HL]         ; palette A from ROM table
    code.extend([0xFE, 0xFF])        # CP 0xFF            ; garbage? (mode 3)
    skip_both_pos = len(code)
    code.extend([0x28, 0x00])        # JR Z, skip_both    ; skip both if mode 3
    code.extend([0x4F])              # LD C, A            ; C = palette for 9800

    # --- Toggle to 0x9C00 and read ---
    code.extend([0xCB, 0xD2])        # SET 2, D           ; D: 0x98→0x9C (8cy, 2 bytes)
    code.extend([0x1A])              # LD A, [DE]         ; read 9C00 tile
    code.extend([0x6F])              # LD L, A            ; lookup index
    code.extend([0x7E])              # LD A, [HL]         ; palette B from ROM table

    # --- Check 9C00 for garbage ---
    code.extend([0xFE, 0xFF])        # CP 0xFF            ; 9C00 garbage?
    write_a_only_pos = len(code)
    code.extend([0x28, 0x00])        # JR Z, write_9800_only

    # --- Write BOTH palettes (single VBK switch pair) ---
    code.extend([0x6F])              # LD L, A            ; L = palette B (save)
    code.extend([0x3E, 0x01])        # LD A, 1
    code.extend([0xE0, 0x4F])        # LDH [FF4F], A      ; VRAM bank 1
    code.extend([0x7D])              # LD A, L            ; palette B
    code.extend([0x12])              # LD [DE], A          ; write 9C00 attr
    code.extend([0xCB, 0x92])        # RES 2, D           ; D: 0x9C→0x98 (back)
    code.extend([0x79])              # LD A, C            ; palette A
    code.extend([0x12])              # LD [DE], A          ; write 9800 attr
    code.extend([0xAF])              # XOR A
    code.extend([0xE0, 0x4F])        # LDH [FF4F], A      ; VRAM bank 0
    jr_to_advance_pos = len(code)
    code.extend([0x18, 0x00])        # JR advance

    # --- Write 9800 only (9C00 was garbage) ---
    write_a_only_target = len(code)
    code[write_a_only_pos + 1] = write_a_only_target - (write_a_only_pos + 2)
    code.extend([0xCB, 0x92])        # RES 2, D           ; back to 9800
    code.extend([0x3E, 0x01])        # LD A, 1
    code.extend([0xE0, 0x4F])        # LDH [FF4F], A      ; VRAM bank 1
    code.extend([0x79])              # LD A, C            ; palette A
    code.extend([0x12])              # LD [DE], A          ; write 9800 attr
    ...
    # --- Skip target (mode 3 garbage on 9800 read) ---
    skip_both_target = len(code)
    code[skip_both_pos + 1] = skip_both_target - (skip_both_pos + 2)
    # Fix the JR advance from the "both valid" path
    code[jr_to_advance_pos + 1] = skip_both_target - (jr_to_advance_pos + 2)

    # --- Advance position ---
    code.extend([0x13])              # INC DE

    # --- Wrap check (hardcoded 0x9C boundary) ---
    code.extend([0x7A])              # LD A, D
    code.extend([0xFE, 0x9C])        # CP 0x9C
    no_wrap_pos = len(code)
    code.extend([0x20, 0x00])        # JR NZ, no_wrap
    code.extend([0x16, 0x98])        # LD D, 0x98          ; reset to base
...
def main():
    ...
    print("=== Penta Dragon DX v2.58 ===")
    print("Dual-tilemap SET/RES + ROM table + 191 tiles/frame + v2.36 hook")
    print()
