#!/usr/bin/env python3
"""
Diagnostic: Add delays between palette writes to test timing hypothesis.
"""
import sys
import yaml
...
def create_slow_palette_loader(rom_data_addr: int) -> bytes:
    """
    Palette loader with NOP delays between writes.
    """
    code = bytearray()

    # Load BG palettes with delays
    code.extend([0x21, rom_data_addr & 0xFF, (rom_data_addr >> 8) & 0xFF])  # LD HL, rom_addr
    ...
    code.extend([0x00, 0x00, 0x00, 0x00])  # 4 NOPs for delay
    code.append(0x0D)                # DEC C
    bg_offset = bg_loop - len(code) - 2
    code.extend([0x20, bg_offset & 0xFF])

    # Load OBJ palettes with delays
    obj_data_addr = rom_data_addr + 64
    code.extend([0x21, obj_data_addr & 0xFF, (obj_data_addr >> 8) & 0xFF])  # LD HL, obj_addr
    code.extend([0x3E, 0x80])        # LD A, 0x80
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_SLOW.gb")
    palette_path = Path("palettes/penta_palettes_v097.yaml")

    print("=== DIAGNOSTIC: Slow palette loading with NOPs ===")
    print()
    ...
    palette_loader = create_slow_palette_loader(palette_data_addr)
