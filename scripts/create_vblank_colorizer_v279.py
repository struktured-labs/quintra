#!/usr/bin/env python3
"""
v2.79: Hook-First Hybrid — FFEE corruption fix (anti-flicker)

Root cause of flickering: VBlank fires during enhanced copy's Pass 1, sweep
overwrites FFEE (tilemap base_hi). Pass 2 reads corrupted FFEE → writes palettes
to the WRONG tilemap → visible flicker every time VBlank fires mid-copy.

Fix: Set hook flag (FF91=0x5A) at the START of enhanced copy (not just end).
VBlank checks flag but does NOT clear it. Enhanced copy clears flag at end.
This suppresses sweep during the entire enhanced copy, protecting FFEE.

Timeline:
  Copy starts → FF91=0x5A (sweep suppressed)
  VBlank fires during copy → sees 0x5A → skips sweep → FFEE safe
  Copy ends → FF91=0x00 → next VBlank runs sweep normally
...
def create_enhanced_tilemap_copy(bg_table_addr: int, base_addr: int,
    ...
    # SET HOOK FLAG EARLY: Suppress sweep during entire copy to protect FFEE
    ...
    # ================================================================
    # CLEAR HOOK FLAG: Allow sweep on next VBlank after copy completes
    # ================================================================
    emit([0xAF])               # XOR A  (A = 0)
...
if __name__ == "__main__":
    rom_path = build_v279()
    print(f"\nBuilt v2.79: {rom_path}")
    print(f"Hook-first hybrid with FFEE corruption fix (anti-flicker)")
    print(f"Fix: Hook flag set at START of copy, VBlank skips sweep, flag cleared at END")
