...
for replay in '11 500 2064128703' '12 520 2064128323' '19 440 2064128755'; do
...
awk -F, '
  ...
  {
    rows++
    if ($(col["seed"]) != (NR == 2 ? 2064128703 : NR == 3 ? 2064128323 : 2064128755))
      wrong_seed = 1
    bosses += $(col["bosses"])
    if ($(col["bosses"]) >= 1) first_bosses++
    if ($(col["death_source"]) != 255 && $(col["death_bosses"]) < 1)
      early_deaths++
  }
  END {
    ...
    if (first_bosses != 3) { print "[sauran-boss] classwise policy did not clear all three opening bosses" > "/dev/stderr"; exit 1 }
    if (bosses < 3) { print "[sauran-boss] classwise policy cleared fewer than three giants" > "/dev/stderr"; exit 1 }
