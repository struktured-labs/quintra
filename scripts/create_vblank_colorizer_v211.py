#!/usr/bin/env python3
"""
v2.11: Debug BG Colorization - Fixed Value Test

Testing if the issue is with lookup table reading or VBK writing.
This version writes a FIXED palette (3) to all tiles to verify
the write loop is working correctly.
"""
import sys
import yaml
...
def create_bg_colorizer_fixed() -> bytes:
    """
    DEBUG VERSION: Write FIXED palette 3 to all tiles.
    This tests if the loop itself works correctly.

    Uses v2.09's proven working loop structure (with E register).
    """
    code = bytearray()
    ...
    # Fill 0x9800-0x99FF (512 bytes = 16 rows) with palette 3
    code.extend([0x21, 0x00, 0x98])        # LD HL, 0x9800
    code.extend([0x01, 0x00, 0x02])        # LD BC, 0x0200 (512)
    code.extend([0x1E, 0x03])              # LD E, 3 (palette 3 = green)
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v211.gb")
    ...
    print("\n=== v2.11: Fixed Palette Test ===")
    print("  DEBUG: Writes palette 3 (green) to ALL BG tiles")
    print("  Uses v2.09's proven loop structure")
    print("  Expected: Entire BG should be GREEN")
    print()
    ...
    bg_colorizer = create_bg_colorizer_fixed()  # Fixed value version
