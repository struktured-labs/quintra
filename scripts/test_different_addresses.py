#!/usr/bin/env python3
"""
Test: EXACT same as ZEROBG_GRAYOBJ but with different code addresses.
Uses 0x6E00/0x6E20/0x6E50 like LOOP8 instead of 0x6C00/0x6C20/0x6C50.
"""
import sys
import yaml
...
def create_test_loader(data_addr: int) -> bytes:
    """Same as ZEROBG_GRAYOBJ - Zero BG + Grayscale OBJ."""
    code = bytearray()
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_DIFFADDR.gb")
    palette_path = Path("palettes/penta_palettes_v097.yaml")

    print("=== Test: Same as ZEROBG_GRAYOBJ but at 0x6E00 addresses ===")
    ...
    # Use DIFFERENT addresses like LOOP8
    palette_loader_addr = 0x6800  # Same
    colorizer_addr = 0x6E00       # DIFFERENT
    colorizer_main_addr = 0x6E20  # DIFFERENT
    combined_addr = 0x6E50        # DIFFERENT
    ...
    print(f"Colorizer at: 0x{colorizer_addr:04X}")
    print(f"Combined at: 0x{combined_addr:04X}")
    ...
    print()
    print("If YELLOW: addresses don't matter")
    print("If broken: addresses are the issue")
