#!/usr/bin/env python3
"""Diagnostic: GDMA every frame, NO attr_comp. Tests if GDMA itself breaks game."""
import sys
...
def build():
    ...
    # FFC1 gate: ALWAYS call GDMA when FFC1=1. No attr_comp.
    code.extend([0xF0, 0xC1, 0xB7])
    fg = len(code) + 1
    code.extend([0x28, 0x00])
    code.extend([0xCD, 0x80, 0xFF])
    code.extend([0xCD, A['shm'] & 0xFF, (A['shm'] >> 8) & 0xFF])
    code.extend([0xCD, A['gd'] & 0xFF, (A['gd'] >> 8) & 0xFF])  # GDMA always when FFC1=1
    code[fg] = (len(code) - fg - 1) & 0xFF

    code.extend([0xF1, 0xE0, 0x4F, 0xF1, 0xE0, 0x99, 0xC9])
    w(A['cz'], bytes(code))
    print(f"  colorize handler (GDMA-only test): {len(code)} bytes")
    ...
    rom[0x14D] = chk
    out = Path("rom/working/penta_dragon_dx_v301_gdma_only.gb")
    out.write_bytes(rom)
    print(f"Wrote {out}")
