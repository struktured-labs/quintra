#!/usr/bin/env python3
"""
v1.13: Smart BG Item Colorization

Fixes from v1.12:
1. Skip on title/menu screens (check if 0xC000 == 0, meaning no sprites)
2. Heavy frame throttling (every 32 frames = ~0.5 sec)
3. Run AFTER DMA to win race condition

Item tiles (BG): 0x88-0xDF -> BG palette 1 (gold/yellow)

OBJ Tile-based coloring (same as v1.10+):
- Sara W: palette 2, Sara D: palette 1
- Hornets: palette 4, Orcs: palette 5, Humanoids: palette 6
- Boss mode via 0xFFBF with dynamic palette loading
"""
import sys
import yaml
...
def create_tile_based_colorizer() -> bytes:
    """Tile-based OBJ colorizer with boss/miniboss override."""
    code = bytearray()
    labels = {}
    jumps_to_fix = []
...
def create_smart_bg_colorizer() -> bytes:
    """
    Smart BG item colorizer with gameplay detection and throttling.

    Checks:
    1. If [0xC000] == 0, skip (title/menu screen - no sprites)
    2. Frame counter: only run every 32 frames

    Scans the ENTIRE 32x32 tilemap for item tiles.
    This avoids scroll alignment issues - just color all items in the map.
    """
    code = bytearray()
    labels = {}
    jumps_to_fix = []
    ...
    # === GAMEPLAY CHECK ===
    # If shadow OAM buffer is empty (C000 == 0), we're on title/menu
    code.extend([0xFA, 0x00, 0xC0])  # LD A, [C000]
    code.append(0xB7)                # OR A
    jumps_to_fix.append((len(code), 'skip'))
    code.extend([0x28, 0x00])        # JR Z, skip (no sprites = title screen)

    # === FRAME THROTTLE ===
    # Only run every 64 frames (~1 sec at 60fps) since we scan more tiles
    code.extend([0xF0, 0xB2])        # LDH A, [FFB2]
    code.append(0x3C)                # INC A
    code.extend([0xE6, 0x3F])        # AND 0x3F (mask to 0-63)
    code.extend([0xE0, 0xB2])        # LDH [FFB2], A
    jumps_to_fix.append((len(code), 'skip'))
    code.extend([0x20, 0x00])        # JR NZ, skip (only run when counter == 0)

    # === BG TILE SCAN ===
    # Scan ENTIRE 32x32 tilemap (1024 tiles) to avoid scroll issues
    # This only runs once per second so it's acceptable
    code.extend([0x21, 0x00, 0x98])  # LD HL, 0x9800

    # Use DE as tile counter (1024 = 0x400)
    code.extend([0x11, 0x00, 0x04])  # LD DE, 1024
    ...
    # Decrement DE counter
    code.append(0x1B)                # DEC DE
    code.append(0x7A)                # LD A, D
    code.append(0xB3)                # OR E
    tile_offset = labels['tile_loop'] - len(code) - 2
...
def create_combined_with_dma(palette_loader_addr: int, shadow_main_addr: int, bg_colorizer_addr: int) -> bytes:
    ...
    # v1.13: Smart BG colorizer runs AFTER DMA with gameplay check + throttle
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v113.gb")
    ...
    print("\n=== v1.13: Smart BG Item Colorization ===")
    print("  Gameplay detection: Skip if [0xC000]==0 (title/menu)")
    print("  Frame throttle: Every 32 frames (~0.5 sec)")
    ...
    palette_loader_addr = 0x6A70
    combined_addr = 0x6AF0

    colorizer = create_tile_based_colorizer()
    bg_colorizer = create_smart_bg_colorizer()
    ...
    print(f"Smart BG Colorizer: {len(bg_colorizer)} bytes at 0x{bg_colorizer_addr:04X}")
