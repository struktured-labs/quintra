#!/usr/bin/env python3
"""
v1.68: Simplified BG Colorizer

Simplified BG colorization:
- NO warmup (was causing issues on scene transitions)
- VBK saved in register B (not HRAM - avoids conflicts)
- Simple row counter wrapping at 32
"""
import sys
import yaml
...
def create_bg_row_colorizer_lazy(lookup_table_addr: int) -> bytes:
    """Simplified BG colorizer - no warmup, VBK saved in register."""
    WRAM_BUFFER = 0xDFF0
    code = bytearray()
    ...
    # Save original VBK in B (before we use it for anything else)
    code.extend([0xF0, 0x4F])           # LDH A, [0xFF4F]
    code.append(0x47)                   # LD B, A (save VBK)

    # Read row counter from 0xFFC0, calculate row = counter & 0x1F
    code.extend([0xF0, 0xC0])           # LDH A, [0xFFC0]
    code.append(0x4F)                   # LD C, A (save full counter)
    code.extend([0xE6, 0x1F])           # AND 0x1F (row 0-31)
    code.append(0x57)                   # LD D, A (save row number)
    ...
    # Phase 1: Read 32 tiles (VBK=0), lookup palettes, store in WRAM
    code.extend([0x11, WRAM_BUFFER & 0xFF, (WRAM_BUFFER >> 8) & 0xFF])
    code.extend([0x3E, 0x20])           # LD A, 32
    code.append(0x57)                   # LD D, A (loop counter)
    ...
    code.extend([0x11, WRAM_BUFFER & 0xFF, (WRAM_BUFFER >> 8) & 0xFF])
    code.extend([0x3E, 0x20])
    code.append(0x57)                   # LD D, 32
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v168.gb")
    ...
    print("\n=== v1.68: Simplified BG Colorizer ===")
    print("  NO warmup, VBK in register B, simple counter wrap at 32")
    print()
