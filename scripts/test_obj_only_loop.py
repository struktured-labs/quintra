#!/usr/bin/env python3
"""
Test: OBJ palettes ONLY using loop (no BG), to test timing hypothesis.
If this works, the issue is total loop time exceeding VBlank.
"""
import sys
import yaml
...
def create_obj_only_loop_loader(obj_data_addr: int) -> bytes:
    """OBJ palettes ONLY, no BG, using loop."""
    code = bytearray()
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_OBJONLY.gb")
    palette_path = Path("palettes/penta_palettes_v097.yaml")

    print("=== OBJ-only loop loader (no BG palettes) ===")
    print("Tests if removing BG palette loop fixes timing")
    print()
    ...
    obj_data_addr = 0x6800  # OBJ palette data
    palette_loader_addr = 0x6850  # Loader code
    colorizer_addr = 0x6880
    shadow_main_addr = 0x68A0
    combined_addr = 0x68D0

    palette_loader = create_obj_only_loop_loader(obj_data_addr)
    ...
    print(f"OBJ data at 0x{obj_data_addr:04X}")
