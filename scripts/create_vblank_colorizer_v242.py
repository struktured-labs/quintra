#!/usr/bin/env python3
"""
v2.42: STAT-Safe BG Colorizer (Stability Fix)

Fixes v2.41 crash: 96 tiles/frame was too aggressive, causing game
instability in GUI mode. Reduced to 48 tiles/frame with STAT waits.

ROOT CAUSE (from v2.41): Game services VBlank interrupts LATE (LY=2-6,
already in LCD rendering). VRAM writes during mode 3 are silently dropped.
FIX: STAT mode check (wait for mode 0 HBlank) before each VRAM access.

48 tiles/frame × ~162M avg = ~7776M per frame (~17 scanlines).
Full sweep: 1024/48 ≈ 21 frames (~0.35 seconds at 60fps).
Game retains ~89% of CPU time for main loop.

Also adds FFEB safety mask on load to prevent first-frame corruption
if HRAM contains stale data from a save state.

INHERITED from v2.41/v2.36:
...
def create_bg_colorizer_dual_rw_vblank_safe(lookup_table_addr: int) -> bytes:
    ...
    STAT mode check before each tile's VRAM operations.
    ...
    Per-tile cost: ~162M average (124M STAT wait + 38M operations)
    48 tiles × ~162M = ~7776M per frame (~17 scanlines).
    Full sweep: 1024/48 ≈ 21 frames (~0.35 seconds).
    Game retains ~89% of CPU time.
    """
...
def main():
    ...
    print("=== Penta Dragon DX v2.42 ===")
    print("STAT-Safe BG Colorizer (Stability Fix)")
    print()
    print("Fixes v2.41 crash: reduced from 96 to 48 tiles/frame")
    print("  STAT mode check before each VRAM access (HBlank-safe)")
    print("  48 tiles/frame = ~7776M (~17 scanlines, game keeps ~89% CPU)")
    print("  Full sweep in 21 frames (~0.35s)")
    print("  FFEB safety mask prevents first-frame corruption")
    print()
