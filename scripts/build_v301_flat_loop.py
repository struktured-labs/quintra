#!/usr/bin/env python3
"""Flat loop of N tiles inside one DI window — same body as attr_comp tile_loop.

Tests whether the freeze comes from inner-loop iteration count, or from the
outer row_loop machinery (PUSH AF nesting, DE skip-8, etc.).

usage: build_v301_flat_loop.py <count>
"""
import sys
...
def create_flat_loop(bg_table_addr: int, count: int) -> bytes:
    ...
    code.extend([0x3E, count & 0xFF])             # LD A, count
    tl = len(code)
...
def build(count: int):
    ...
    w(A['sw'], bytes(sw))
    w(A['ac'], create_flat_loop(A['bt'], count))
    ...
    rom[0x14D] = chk
    out = Path(f"rom/working/penta_dragon_dx_v301_flat_{count}.gb")
    out.write_bytes(rom)
    print(f"Wrote {out}  (flat loop of {count} iters, DI ≈ {count*80}T)")
