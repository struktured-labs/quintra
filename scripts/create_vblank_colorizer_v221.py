#!/usr/bin/env python3
"""
v2.21: Fix Crow Palette Bug + OBJ + BG Colorization

FIX: Crow tiles (0x30-0x3F) now correctly get palette 3 (dark blue)
     instead of palette 4 (yellow/hornet).

Based on v2.20 with working OBJ + BG colorization.
...
def create_palette_loader(palette_data_addr: int, gargoyle_addr: int, spider_addr: int) -> bytes:
    """Palette loader - UNCHANGED from v2.19."""
    code = bytearray()
...
def main():
    ...
    output_rom = project_root / "rom/working/penta_dragon_dx_v221.gb"
    ...
    print("\n=== v2.21: Fix Crow Palette + OBJ + BG Colorization ===")
    print("  FIX: Crow tiles (0x30-0x3F) now get palette 3 (dark blue)")
