#!/usr/bin/env python3
"""
v2.39: Dual-Tilemap BG Colorizer (No-Reset Sweep)

FIXES over v2.38:
1. DUAL TILEMAP WRITES: Each tile write goes to BOTH 0x9800 and 0x9C00 tilemaps.
   v2.38 only wrote to the active tilemap and reset on flip, leaving ~12% of
   positions permanently uncolored (128 of 1024). v2.39 has zero dead zones.
2. NO FLIP RESET: Counter sweeps continuously without resetting on tilemap swap.
   Both tilemaps get fully colored in ~10 frames and stay colored.
3. NO SCROLL-EDGE DETECTION: Game uses room-based scrolling (SCX 0-15 pixels),
   not continuous scrolling. Edge column colorization is unnecessary overhead.
4. ~110 TILES PER VBLANK: 38 M-cycles per tile (read + lookup + dual write).
   Full 1024-tile sweep in ~10 frames. Both tilemaps always fully colored.

Key insight: v2.38's active-tilemap-only approach had a fundamental flaw:
with 128 tiles/frame and 7-frame flip interval, only 896/1024 positions got
colored before reset. The last 128 positions were NEVER reached. Dual writes
cost more per tile but guarantee 100% coverage.

Per-tile cost breakdown (56 M-cycles):
  Read tile A:  LD A,[DE] + lookup via HL = 8M
  Read tile B:  XOR D→0x9C + LD A,[HL] + lookup = 15M
  Bank 1:       LD A,1 + LDH [FF4F] = 5M
  Write A:      LD H,D + LD L,E + LD [HL],C = 4M
  Write B:      XOR H + LDH A,[FFEE] + LD [HL],A = 9M
  Advance+wrap: INC DE + CP + JR = 7M
  Bank 0:       XOR A + LDH [FF4F] = 4M
  Loop:         DEC B + JR NZ = 4M

HRAM usage:
  FFEA: sweep position counter (low byte)
  FFEB: sweep position counter (high, 0-3)
  (FFEC, FFED no longer used)

INHERITED from v2.38/v2.36:
...
def create_bg_colorizer_dual_write(lookup_table_addr: int) -> bytes:
    """BG colorizer that reads+writes BOTH tilemaps independently.

    v2.39 key insight: v2.38's active-tilemap-only approach left ~12% of
    positions permanently uncolored due to counter reset on tilemap flip.
    This version reads tiles from EACH tilemap separately, looks up the
    correct palette for each, and writes the correct attributes to both.

    Flow:
    1. Check game mode (FFC1), skip on menus
    2. Load sweep counter from FFEA/FFEB
    3. Sweep loop: for each tile position:
       a. Read tile from 0x9800+offset (bank 0), lookup palette → C
       b. Read tile from 0x9C00+offset (bank 0), lookup palette → HRAM
       c. Write palette C to 0x9800+offset (bank 1)
       d. Write HRAM palette to 0x9C00+offset (bank 1)
    4. Save counter back to FFEA/FFEB

    Per-tile cost: 56 M-cycles
      Read tile A:  LD A,[DE] + lookup = 8M
      Read tile B:  compute addr + LD A,[HL] + lookup = 15M
      Bank 1 switch: 5M
      Write A:      LD H,D + LD L,E + LD [HL],C = 4M
      Write B:      XOR H + LDH A + LD [HL],A = 9M
      Bank 0 switch: 4M
      Advance+wrap: 7M
      Loop:         4M

    Budget: ~4200M available → ~75 tiles per VBlank
    Full sweep: 1024/75 ≈ 14 frames (both tilemaps correctly colored)

    HRAM layout:
      FFEA: counter low (E offset, 0x00-0xFF)
      FFEB: counter high (0-3, added to 0x98 for tilemap base)
      FFEE: temporary palette storage for tilemap B
    """
    ...
    # === LOAD SWEEP POSITION ===
    # Counter stored as offset 0-1023 relative to 0x9800
    ...
    # Set tile count (75 tiles per VBlank)
    code.extend([0x06, 75])          # LD B, 75
    ...
    # --- Read tile from tilemap A (0x9800 range, DE points here) ---
    ...
    code.extend([0x7E])              # LD A, [HL]        ; 2M → palette A
    code.extend([0x4F])              # LD C, A           ; 1M → save palette A in C

    # --- Read tile from tilemap B (0x9C00 range, use HL) ---
    code.extend([0x7A])              # LD A, D           ; 1M → D = 0x98-0x9B
    code.extend([0xEE, 0x04])        # XOR 0x04          ; 2M → A = 0x9C-0x9F
    code.extend([0x67])              # LD H, A           ; 1M → H = high byte for 0x9C00
    code.extend([0x6B])              # LD L, E           ; 1M → L = E (same low byte)
    code.extend([0x7E])              # LD A, [HL]        ; 2M → tile ID from 0x9C00
    code.extend([0x6F])              # LD L, A           ; 1M
    code.extend([0x26, lookup_high]) # LD H, lookup_high ; 2M → H = 0x6B
    code.extend([0x7E])              # LD A, [HL]        ; 2M → palette B
    code.extend([0xE0, 0xEE])        # LDH [FFEE], A     ; 3M → save palette B in HRAM
    ...
    # Write palette B to tilemap B (0x9C00 range, XOR H with 0x04)
    code.extend([0x7C])              # LD A, H           ; 1M
    code.extend([0xEE, 0x04])        # XOR 0x04          ; 2M
    code.extend([0x67])              # LD H, A           ; 1M
    code.extend([0xF0, 0xEE])        # LDH A, [FFEE]     ; 3M → reload palette B
    code.extend([0x77])              # LD [HL], A         ; 2M → write palette B
    ...
    code.extend([0x38, 0x02])        # JR C, +2          ; 2M (no wrap)
    ...
    # === CLEANUP ===
    # Bank 0 already set from loop exit
...
def main():
    ...
    print("=== Penta Dragon DX v2.39 ===")
    print("Dual-Tilemap BG Colorizer (No-Reset Sweep)")
    print()
    print("NEW in v2.39:")
    print("  1. Dual write: writes attributes to BOTH 0x9800 and 0x9C00 tilemaps")
    print("  2. ~110 tiles/VBlank: 38 M-cycles per tile (read + lookup + dual write)")
    print("  3. No reset: sweep continues across tilemap flips, zero dead zones")
    print("  4. Full sweep in ~10 frames, both tilemaps always fully colored")
    print("  5. No scroll-edge: game uses room-based scrolling (unnecessary)")
    print()
    ...
    bg_colorizer = create_bg_colorizer_dual_write(lookup_table_addr)
