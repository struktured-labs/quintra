#!/usr/bin/env python3
"""
v1.07: Boss/Miniboss detection + tile-based coloring

When 0xFFBF != 0 (boss/miniboss present):
  → ALL enemies get palette 7

When 0xFFBF == 0 (normal gameplay):
  → Tile-based coloring:
...
def create_shadow_colorizer_main(colorizer_addr: int) -> bytes:
    ...
    # E = 7 if boss present, E = 0 if normal
    code.extend([0xF0, 0xBF])        # LDH A, [0xFFBF]
    code.append(0xB7)                # OR A
    code.extend([0x28, 0x04])        # JR Z, +4 (no boss)
    code.extend([0x1E, 0x07])        # LD E, 7 (boss mode)
    code.extend([0x18, 0x02])        # JR +2
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v107.gb")
    ...
    print("\n=== v1.07: Boss/Miniboss Detection + Tile-based Coloring ===")
    print("  Boss detection: 0xFFBF != 0 → ALL enemies palette 7")
    print("  Normal mode: tile-based coloring:")
