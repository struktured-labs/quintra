#!/usr/bin/env python3
"""v3.01 BANK 2 WRITE TEST — does a SINGLE byte write to bank 2 freeze?

Builds on the FF70 isolation result (FF70 op alone is fine).
Inside FFC1 gate, this version does:
    DI; FF70=2; [$D000] = 0x42; FF70=1; EI    (13 bytes)

If this freezes → writing to bank 2 (with FF70=2 active) is the problem.
If this passes → loop-machinery / volume is the problem. Next: scale up.

Output: rom/working/penta_dragon_dx_v301_bank2write_test.gb
"""
import sys
...
def build():
    ...
    output_path = Path("rom/working/penta_dragon_dx_v301_bank2write_test.gb")
    ...
    # ─── BANK 2 WRITE TEST ───
    # Switch to bank 2, write 1 byte to 0xD000, switch back.
    code.extend([0xF3])                       # DI
    code.extend([0x3E, 0x02, 0xE0, 0x70])     # LD A,2; LDH [FF70],A
    code.extend([0x3E, 0x42])                 # LD A, 0x42 (test value)
    code.extend([0xEA, 0x00, 0xD0])           # LD [0xD000], A  ← THE bank-2 write
    ...
    print(f"  (TEST: DI; FF70=2; [$D000]=0x42; FF70=1; EI inside FFC1 gate)")
