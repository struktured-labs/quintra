#!/usr/bin/env python3
"""
v1.01: FIXED buffer selection + modify BOTH buffers for complete coverage.

Bug fix from v1.00: Was modifying the WRONG shadow buffer!
- After DMA, FFCB contains the buffer that WAS just transferred
- Next frame will transfer the OTHER buffer
- v1.00 was modifying FFCB directly, should have XOR'd to get next buffer

v1.01 Strategy:
- Modify hardware OAM (0xFE00) for immediate display
- Modify BOTH shadow buffers (0xC000 AND 0xC100) to ensure
  no matter which one gets DMA'd next, it has correct palettes
"""
import sys
import yaml
...
def create_oam_colorize_subroutine(tile_table_addr: int) -> bytes:
    """
    Subroutine to colorize 40 sprites starting at address in HL.
    Expects: HL = OAM buffer base (0xFE00, 0xC000, or 0xC100)
             D = Sara palette
             E = Boss palette (0 if no boss)
    """
    code = bytearray()
    ...
    code.extend([0x18, 0x12])        # JR +18 (to apply)

    # Enemy: check boss mode
    code.append(0x7B)                # LD A, E
    code.append(0xB7)                # OR A
    code.extend([0x20, 0x0E])        # JR NZ, +14 (use boss palette in E)
    ...
    code.append(0x23)                # INC HL (next sprite)
...
def create_v101_oam_loop(tile_table_addr: int, colorize_sub_addr: int) -> bytes:
    """
    v1.01: Process ALL THREE OAM locations for complete flicker elimination.

    1. Hardware OAM (0xFE00) - immediate display
    2. Shadow buffer 1 (0xC000) - one of the DMA sources
    3. Shadow buffer 2 (0xC100) - the other DMA source

    By colorizing all three, we ensure the correct colors regardless of
    which buffer the game is updating or which gets DMA'd next.
    """
    code = bytearray()
    ...
    code.extend([0x1E, 0x07])        # LD E, 7 (Spider boss palette)
    code.extend([0x18, 0x06])        # JR +6
    code.extend([0x1E, 0x06])        # LD E, 6 (Gargoyle boss palette)
    code.extend([0x18, 0x02])        # JR +2
    code.extend([0x1E, 0x00])        # LD E, 0 (no boss)

    # === Process hardware OAM (0xFE00) ===
    code.extend([0x21, 0x00, 0xFE])  # LD HL, 0xFE00
    code.extend([0xCD, colorize_sub_addr & 0xFF, colorize_sub_addr >> 8])

    # === Process shadow buffer 1 (0xC000) ===
    code.extend([0x21, 0x00, 0xC0])  # LD HL, 0xC000
    code.extend([0xCD, colorize_sub_addr & 0xFF, colorize_sub_addr >> 8])

    # === Process shadow buffer 2 (0xC100) ===
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v101.gb")
    ...
    print("\n=== v1.01: Complete Flicker Elimination ===")
    print("  Strategy: Modify ALL THREE OAM locations every frame")
    print("    - Hardware OAM (0xFE00) for immediate display")
    print("    - Shadow buffer 1 (0xC000)")
    print("    - Shadow buffer 2 (0xC100)")
    print("  This ensures correct colors regardless of buffer timing.")
    print()
    ...
    SPIDER_PAL = 0x6888
    TILE_TABLE = 0x6890
    COLORIZE_SUB = 0x6990    # Subroutine to colorize one buffer
    OAM_LOOP = 0x69E0        # Main loop that calls subroutine 3x
    PALETTE_LOADER = 0x6A40
    ...
    # Write colorize subroutine
    colorize_sub = create_oam_colorize_subroutine(TILE_TABLE)
    offset = BANK13_BASE + (COLORIZE_SUB - 0x4000)
    rom[offset:offset+len(colorize_sub)] = colorize_sub
    print(f"Colorize subroutine: {len(colorize_sub)} bytes at 0x{COLORIZE_SUB:04X}")

    # Write main OAM loop
    oam_loop = create_v101_oam_loop(TILE_TABLE, COLORIZE_SUB)
