#!/usr/bin/env python3
"""
v1.57 MINIMAL: Test if the trampoline alone causes the crash.

This is v1.09 (working sprites) + ONLY the trampoline hook.
NO BG colorization, NO VRAM bank 1 manipulation.

If this crashes -> the trampoline is the problem
If this works -> the BG colorization is the problem
"""
import sys
import yaml
...
def create_trampoline_handler_minimal() -> bytes:
    """
    Handler at 0x0008 - MINIMAL version.
    Just switches to bank 4, jumps to our minimal wrapper.
    """
    code = bytearray()
...
def create_vram_init_wrapper_minimal() -> bytes:
    """
    Minimal wrapper at 0x7000 in bank 4.
    Just does the original VRAM clear, NO bank 1 manipulation.
    """
    code = bytearray()

    # Original code from 0x4003-0x400B (EXACTLY as it was)
...
# v1.09 sprite colorization code (unchanged)
...
def create_combined_with_dma(palette_loader_addr: int, shadow_main_addr: int) -> bytes:
    """NO BG colorization - just sprites"""
    code = bytearray()
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v157_minimal.gb")
    ...
    print("\n=== v1.57 MINIMAL: Trampoline test (no BG colorization) ===")
    print("  If this crashes -> trampoline is the problem")
    print("  If this works -> BG colorization is the problem")
    print()

    rom = bytearray(input_rom.read_bytes())
    apply_all_display_patches(rom)

    # Minimal trampoline at 0x0008
    trampoline = create_trampoline_handler_minimal()
    print(f"Trampoline: {len(trampoline)} bytes at 0x0008")
    rom[0x0008:0x0008 + len(trampoline)] = trampoline

    # Patch 0x4003 with JP 0x0008
    ...
    for i in range(0x4006, 0x400C):
        rom[i] = 0x00
    print("Patched 0x4003: C3 08 00 (JP 0x0008)")

    # Minimal wrapper in bank 4 (no VRAM bank 1 manipulation!)
    wrapper = create_vram_init_wrapper_minimal()
    bank4_offset = 4 * 0x4000
    wrapper_rom_offset = bank4_offset + (0x7000 - 0x4000)
    print(f"Minimal wrapper: {len(wrapper)} bytes at ROM 0x{wrapper_rom_offset:05X}")
    rom[wrapper_rom_offset:wrapper_rom_offset + len(wrapper)] = wrapper

    # v1.09 sprite code in bank 13 (unchanged)
