#!/usr/bin/env python3
"""
v1.96: GDMA-based BG attribute copy

HYPOTHESIS: The VBK switching in a loop during tilemap scanning is causing
Sara corruption. Even simple address incrementing causes issues.

SOLUTION: Use GDMA (hardware DMA) instead of software loops.
- Pre-initialize attribute buffer in WRAM (0xD000-0xD3FF)
- Use GDMA to instantly copy buffer to VRAM bank 1 every frame
- NO software loop, NO repeated VBK switching

If this works without Sara corruption, GDMA is safe and we can focus on
properly building the attribute buffer.
"""
import sys
import yaml
...
def create_gdma_bg_colorizer() -> bytes:
    ...
    Uses hardware GDMA to instantly copy a pre-built attribute buffer
    from WRAM (0xD000-0xD3FF) to VRAM bank 1 (0x9800-0x9BFF).

    GDMA registers:
    - 0xFF51 (HDMA1): Source high byte
    - 0xFF52 (HDMA2): Source low byte
    - 0xFF53 (HDMA3): Destination high byte (0x80-0x9F only)
    - 0xFF54 (HDMA4): Destination low byte
    - 0xFF55 (HDMA5): Length/mode. Bit 7=0 for GDMA. Bits 0-6 = blocks-1.

    For 1024 bytes (32x32 tilemap): 64 blocks of 16 bytes = write 0x3F
    """
    ATTR_BUFFER = 0xD000  # 1024 bytes in WRAM bank 1
    TILEMAP_VRAM = 0x9800
    BLOCKS = 64  # 64 blocks * 16 bytes = 1024 bytes
    ...
    # Set GDMA source: 0xD000
    code.extend([0x3E, (ATTR_BUFFER >> 8) & 0xFF])  # LD A, 0xD0
    code.extend([0xE0, 0x51])        # LDH [HDMA1], A
    code.extend([0x3E, ATTR_BUFFER & 0xFF])         # LD A, 0x00
    code.extend([0xE0, 0x52])        # LDH [HDMA2], A

    # Set GDMA dest: 0x9800 (VRAM tilemap)
    code.extend([0x3E, (TILEMAP_VRAM >> 8) & 0xFF])  # LD A, 0x98
    code.extend([0xE0, 0x53])        # LDH [HDMA3], A
    code.extend([0x3E, TILEMAP_VRAM & 0xFF])         # LD A, 0x00
    code.extend([0xE0, 0x54])        # LDH [HDMA4], A

    # Start GDMA: 64 blocks, GDMA mode (bit 7 = 0)
    code.extend([0x3E, BLOCKS - 1])  # LD A, 0x3F (64-1)
    code.extend([0xE0, 0x55])        # LDH [HDMA5], A  -- Transfer starts immediately!

    # After GDMA completes, switch back to VRAM bank 0
...
def create_attr_buffer_initializer() -> bytes:
    """
    Initialize attribute buffer at 0xD000-0xD3FF with test pattern.

    For testing, we'll use a simple pattern:
    - All tiles get palette 1 (should show as purple/wall color)

    This runs ONCE at startup to fill the buffer.
    """
    ATTR_BUFFER = 0xD000
    BUFFER_SIZE = 1024
    TEST_PALETTE = 0x01  # BG palette 1
    ...
    # HL = buffer start
    code.extend([0x21, ATTR_BUFFER & 0xFF, (ATTR_BUFFER >> 8) & 0xFF])

    # BC = count (1024)
    code.extend([0x01, BUFFER_SIZE & 0xFF, (BUFFER_SIZE >> 8) & 0xFF])

    # D = palette value
    code.extend([0x16, TEST_PALETTE])

    # Fill loop
    fill_loop = len(code)
    code.append(0x72)                # LD [HL], D
    code.append(0x23)                # INC HL
    code.append(0x0B)                # DEC BC
    code.append(0x78)                # LD A, B
    code.append(0xB1)                # OR C
    offset = fill_loop - len(code) - 2
    code.extend([0x20, offset & 0xFF])  # JR NZ, fill_loop
...
def create_combined_with_dma_and_gdma(palette_loader_addr: int, shadow_main_addr: int,
                                       gdma_colorizer_addr: int, init_buffer_addr: int) -> bytes:
    """
    Combined function: load palettes, colorize sprites, GDMA BG attrs, run DMA.

    Also initializes buffer on first call using HRAM flag.
    """
    HRAM_INIT_FLAG = 0xC7  # 0xFFC7 = buffer initialized flag

    code = bytearray()

    # Check if buffer needs initialization
    code.extend([0xF0, HRAM_INIT_FLAG])  # LDH A, [0xFFC7]
    code.append(0xB7)                    # OR A
    code.extend([0x20, 0x09])            # JR NZ, skip_init (if non-zero, skip)

    # Initialize buffer (one time only)
    code.extend([0xCD, init_buffer_addr & 0xFF, init_buffer_addr >> 8])
    code.extend([0x3E, 0x01])            # LD A, 1
    code.extend([0xE0, HRAM_INIT_FLAG])  # LDH [0xFFC7], A (mark initialized)
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v196.gb")
    ...
    print("\n=== v1.96: GDMA-based BG attribute copy ===")
    print("  Uses hardware GDMA instead of software loops")
    print("  Pre-builds attribute buffer in WRAM (0xD000)")
    print("  Copies 1024 bytes instantly via GDMA")
    print("  NO VBK switching in loops - should prevent Sara corruption")
    print()
    ...
    gdma_colorizer_addr = 0x6A40
    init_buffer_addr = 0x6A70
    combined_addr = 0x6AA0
    ...
    init_buffer = create_attr_buffer_initializer()
    combined = create_combined_with_dma_and_gdma(palette_loader_addr, shadow_main_addr,
                                                  gdma_colorizer_addr, init_buffer_addr)
    ...
    print(f"Buffer init: {len(init_buffer)} bytes at 0x{init_buffer_addr:04X}")
    ...
    rom[bank13_offset + (init_buffer_addr - 0x4000):bank13_offset + (init_buffer_addr - 0x4000) + len(init_buffer)] = init_buffer
    ...
    print("\n=== v1.96 Build Complete ===")
    print("\nTest: This uses a STATIC attribute buffer (all palette 1).")
    print("If Sara is OK, GDMA is safe and we can focus on building the buffer correctly.")
