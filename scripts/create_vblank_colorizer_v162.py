#!/usr/bin/env python3
"""
v1.62: MINIMAL TEST - BG colorizer stub

BG colorizer does LCD check + immediate return.
Tests if just CALLING the function breaks fresh boot.
"""
import sys
import yaml
...
def create_bg_row_colorizer_lazy(lookup_table_addr: int) -> bytes:
    """MINIMAL TEST: LCD check + immediate return. No VRAM access."""
    code = bytearray()
    ...
    # LCD is on - just return immediately for this test
    code.append(0xC9)                   # RET
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v162.gb")
    ...
    print("\n=== v1.62: MINIMAL TEST - BG Stub ===")
    print("  BG colorizer: LCD check + immediate return (no VRAM access)")
    print("  Testing if just the CALL breaks fresh boot")
    print()
