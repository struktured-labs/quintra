#!/usr/bin/env python3
"""DI-budget probe: pure NOP padding inside one DI window.

Goal: find the DI cliff precisely, decoupled from instruction type. Each
NOP is exactly 4T cycles, no memory access, no register changes. Vary
count via CLI:

  python3 scripts/build_v301_di_nops.py <nop_count>

Builds rom/working/penta_dragon_dx_v301_di_nop_<count>.gb. Run the
gameplay-palette probe to see if it freezes.
"""
import sys
...
def create_di_nop(nop_count: int) -> bytes:
    """DI + nop_count NOPs + EI. No bank switching, no writes."""
    code = bytearray()
    code.extend([0xF3])                  # DI
    code.extend([0x00] * nop_count)      # nop_count NOPs (4T each)
    code.extend([0xFB])                  # EI
    code.extend([0xC9])                  # RET
    return bytes(code)


def build(nop_count: int):
    ...
    # NOP padding goes in attr_comp slot; budget at 0x7100 is huge (4KB)
    nop_routine = create_di_nop(nop_count)
    assert A['ac'] + len(nop_routine) <= 0x8000, f"NOP routine too big for bank"
    w(A['ac'], nop_routine)
    ...
    code.extend([0xCD, A['ac'] & 0xFF, (A['ac'] >> 8) & 0xFF])     # NOP-padding test
    ...
    rom[0x14D] = chk
    out = Path(f"rom/working/penta_dragon_dx_v301_di_nop_{nop_count}.gb")
    out.write_bytes(rom)
    print(f"Wrote {out} (DI + {nop_count} NOPs ≈ {nop_count * 4}T + EI)")
