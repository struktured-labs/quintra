#!/usr/bin/env python3
"""Penta Dragon DX v2.97 — merge Path A (calibrated bg_table) + Path B (viewport sweep).

Combines:

  - **Path B**'s phantom-safe visible-viewport BG sweep (from
    build_v296_phantomsafe.py). Every visible row gets its attrs refreshed
    each frame cycle — no 24-frame stale lag on dungeon walls. Uses
    correct 16-bit row math (fixes v2.94 row >=8 alias bug).

  - **Path A**'s empirically-calibrated bg_tile_table (256 bytes
    clustered from 16 real level-1 save states). Spikes → pal5 (red),
    walls → pal6 (slate), items → pal1, floor → pal0, etc.

Both paths use the v2.95 architectural skeleton (no trampoline → no
phantom sound) so v2.97 inherits that safety.

Verification target:
  - All 5 harnesses pass (title, phantom, BG color, mini-boss, scroll-tear)
  - Visual: spikes render red, walls render dark-slate, NO row-edge
    aliasing artifacts on lower half of screen, NO multi-second lag on
    BG attrs after room entry.
"""
import sys
...
from build_v296_calibrated import BG_TABLE_BYTES


def build_v297():
    input_rom = Path("rom/Penta Dragon (J).gb")
    output_path = Path("rom/working/penta_dragon_dx_v297.gb")
    ...
    # *** v297 merge: calibrated bg_table + viewport sweep ***
    ...
    # Colorize handler (same as v295 / v296)
    code = bytearray()
    ...
    print(f"  sweep: {len(sweep)} bytes (viewport-gated)")
    print(f"  bg_table: {len(BG_TABLE_BYTES)} bytes (calibrated)")
    return output_path
