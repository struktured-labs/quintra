#!/usr/bin/env python3
"""
v1.71: BG Colorizer with gameplay detection

- Only processes 8 tiles per frame to stay within VBlank
- Checks if Sara is present in OAM before colorizing BG
- If no Sara → we're on a menu → skip BG colorization
- This prevents title screen corruption
"""
import sys
import yaml
...
def create_bg_row_colorizer_lazy(lookup_table_addr: int) -> bytes:
    ...
    # === GAMEPLAY DETECTION: Check if Sara is present ===
    # If no Sara in OAM, we're on a menu - skip BG colorization
    # Check OAM slots 0-3 for Sara tiles (0x20-0x2F)
    code.extend([0x21, 0x02, 0xFE])     # LD HL, 0xFE02 (tile byte of sprite 0)
    code.extend([0x06, 0x04])           # LD B, 4 (check 4 sprites)
    sara_check_loop = len(code)
    code.append(0x7E)                   # LD A, [HL] (get tile)
    code.extend([0xE6, 0xF0])           # AND 0xF0 (check high nibble)
    code.extend([0xFE, 0x20])           # CP 0x20 (Sara tiles are 0x20-0x2F)
    sara_found_jump = len(code)
    code.extend([0x28, 0x00])           # JR Z, sara_found (placeholder)
    code.extend([0x23, 0x23, 0x23, 0x23])  # INC HL x4 (next sprite)
    code.append(0x05)                   # DEC B
    offset = sara_check_loop - len(code) - 2
    code.extend([0x20, offset & 0xFF])  # JR NZ, sara_check_loop
    # No Sara found - return without doing anything
    code.append(0xC9)                   # RET
    # sara_found:
    sara_found_label = len(code)
    code[sara_found_jump + 1] = (sara_found_label - sara_found_jump - 2) & 0xFF
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v171.gb")
    ...
    print("\n=== v1.71: BG Colorizer with Gameplay Detection ===")
    print("  Skips BG colorization on menus (checks for Sara in OAM)")
    print()
