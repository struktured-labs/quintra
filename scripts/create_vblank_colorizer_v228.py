#!/usr/bin/env python3
"""
v2.28: Stage Detection + Jet Form Palettes (based on working v2.26)

NEW in v2.28:
- Stage detection via 0xFFD0 (0x00=Level 1, 0x01=Bonus stage)
- Jet form OBJ palettes for Sara W and Sara D in bonus stage
- Minimal changes from v2.26 to ensure stability

Boss detection via 0xFFBF still works:
  - 0xFFBF = 1 → Gargoyle palette 6
  - 0xFFBF = 2 → Spider palette 7
"""
import sys
import yaml
...
def create_tile_based_colorizer() -> bytes:
    """Tile-based colorizer (unchanged from v2.26)."""
    code = bytearray()
    labels = {}
    jumps_to_fix = []
...
def create_shadow_colorizer_main(colorizer_addr: int) -> bytes:
    """Shadow colorizer (unchanged from v2.26)."""
    code = bytearray()
    ...
    code.extend([0xFE, 0x02, 0x28, 0x08])  # FIX: was +6, should be +8 to reach LD E, 7
...
def create_palette_loader(
    ...
    """
    Load CGB palettes with stage and boss detection.
    ...
    code.extend([0x7A, 0xFE, 0x01, 0x20, 0x03])  # LD A, D; CP 1; JR NZ, +3 (check stage from D)
    ...
    code.extend([0x7B, 0xFE, 0x01, 0x20, 0x03])  # LD A, E; CP 1; JR NZ, +3 (reuse boss flag from E)
    ...
    code.extend([0x7B, 0xFE, 0x02, 0x20, 0x03])  # LD A, E; CP 2; JR NZ, +3 (reuse boss flag from E)
...
def create_bg_colorizer_oneshot(lookup_table_addr: int) -> bytes:
    """One-shot BG colorizer (unchanged from v2.26)."""
    code = bytearray()
...
def create_combined_with_dma(palette_loader_addr: int, shadow_main_addr: int, bg_colorizer_addr: int) -> bytes:
    """Combined function (unchanged from v2.26)."""
    code = bytearray()
...
def create_vblank_hook_with_input(combined_func_addr: int) -> bytes:
    """VBlank hook (unchanged from v2.26)."""
...
def main():
    ...
    print("=== Penta Dragon DX v2.28 ===")
    print("Stage detection + Jet form palettes (based on working v2.26)")
    ...
    palette_loader_addr = 0x6900  # ~150 bytes
    shadow_main_addr = 0x69A0     # 52 bytes
    colorizer_addr = 0x69E0       # 98 bytes
    ...
    print(f"  Stage detection at 0xFFD0")
    print(f"  0x00=Level 1 (normal), 0x01=Bonus (jet forms)")
