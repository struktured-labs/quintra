#!/usr/bin/env python3
"""
v2.80: Hook-First Hybrid — IE masking + palette-first pass order

Two fixes for remaining flickering and SFX corruption:

1. IE MASKING: Save IE register at start, set IE=0x01 (VBlank only).
   This prevents timer interrupt (0x06B3 → bank 3 sound engine) and
   STAT interrupt (0x0048 → 0x0853) from firing during the copy.
   Only VBlank can fire, and the hook flag suppresses the sweep.
   Fixes: random SFX noises from interrupt corruption.

2. PALETTE-FIRST: Write palette attributes (VBK=1) BEFORE tile IDs (VBK=0).
   New tiles appear with correct palettes already in place.
   Old tiles briefly show with new palettes (scrolling off-screen = invisible).
   Fixes: ~5% scroll-edge flicker from tile→palette gap in v2.79.

Timeline:
  Copy starts → FF91=0x5A, IE=0x01 (sweep suppressed, timer/STAT masked)
  Pass 1: palette attributes (VBK=1) via ROM lookup table
  Pass 2: tile IDs (VBK=0) from WRAM C1A0
  Copy ends → FF91=0x00, IE restored → normal operation resumes
...
def create_enhanced_tilemap_copy(bg_table_addr: int, base_addr: int,
    ...
    """Enhanced tilemap copy: Pass 1 (palettes) + Pass 2 (tiles).

    v2.80 changes from v2.79:
    1. IE masking: save IE, set IE=0x01 (VBlank only) during entire copy
    2. Palette-first: Pass 1 writes palette attributes, Pass 2 writes tile IDs
       → new tiles appear with correct palettes already in place
    ...
    # ================================================================
    # PASS 1: Palette attributes via ROM lookup (VBK=1)
    # Write palettes FIRST so new tiles appear with correct colors.
    # Uses stack-based row counter (B needed for lookup table hi).
    # ================================================================
    ...
    emit_jr_back(0x20, 'p1_stat')  # wait for HBlank

    for _ in range(4):         # 4 tiles per HBlank window
    ...
    # Skip 8-column gap (tilemap is 32 wide, we only use 24)
    emit([0x7D])               # LD A, L
    ...
    emit([0xF5])               # PUSH AF
    emit_jp_back(0xC3, 'p1_row')

    # ================================================================
    # TRANSITION: Pass 1 done → Pass 2
    # ================================================================
    emit([0xAF])               # XOR A
    emit([0xE0, 0x4F])        # VBK = 0              ; tile data map

    emit([0xF0, 0xEE])        # LDH A,[FFEE]
    emit([0x67])               # LD H, A             ; rewind H
    emit([0x2E, 0x00])        # LD L, 0              ; rewind L
    emit([0x11, 0xA0, 0xC1]) # LD DE, 0xC1A0        ; rewind DE
    emit([0x0E, 24])          # C = 24 rows

    # ================================================================
    # PASS 2: Tile copy from C1A0 to VRAM (VBK=0)
    # B and C are free (no lookup table needed).
    # ================================================================
    mark('p2_row')
    ...
    emit([0x0D])               # DEC C (rows)
    emit_jp_back(0xC2, 'p2_row')

    # ================================================================
    # CLEANUP: restore VBK, clear hook flag, restore IE
    # ================================================================
    emit([0xAF])               # XOR A
    emit([0xE0, 0x4F])        # VBK = 0 (already, but be safe)
...
def build_v280():
    """Build v2.80 ROM — IE masking + palette-first pass order."""
    ...
    # Enhanced copy (hook) — now sets FF91=0x5A + FFA5=24
...
if __name__ == "__main__":
    rom_path = build_v280()
    print(f"\nBuilt v2.80: {rom_path}")
    print(f"Hook-first hybrid with IE masking + palette-first pass order")
    print(f"Fix 1: IE=0x01 during copy → no timer/STAT interrupts → no SFX corruption")
    print(f"Fix 2: Palette attributes written BEFORE tile IDs → no scroll-edge flicker")
