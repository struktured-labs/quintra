#!/usr/bin/env python3
"""
v2.43: Input Fix + No-STAT BG Colorizer

Fixes from v2.42:
1. PHANTOM INPUT FIX: Added 2nd d-pad read for proper debounce
   - Original game reads FF00 twice for d-pad (pullup settle time)
   - v2.42 only read once → phantom right-press → Sara walks forward alone
   - Saved 2 bytes in hook via A=0 trick (DMA leaves A=0, MBC1 bank 0=1)

2. SPEED FIX: Removed STAT waits from BG colorizer
   - v2.42 STAT waits cost ~124M per tile waiting for HBlank
   - 48 tiles × 185M = 8,880M = ~19 scanlines stolen from game
   - Now: 48 tiles × ~54M = 2,592M = ~6 scanlines (game keeps ~96% CPU)
   - Trade-off: Mode 3 VRAM reads return 0xFF → palette 0 (neutral floor)
   - Self-corrects within 2-3 sweeps (~0.7s) - same as stable v2.36

3. HRAM FIX: Moved BG counter from FFEA/FFEB to FF91/FF92
   - Game uses FFEA as VBlank countdown timer
   - Game uses FFEB for animation toggle + entity state (Sara form!)
   - Game uses FFEE for sprite Y position
   - Our writes to these caused "spaceship mode" (Sara forced movement)
   - FF91/FF92 are confirmed unused (between DMA at FF90 and game at FF93)
   - Eliminated FFEE temp by using PUSH/POP for palette B

INHERITED from v2.42/v2.36:
...
def create_bg_colorizer_no_stat(lookup_table_addr: int) -> bytes:
    """BG colorizer: direct VRAM access, no STAT waits (48 tiles/frame).

    Runs FIRST in VBlank handler when VRAM is freely accessible.
    No STAT mode checking - VRAM reads during mode 3 return 0xFF,
    which maps to palette 0 (neutral floor). Self-corrects in 2-3 sweeps.

    CRITICAL FIX (v2.43): Uses FF91/FF92 for position counter instead of
    FFEA/FFEB. The game uses FFEA as a VBlank countdown timer and FFEB for
    animation toggle + entity state (controls Sara form!). Using them caused
    Sara to move erratically ("spaceship mode"). FF91/FF92 are confirmed
    unused (between DMA routine at FF80-FF90 and game vars at FF93+).
    Also eliminated FFEE temp by using PUSH/POP (game uses FFEE for sprite Y).

    Per-tile cost: ~54M (no STAT wait overhead)
    48 tiles x ~54M = 2,592M (~6 scanlines, game keeps ~96% CPU)
    ...
    code.extend([0x7E])              # LD A, [HL]        ; 2M -> palette A (ROM read)
    ...
    code.extend([0xEE, 0x04])        # XOR 0x04          ; 2M -> 0x98->0x9C
    code.extend([0x67])              # LD H, A           ; 1M
    code.extend([0xF1])              # POP AF            ; 3M -> reload palette B
...
def create_vblank_hook_with_input(combined_func_addr: int) -> bytes:
    ...
    v2.43 changes from v2.42:
    - D-pad: 2 reads instead of 1 (proper debounce, prevents phantom input)
    - Hook: uses A=0 trick (DMA leaves A=0, MBC1 maps bank 0 -> bank 1)
      Saves 2 bytes (no need for LD A, 0x01 before bank switch back)
    """
...
def main():
    ...
    print("=== Penta Dragon DX v2.43 ===")
    print("Input Fix + No-STAT BG Colorizer")
    print()
    print("Fixes:")
    print("  1. D-pad debounce: 2 reads (was 1) - fixes phantom movement")
    print("  2. No STAT waits: 48 tiles x ~54M = 2,592M (~6 scanlines)")
    print("     Game keeps ~96% CPU (was ~89% with STAT waits)")
    print("  3. A=0 trick: DMA leaves A=0, MBC1 bank 0=1 (saves 2 bytes)")
    print()
