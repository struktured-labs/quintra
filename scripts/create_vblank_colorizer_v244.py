#!/usr/bin/env python3
"""
v2.44: Mode 3 Skip Fix + Wall Edge Tiles

Fixes from v2.43:
1. MODE 3 SKIP: When VRAM reads return 0xFF (LCD mode 3), the colorizer now
   SKIPS writing that tile's attribute instead of overwriting correct palettes
   with palette 0. This prevents wall tiles (palette 6) from flickering to
   floor color (palette 0). Uses sentinel value 0x80 in lookup[0xFF] +
   BIT 7 check before each attribute write.

2. WALL EDGE TILES: Tiles 0x16-0x17 (vertical wall lines at corridor edges)
   now assigned palette 6 (stone) instead of 0 (floor). These tiles form the
   visible border between walls and void, and should match wall color.

INHERITED from v2.43:
- HRAM fix (FF91/FF92 for counter, PUSH/POP for temp)
- D-pad debounce (2 reads)
- No-STAT BG colorizer (64 tiles/frame, ~8 scanlines)
- A=0 bank switch trick
...
def create_tile_palette_lookup() -> bytes:
    ...
    v2.44 changes:
    - Entry 0xFF = 0x80 (sentinel: bit 7 set = "don't write attribute")
      Mode 3 VRAM reads return 0xFF. Instead of writing palette 0 (which
      overwrites correct palette 6 on wall tiles), the colorizer skips
      the write, preserving the previously-correct palette.
    - Tiles 0x13-0x3F reclassified as palette 6 (stone/structural) based on
      actual VRAM tile pixel analysis showing brick/crosshatch patterns with
      density 0.84-2.78, clearly distinct from floor tiles (density 0.31-0.33)

    Palette assignments based on VRAM pixel density analysis:
      0    = Floor checkerboard + void (density < 0.5, tiles 0x00-0x12)
      0    = Arches/doors (tiles 0x60-0x87, blend with floor)
      1    = Items (bright gold, tiles 0x88-0xDF)
      6    = Stone: walls + platforms + stairs + decorative
             (density > 0.8, tiles 0x13-0x5F and 0xE0-0xFD)
      0x80 = Sentinel (mode 3 skip - don't overwrite existing attribute)
    """
    lookup = bytearray(256)

    for i in range(256):
        if i <= 0x04:
            lookup[i] = 0   # Floor checkerboard (0x00=empty, 0x01-0x04=floor pattern)
        elif i <= 0x06:
            lookup[i] = 6   # Platform corner pieces (connect floor to stone structures)
        elif i <= 0x12:
            lookup[i] = 0   # Floor edges + rare tiles (0x07-0x12)
        elif i <= 0x5F:
            lookup[i] = 6   # Stone: structural edges (0x13-0x1F) + platforms/stairs (0x20-0x3F) + walls (0x40-0x5F)
        elif i <= 0x87:
            ...
            lookup[i] = 0   # Void tile
        else:
            lookup[i] = 0x80  # 0xFF: sentinel (mode 3 skip)

    return bytes(lookup)


def create_bg_colorizer_no_stat(lookup_table_addr: int) -> bytes:
    """BG colorizer with mode 3 skip protection (48 tiles/frame).

    v2.44 changes from v2.43:
    - Added sentinel check (BIT 7) before each attribute write
    - When lookup returns 0x80 (sentinel, from mode 3 read returning 0xFF),
      the write is SKIPPED, preserving the existing correct attribute
    - This prevents wall tiles from losing palette 6 due to mode 3 failures

    HRAM: FF91=counter low, FF92=counter high (confirmed unused by game)
    Temp: PUSH/POP for palette B (avoids FFEE which game uses for sprite Y)

    Per-tile cost: ~58M (4M more than v2.43 for BIT checks)
    64 tiles x ~58M = 3,712M (~8 scanlines, game keeps ~94% CPU)
    Full sweep: 1024/64 = 16 frames (~0.27 seconds at 60fps)
    """
    ...
    code.extend([0x7E])              # LD A, [HL]        ; 2M -> palette A (or 0x80 sentinel)
    ...
    code.extend([0x7E])              # LD A, [HL]        ; 2M -> palette B (or 0x80 sentinel)
    code.extend([0xF5])              # PUSH AF           ; 4M -> save palette B on stack

    # --- Bank 1: write attributes ---
    code.extend([0x3E, 0x01])        # LD A, 1           ; 2M
    code.extend([0xE0, 0x4F])        # LDH [FF4F], A     ; 3M -> VRAM bank 1

    # Write palette A to tilemap A (0x9800 range) - with sentinel check
    code.extend([0x62])              # LD H, D           ; 1M -> always set HL to 9800 addr
    code.extend([0x6B])              # LD L, E           ; 1M    (needed for B address calc)
    code.extend([0xCB, 0x79])        # BIT 7, C          ; 2M -> sentinel check (NEW)
    code.extend([0x20, 0x01])        # JR NZ, +1         ; 3M -> skip write if sentinel (NEW)
    code.extend([0x71])              # LD [HL], C        ; 2M -> write palette A

    # Write palette B to tilemap B (0x9C00 range) - with sentinel check
    code.extend([0xF1])              # POP AF            ; 3M -> reload palette B (always, for stack)
    code.extend([0xCB, 0x7F])        # BIT 7, A          ; 2M -> sentinel check (NEW)
    code.extend([0x20, 0x06])        # JR NZ, +6         ; 3M -> skip write if sentinel (NEW)
    code.extend([0x4F])              # LD C, A           ; 1M -> save palette B in C (reuse C)
    code.extend([0x7C])              # LD A, H           ; 1M -> H still has D value
    code.extend([0xEE, 0x04])        # XOR 0x04          ; 2M -> 0x98->0x9C
    code.extend([0x67])              # LD H, A           ; 1M
    code.extend([0x71])              # LD [HL], C        ; 2M -> write palette B to 9C00
...
def create_vblank_hook_with_input(combined_func_addr: int) -> bytes:
    """VBlank hook with debounced input handling. Unchanged from v2.43."""
...
def main():
    ...
    print("=== Penta Dragon DX v2.44 ===")
    print("Mode 3 Skip Fix + Wall Edge Tiles")
    print()
    print("Fixes from v2.43:")
    print("  1. Mode 3 skip: VRAM reads returning 0xFF no longer overwrite")
    print("     correct palette attributes. Uses sentinel (0x80) + BIT 7 check.")
    print("  2. Stone tile reclassification: 0x13-0x3F now palette 6 (was 0)")
    print("     Based on VRAM pixel analysis: brick/crosshatch patterns (density 0.8-2.8)")
    print("     vs floor checkerboard (density 0.3). Covers structural edges,")
    print("     platforms, stairs - all visually stone, not floor.")
    print()
    ...
    bg_colorizer = create_bg_colorizer_no_stat(lookup_table_addr)
    ...
    if sentinel_count > 0:
        print(f"  Sentinel (mode 3 skip): {sentinel_count} entries")
