#!/usr/bin/env python3
"""
v1.99: Debug version - static buffer (palette 0), NO VRAM reading

Testing if the VRAM reading in buffer builder causes the corruption.
This version:
- Initializes buffer to all palette 0 (grayscale, should preserve text)
- Does GDMA every frame
- NO buffer building from VRAM (just uses static values)

If text is complete with this version, the bug is in the buffer builder's
VRAM reading logic.
"""
import sys
import yaml
...
def create_gdma_bg_colorizer() -> bytes:
    """GDMA copy with initialization flag."""
    ATTR_BUFFER = 0xD000
    TILEMAP_VRAM = 0x9800
    BLOCKS = 64
    HRAM_INIT = 0xC7
    ...
    code.extend([0x20, 0x12])             # JR NZ, skip_init (18 bytes ahead)

    # Initialize buffer to all palette 0
    code.extend([0x21, 0x00, 0xD0])       # LD HL, 0xD000
    code.extend([0x01, 0x00, 0x04])       # LD BC, 0x0400 (1024)
    code.extend([0xAF])                   # XOR A (palette 0)
    # fill_loop:
    ...
    code.extend([0x20, 0xF9])             # JR NZ, fill_loop
...
def create_combined_func(palette_loader_addr: int, shadow_main_addr: int,
    ...
    """Combined: palettes, sprites, GDMA (no buffer building), DMA."""
    code = bytearray()
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v199.gb")
    ...
    print("\n=== v1.99: Debug - static buffer palette 0, NO VRAM reading ===")
    print("  Tests if VRAM reading in buffer builder causes corruption")
    print()
