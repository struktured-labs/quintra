#!/usr/bin/env python3
"""v3.01 with attr_computation reduced to 2 rows × 24 tiles (1 DI window).

Tests the hypothesis: full attr_comp (8 chunks × ~6100T = ~48800T) overruns
the VBlank window (~4560T) and the LCD/rendering keeps going while we work,
causing cascading state corruption that looks like a freeze.

1 chunk ≈ 6100T. Still a bit over VBlank (~4560T) but much closer.
If passes → confirm scaling is the issue. Drop to <=1 chunk for safety.
If freezes → the issue isn't pure volume; investigate further.

Output: rom/working/penta_dragon_dx_v301_attr_2rows.gb
"""
import sys
...
def create_attr_computation_1chunk(bg_table_addr: int) -> bytes:
    """Single chunk of attr_comp: 3 rows × 24 tiles, one DI window."""
    ...
    # 3 rows
    code.extend([0x3E, 0x02])                     # LD A, 2 (row counter)
    code.extend([0xF5])                           # PUSH AF
    row_loop = len(code)
    code.extend([0x06, bg_table_hi])              # LD B, bg_table_hi
    code.extend([0xF5])                           # PUSH AF (save row ctr)
...
def build():
    ...
    output_path = Path("rom/working/penta_dragon_dx_v301_attr_2rows.gb")
    ...
    attr_comp = create_attr_computation_1chunk(bg_table_addr)
    ...
    print(f"  1-chunk attr_comp: {len(attr_comp)} bytes (3 rows × 24 tiles)")
