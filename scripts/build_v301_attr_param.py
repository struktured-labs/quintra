#!/usr/bin/env python3
"""Parametric attr_comp builder: row count + GDMA on/off.

Usage: python build_v301_attr_param.py <rows> <0|1>
  rows: number of rows of attr_comp per frame (1-24)
  gdma: 0 disabled, 1 enabled

Writes rom/working/penta_dragon_dx_v301_attr_R{rows}_G{gdma}.gb
"""
import sys
...
def create_attr_computation_n(bg_table_addr: int, n_rows: int) -> bytes:
    """attr_computation with parameterized row count."""
    bg_table_hi = (bg_table_addr >> 8) & 0xFF
    code = bytearray()
    code.extend([0xC5, 0xD5, 0xE5, 0xF5])
    code.extend([0x21, 0xA0, 0xC1])
    code.extend([0x11, 0x00, 0xD0])
    code.extend([0x3E, n_rows])
...
def build(n_rows: int, with_gdma: bool):
    ...
    rom[0x14D] = chk
    out = Path(f"rom/working/penta_dragon_dx_v301_attr_R{n_rows}_G{int(with_gdma)}.gb")
    out.write_bytes(rom)
    print(f"Wrote {out}")
