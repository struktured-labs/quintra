#!/usr/bin/env python3
"""
v1.82: BG Colorizer - simple loop, no WRAM buffer

- 8 tiles per frame, direct VBK switch per tile
- No WRAM buffer (simpler logic)
"""
import sys
import yaml
...
def create_bg_row_colorizer_lazy(lookup_table_addr: int) -> bytes:
    """BG colorizer - simple: 8 tiles with direct VBK switching per tile."""
    HRAM_COUNTER = 0xC4
    WARMUP_FRAMES = 60
    TOTAL_STEPS = 128
    TILES_PER_FRAME = 8
    ...
    # Restore HL (VRAM pos), write palette
    code.append(0xE1)                   # POP HL
    code.append(0x7B)                   # LD A, E (palette)
    code.append(0x77)                   # LD [HL], A

    # VBK = 0 (restore for next read)
    code.extend([0xAF])                 # XOR A
    code.extend([0xE0, 0x4F])           # LDH [VBK], A
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v182.gb")
    ...
    print("\n=== v1.82: Simple loop - no WRAM buffer ===")
    print("  8 tiles/frame, direct VBK switch per tile")
    print()
