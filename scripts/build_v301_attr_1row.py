#!/usr/bin/env python3
"""v3.01 with attr_computation reduced to 1 row × 24 tiles, single DI window."""
import sys
...
def create_attr_1row(bg_table_addr):
    ...
    code.extend([0x3E, 0x18])                     # 24 tiles
    tl = len(code)
    code.extend([0xF5])
    code.extend([0x2A, 0x4F, 0x0A, 0x12, 0x13])
    code.extend([0xF1, 0x3D])
    off = tl - (len(code) + 2)
    code.extend([0x20, off & 0xFF])
...
def build():
    ...
    print(f"  attr 1row: {len(create_attr_1row(A['bt']))} bytes")
    ...
    rom[0x14D] = chk
    out = Path("rom/working/penta_dragon_dx_v301_attr_1row.gb")
    out.write_bytes(rom)
    print(f"Wrote {out}")
