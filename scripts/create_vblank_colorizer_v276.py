#!/usr/bin/env python3
"""
v2.76: Hook-First Hybrid — Zero VBlank BG during scrolling, sweep fallback when idle

Combines the O(1) tilemap hook (v2.75) with a lightweight VBlank sweep (v2.73).

During scrolling: Hook intercepts 0x42A7, writes palettes inline → ZERO VBlank BG cost
During idle:      VBlank sweep maintains palette attributes → ~13% VBlank BG cost

Hook sets FF91=0x5A ("I just ran") + FFA5=18 (skip sweep Phase 1).
VBlank combined checks FF91: if 0x5A → clear flag, skip BG. Else → run sweep.
...
Net effect:
  ...
  - 100% accuracy on ALL gameplay states
"""
...
def build_v276():
    """Build v2.76 ROM — hook-first hybrid."""
...
if __name__ == "__main__":
    rom_path = build_v276()
    print(f"\nBuilt v2.76: {rom_path}")
    print(f"Hook-first hybrid: zero VBlank BG during scrolling, sweep fallback when idle")
