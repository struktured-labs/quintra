#!/usr/bin/env python3
"""DI-budget probe with FF70=2 HELD across the DI window (no writes).

Tests: is the freeze about how long bank 2 stays mapped, or about doing
work while bank 2 is mapped?

Sequence: DI; FF70=2; <nop_count NOPs>; FF70=1; EI.
"""
import sys
...
def create_ff70_hold(nop_count: int) -> bytes:
    code = bytearray()
    code.extend([0xF3])                   # DI
    code.extend([0x3E, 0x02, 0xE0, 0x70]) # FF70 = 2
    code.extend([0x00] * nop_count)       # NOPs (4T each) while bank 2 mapped
...
def build(nop_count: int):
    ...
    w(A['sw'], bytes(sw))
    w(A['ac'], create_ff70_hold(nop_count))
    ...
    rom[0x14D] = chk
    out = Path(f"rom/working/penta_dragon_dx_v301_ff70hold_{nop_count}.gb")
    out.write_bytes(rom)
    print(f"Wrote {out} (DI + FF70=2 + {nop_count} NOPs ({nop_count * 4}T) + FF70=1 + EI)")
