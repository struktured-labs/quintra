#!/usr/bin/env python3
"""
Test: Keep colorizer and colorizer_main at working addresses (0x6C00, 0x6C20),
but move ONLY the combined routine to 0x6CD0.
"""
import sys
import yaml
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_COMBINED_6CD0.gb")
    ...
    # Keep colorizer and colorizer_main at working addresses
    palette_loader_addr = 0x6800
    colorizer_addr = 0x6C00       # Same as ZEROBG_GRAYOBJ
    colorizer_main_addr = 0x6C20  # Same as ZEROBG_GRAYOBJ
    combined_addr = 0x6CD0        # ONLY THIS CHANGES
    ...
    print(f"Combined at: 0x{combined_addr:04X} (TEST ADDRESS)")
    print(f"Colorizer_main at: 0x{colorizer_main_addr:04X} (working address)")
    print(f"Colorizer at: 0x{colorizer_addr:04X} (working address)")
