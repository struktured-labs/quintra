#!/usr/bin/env python3
"""
v1.88: VRAM read test (no write)

- VBK switching + read 8 tiles from VRAM + lookup table
- NO VRAM WRITE to bank 1
- Tests if VRAM reading or lookup causes Sara corruption
"""
import sys
import yaml
...
def create_bg_row_colorizer_lazy(lookup_table_addr: int) -> bytes:
    """VRAM read test - read tiles and lookup, but NO VRAM write.

    Tests if VRAM reading or lookup table access causes Sara corruption.
    """
    TILES_PER_FRAME = 8
    code = bytearray()
    ...
    # Loop: read tiles and lookup palettes (but don't write)
    main_loop = len(code)

    # Read tile ID from VRAM
    code.append(0x7E)                   # LD A, [HL]
    code.append(0xE5)                   # PUSH HL (save VRAM pos)
    ...
    code.append(0x7E)                   # LD A, [HL] = palette (read but don't use)

    # Restore HL
    code.append(0xE1)                   # POP HL

    # NO VRAM WRITE - just skip to next tile
    code.append(0x23)                   # INC HL
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v188.gb")
    ...
    print("\n=== v1.88: VRAM read test (no write) ===")
    print("  Read 8 tiles + lookup palettes, NO VRAM write")
    print("  Tests if VRAM reading causes Sara corruption")
    print()
