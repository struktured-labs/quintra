...
-- Curriculum save-state hooks: auto-save mgba states at key game events.
local CURRICULUM_DIR = "/home/struktured/projects/penta-dragon-dx-claude/rl/saves/curriculum"
os.execute("mkdir -p " .. CURRICULUM_DIR)
local last_d880 = -1
local last_ffba = -1
local last_dcb8 = -1
local last_ffbf = -1
local seen_arenas = {}  -- D880 values 0x0C..0x14 already saved
local kill_count = 0   -- mini-boss kills observed (FFBF non-zero → 0)
...
local REC_PATH = os.getenv("REC_PATH") or "/home/struktured/projects/penta-dragon-dx-claude/rl/bc_data/expert_human_curriculum.jsonl"
...
callbacks:add("frame", function()
    ...
    -- Curriculum save-state hooks: detect transitions BEFORE rate-limit
    local d880 = emu:read8(0xD880)
    local ffba = emu:read8(0xFFBA)
    local dcb8 = emu:read8(0xDCB8)
    local ffbf = emu:read8(0xFFBF)
    local function tag(name)
        local path = string.format("%s/L%d_f%06d_%s.state", CURRICULUM_DIR, ffba, f, name)
        local ok, err = pcall(function() emu:saveStateFile(path) end)
        if ok then
            console:log(string.format("[SAVE] %s @ f=%d D880=%02X FFBA=%d DCB8=%d FFBF=%d",
                name, f, d880, ffba, dcb8, ffbf))
        else
            console:log(string.format("[SAVE-FAIL] %s: %s", name, tostring(err)))
        end
    end
    if last_d880 ~= -1 then
        -- Mini-boss kill: FFBF transition non-zero -> 0
        if last_ffbf ~= 0 and ffbf == 0 then
            kill_count = kill_count + 1
            tag("post_kill_" .. kill_count)
        end
        -- Stage boss arena entry: D880 first time in 0x0C..0x14
        if d880 >= 0x0C and d880 <= 0x14 and not seen_arenas[d880] then
            seen_arenas[d880] = true
            tag("arena_enter_" .. string.format("%02X", d880))
        end
        -- Boss splash (cinematic) — D880 → 0x18
        if last_d880 ~= 0x18 and d880 == 0x18 then
            tag("boss_splash")
        end
        -- Post-boss reload — D880 → 0x16
        if last_d880 ~= 0x16 and d880 == 0x16 then
            tag("post_boss_reload")
        end
        -- Level transition — FFBA increments
        if ffba > last_ffba and last_ffba ~= -1 then
            tag("level_advance_to_" .. ffba)
        end
    end
    last_d880 = d880; last_ffba = ffba; last_dcb8 = dcb8; last_ffbf = ffbf
