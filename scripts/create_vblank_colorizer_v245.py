#!/usr/bin/env python3
"""
v2.45: VBlank Timing Fix - Palettes First

ROOT CAUSE FIX: In v2.44, the BG colorizer consumed 4288M of VBlank (4560M total),
pushing the palette loader OUTSIDE VBlank. Palette color writes during rendering
caused visible color tearing on Sara W and other sprites (mid-frame palette changes).

Fix: Reorder combined function to Palette -> BG -> OBJ -> DMA
- Palette loader runs FIRST (~1400M) - fully within VBlank, no color tearing
- BG colorizer runs SECOND (48 tiles x 58M = 2784M) - fits within VBlank
- OBJ colorizer + DMA run after VBlank (WRAM + OAM, safe outside VBlank)
- Total VBlank usage: 130M hook + 1400M palette + 2784M BG = 4314M < 4560M

Other changes from v2.44:
- REMOVED sentinel check (unnecessary - all VRAM reads happen during VBlank)
- Reduced tiles from 64 to 48 (to fit alongside palette loader in VBlank)
- Kept improved tile lookup (0x05-0x06, 0x13-0x3F as palette 6)

INHERITED:
- HRAM fix (FF91/FF92 for counter)
...
def create_tile_palette_lookup() -> bytes:
    ...
    v2.45: No sentinel needed (all VRAM reads happen during VBlank).
    Tile classification based on VRAM pixel density analysis:
      0 = Floor checkerboard + void (density < 0.5, tiles 0x00-0x04, 0x07-0x12)
      0 = Arches/doors (tiles 0x60-0x87, blend with floor)
      1 = Items (bright gold, tiles 0x88-0xDF)
      6 = Stone: corners (0x05-0x06) + structural (0x13-0x3F) + walls (0x40-0x5F)
      6 = Decorative (0xE0-0xFD)
    """
    ...
    for i in range(256):
        if i <= 0x04:
            ...
            lookup[i] = 6   # Stone: structural edges + platforms/stairs + walls
        elif i <= 0x87:
            ...
            lookup[i] = 0   # 0xFE-0xFF: void/empty

    return bytes(lookup)


def create_bg_colorizer_vblank_safe(lookup_table_addr: int) -> bytes:
    """BG colorizer - runs within VBlank after palette loader (48 tiles/frame).

    v2.45: No sentinel checks needed - all VRAM reads happen during VBlank.
    Palette loader runs first (~1400M), so BG starts at ~1530M into VBlank.
    48 tiles x 58M = 2784M. Total VBlank: 1530 + 2784 = 4314M < 4560M.
    ...
    # Set tile count
    code.extend([0x06, TILES_PER_VBLANK])  # LD B, 48

    # Bank 0 before loop
    code.extend([0xAF])              # XOR A            ; 1M
    code.extend([0xE0, 0x4F])        # LDH [FF4F], A    ; 3M -> VRAM bank 0

    # === SWEEP LOOP (no sentinel - VRAM safe during VBlank) ===
    ...
    code.extend([0x4E])              # LD C, [HL]        ; 2M -> palette A in C directly
    ...
    code.extend([0xF1])              # POP AF            ; 3M -> palette B
    code.extend([0x4F])              # LD C, A           ; 1M
...
def create_combined_with_dma(palette_loader_addr: int, shadow_main_addr: int, bg_colorizer_addr: int) -> bytes:
    """Combined function: palette_loader -> bg_colorizer -> shadow_main -> DMA.

    v2.45 CRITICAL FIX: Palette loader runs FIRST to guarantee palette color
    writes complete within VBlank. Prevents mid-frame color tearing on Sara
    and other sprites that was caused by v2.44's BG-first ordering.

    Order rationale:
    1. Palette loader (~1400M) - MUST be in VBlank to avoid color tearing
    2. BG colorizer (48x58M=2784M) - VRAM writes need VBlank
    3. Shadow OBJ colorizer (~800M) - writes WRAM, safe outside VBlank
    4. DMA (160M) - copies shadow OAM to hardware, safe outside VBlank
...
def main():
    ...
    print("=== Penta Dragon DX v2.45 ===")
    print("VBlank Timing Fix - Palettes First")
    print()
    print("Fixes from v2.44:")
    print("  1. PALETTE TIMING: Palette loader now runs FIRST in VBlank (~1400M)")
    print("     Fixes Sara W color tearing (palette writes during rendering)")
    print("  2. Reduced BG tiles to 48 (fits alongside palette in VBlank)")
    print("  3. Removed sentinel check (unnecessary - reads safe during VBlank)")
    print("  Combined order: Palette -> BG -> OBJ -> DMA")
    print("  VBlank budget: 130M + 1400M + 2784M = 4314M < 4560M")
    print()
    ...
    bg_colorizer = create_bg_colorizer_vblank_safe(lookup_table_addr)
