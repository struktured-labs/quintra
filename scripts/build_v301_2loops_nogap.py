#!/usr/bin/env python3
"""2 sequential tile_loops, NO DE += 8 between. Writes stay contiguous (0xD000-0xD02F).

If PASS → DE += 8 (the gap) is the freezing trigger.
If FREEZE → having two sequential tile_loops (regardless of DE state) is the trigger.
"""
import sys
...
def create_2loops_nogap(bg_table_addr):
    ...
    # First tile_loop (24 tiles, DE: D000→D018)
    code.extend([0x3E, 0x18])
    tl1 = len(code)
    code.extend([0xF5, 0x2A, 0x4F, 0x0A, 0x12, 0x13, 0xF1, 0x3D])
    code.extend([0x20, (tl1 - (len(code) + 2)) & 0xFF])

    # NO DE += 8 — DE remains at 0xD018, contiguous

    # Second tile_loop (24 tiles, DE: D018→D030)
    code.extend([0x3E, 0x18])
    tl2 = len(code)
    code.extend([0xF5, 0x2A, 0x4F, 0x0A, 0x12, 0x13, 0xF1, 0x3D])
...
def build():
    ...
    w(A['sw'], bytes(sw))
    w(A['ac'], create_2loops_nogap(A['bt']))
    ...
    rom[0x14D] = chk
    out = Path("rom/working/penta_dragon_dx_v301_2loops_nogap.gb")
    out.write_bytes(rom)
    print(f"Wrote {out}")
