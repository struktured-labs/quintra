...
TP_OUT = Path("rom/working/penta_dragon_dx_teleport_v17b.gb")
...
def build_teleport_routine() -> bytes:
    ...
    # copy_done:
    copy_done_pos = len(c)
    # ---- v17b minimal: read DF0A, OR A, JR Z to combo_check ----
    # No DX fire path. If the DF0A read alone triggers a freeze, the
    # ISSUE is the read itself (e.g. addressing a side-effect register
    # or the LD inst's interaction with WRAM bank or sound DMA).
    c.extend([0xFA, 0x0A, 0xDF])          # LD A, [DF0A]
    c.extend([0xB7])                      # OR A
    c.extend([0x18, 0x00])                # JR (unconditional, +0) — equivalent
                                          # to a no-op skip since there is no
                                          # DX fire path to gate. Falls through
                                          # to combo check.
