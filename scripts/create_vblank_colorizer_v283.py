#!/usr/bin/env python3
"""
v2.83: Hook-First Hybrid — Mini-batch interleaved (4-tile atomic palette+tile)

Root cause of remaining ~1% flicker in v2.81/v2.82:
In per-row interleaving, all 24 palettes are written (6 HBlank windows), then
all 24 tiles (6 more HBlanks). The ~6 scanline gap between palette and tile
writes for the same tiles means the LCD can render a row with new palettes but
old tiles — visible as transient color flashes during scrolling.

Fix: Mini-batch interleaving. Instead of all palettes then all tiles per row,
process 4 tiles at a time: write 4 palettes (1 HBlank), then immediately write
4 tiles (next HBlank). 6 mini-batches per row. The palette→tile gap shrinks
from ~6 scanlines to <1 scanline — effectively atomic from LCD's perspective.

Same total HBlank count as v2.81 (12 per row), just reordered for minimal gap.
...
def build_v283():
    """Build v2.83 ROM — mini-batch interleaved (4-tile atomic palette+tile)."""
...
if __name__ == "__main__":
    rom_path = build_v283()
    print(f"\nBuilt v2.83: {rom_path}")
    print(f"Hook-first hybrid with mini-batch interleaving")
    print(f"Each 4-tile group: palette→transition→tile in <1 scanline (was 6 in v2.81)")
    print(f"6 mini-batches per row × 24 rows = same HBlank count, near-zero gap")
