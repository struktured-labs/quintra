#!/usr/bin/env python3
"""
v2.22: Fix VBlank Timing - Load Palettes Only Once

CRITICAL FIX: Palette loader now runs only ONCE instead of every frame.
This fixes:
- Music slowdown (VBlank was taking too long)
- Title screen input lag
- General performance issues

Uses HRAM 0xFFE1 as "palettes loaded" flag.
...
def create_vblank_hook_with_input(combined_func_addr: int) -> bytes:
    """VBlank hook with input handler."""
...
def main():
    ...
    output_rom = project_root / "rom/working/penta_dragon_dx_v222.gb"
    ...
    print("\n=== v2.22: Fix VBlank Timing - Palette Loader One-Shot ===")
    print("  CRITICAL FIX: Palette loader runs only ONCE (not every frame)")
    print("  This fixes music slowdown, input lag, and performance issues")
    print("  OBJ: Tile-based colorization with Sara form detection")
    print("  BG: One-shot approach - colorizes once per level entry")
    print()
