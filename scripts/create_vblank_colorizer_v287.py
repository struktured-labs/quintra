#!/usr/bin/env python3
"""
v2.87: Audio Fix — Palette Caching + Joypad Release Fix

Fixes from v2.86:
- Added joypad release (0x30→FF00) after P13 read in combined handler.
  Without this, P13 stays selected through the entire colorizer routine,
  which can cause phantom button ghost reads on MiSTer FPGA hardware.

The GB sound engine runs on Timer interrupt (~89 Hz). Our VBlank handler
blocks Timer for ~5000+ T-cycles (vs vanilla's ~1100 T). Investigation
showed even 1 M-cycle of timing change causes complete audio waveform
divergence (chaotic sensitivity), and the extra VBlank time causes audible
audio dropouts (~12% silence vs vanilla's 5%).

Root cause: palette loading runs every frame (~550 M-cycles) even though
palettes rarely change (only on room/form/boss/tilemap transitions).

Fix: Restore hash-based palette caching using FF83 (verified unused by game
which is used for scroll detection). Hash combines FFBE+FFBF+FFC0+FFD0.
Skips palette loading when hash matches → saves ~550 M-cycles per frame
→ reduces VBlank from ~5000T to ~2800T → dramatically reduces Timer delay.

Previous (v2.85): Preserved P13 routine at 0x083C for bank 10 entry point.
...
3. FFA9 Repurposed: Previously used as conditional palette hash cache + sweep
   palette temp. Now holds "previous SCX/8" for scroll detection.
   v2.87: Palette hash cache moved to FF83 (FFA6 used by game! banks 12/13).
...
WRAM assignments (verified unused: no EA/FA refs in original ROM):
  DF00: Palette state hash cache (FFBE^FFBF^FFC0^FFD0^FFC1^FFBD + 1)
         (was FFA6 — CONFLICT! was FF83 — CONFLICT with DMA routine!)
  DF01: Base hi temp for scroll-edge/BG sweep
         (was FFEE — CONFLICT! was FF89 — CONFLICT with DMA routine!)
"""
...
def create_conditional_palette_cached(palette_loader_addr: int) -> bytes:
    """Hash-cached palette wrapper using FF83 (verified unused by original game).

    v2.87: Computes hash from FFBE (form) ^ FFBF (boss) ^ FFC0 (powerup) ^
    FFD0 (tilemap hi) ^ FFC1 (game state) ^ FFBD (room) + 1.
    Compares with FF83. If unchanged, skips palette loading entirely
    → saves ~550 M-cycles per VBlank frame.

    v2.86 bug: FFC1 was missing from hash, so title→gameplay transition
    didn't trigger palette reload (both had hash=1). Added FFC1 and FFBD.
    """
    code = bytearray()
...
def create_combined_with_scroll_edge(bg_sweep_addr: int, cond_pal_addr: int,
    ...
    emit([
        ...
        0xB0,        # OR B
        0xE0, 0x93,  # LDH [FF93], A        ; store combined result
        # --- Joypad release: deselect both P14+P13 lines ---
        0x3E, 0x30,  # LD A, 0x30           ; deselect both
        0xE0, 0x00,  # LDH [FF00], A        ; release joypad
    ])
    ...
    # Compute right_col = (cur_scx + 19) & 0x1F  (rightmost visible column)
    # Was +20 (1 beyond visible), but that misses the actual new edge column.
    # +19 = the actual rightmost visible tile that just scrolled into view.
    emit([0xC6, 19])          # ADD 19
    ...
    # (FF89 may be stale if sweep just ran with different LCDC; recompute)
    ...
    emit([0xEA, 0x01, 0xDF])  # LD [DF01],A           ; save for D wrap check (WRAM)
    ...
    emit([0xFA, 0x01, 0xDF])  # LD A,[DF01]           ; base_hi (WRAM)
    emit([0x80])               # ADD B
...
def build_v287():
    ...
    output_path = Path("rom/working/penta_dragon_dx_v287.gb")
    ...
    print(f"Cond palette (hash-cached FF83): {len(cond_pal)} bytes at 0x{cond_pal_addr:04X}")
