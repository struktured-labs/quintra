#!/usr/bin/env python3
"""
v1.76: BG Colorizer - NO WRITE test

- Only read tiles, don't write attributes
- Testing if the VRAM write to bank 1 causes freeze
"""
import sys
import yaml
...
def create_bg_row_colorizer_lazy(lookup_table_addr: int) -> bytes:
    """BG colorizer - STUB that just returns immediately for testing."""
    code = bytearray()
    # Just return immediately - testing if even being called causes freeze
    code.append(0xC9)  # RET
    return bytes(code)
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v176.gb")
    ...
    print("\n=== v1.76: BG Colorizer - STUB (just RET) ===")
    print("  Testing if just calling the function causes freeze")
    print()
