#!/usr/bin/env python3
"""
v2.60: Conditional palette loading for MiSTer FPGA speed fix

ROOT CAUSE: On MiSTer, the FULL handler (BG+Palette+OBJ+DMA) exceeds the
per-frame CPU budget, causing 5x game slowdown. BGONLY and OBJONLY are each
individually fast, but combined they overflow. The palette loader alone
costs ~4,800 T-cycles per frame writing 128+ bytes to BCPD/OCPD, even when
game state hasn't changed.

FIX: Conditional palette loading. Hash FFBE^FFBF^FFC0^FFD0 and compare with
cached value in FFA9. Skip palette load when state unchanged (95%+ of frames).
Saves ~4,700 T-cycles per frame.

Key features (inherited from v2.59):
- Interleaved read→write per tilemap (not batched reads then batched writes)
- SET 2, D / RES 2, D for fast 0x9800↔0x9C00 toggling (8cy, 2 bytes each)
- 0xFF anti-flicker filter on BOTH tilemap reads independently
- 109 tiles/frame (prime, coprime with 1024, full speed all scenes)
...
New in v2.60:
- Conditional palette: only reloads when FFBE/FFBF/FFC0/FFD0 changes
- ~116T per frame when skipping (vs ~4,844T every frame in v2.59)
- Net savings: ~4,728 T-cycles per frame on 95%+ of frames
"""
import sys
import yaml
...
def create_bg_colorizer(bg_table_addr: int) -> bytes:
    """BG colorizer: INTERLEAVED read→write for both tilemaps.

    Uses interleaved pattern: read 9800 tile → write 9800 attr → read 9C00
    tile → write 9C00 attr. This keeps ~30M between read and write (writes
    succeed) vs batched ~65M gap (writes dropped during PPU mode 3).

    SET 2, D / RES 2, D for fast 0x9800↔0x9C00 toggling.
    0xFF anti-flicker filter on BOTH tilemap reads independently.
    Safe HRAM: FF91=counter_lo, FFA5=counter_hi
    """
    # Prime, coprime with 1024 (GCD=1). Both tilemaps at each position.
    # Full sweep per tilemap: 1024/109 ≈ 9.4 frames (~0.16s).
    # Empirical speed test: 109 = full speed on ALL scenes (alone + busy hornets).
    # 127 = full speed on calm scenes but ~80% on busy scenes.
    # Dual-tilemap at 109 positions = ~26,160 T-cycles BG.
    TILES_PER_VBLANK = 109  # 0x6D - prime, coprime with 1024, full speed all scenes

    bg_table_hi = (bg_table_addr >> 8) & 0xFF

    code = bytearray()
    forward_jumps = []  # list of (offset_pos, target_label)
    labels = {}
    ...
    # === LOAD SWEEP POSITION (always 0x98xx base) ===
    emit([0xF0, 0xA5])        # LDH A, [FFA5]     ; counter high
    emit([0xE6, 0x03])        # AND 0x03           ; 0-3
    emit([0xC6, 0x98])        # ADD A, 0x98        ; D = 0x98-0x9B
    emit([0x57])              # LD D, A
    ...
    # === 9800 read-write (interleaved) ===
    emit([0x1A])              # LD A, [DE]         ; read 9800 tile (VRAM bank 0)
    emit([0x6F])              # LD L, A            ; lookup index
    emit([0x7E])              # LD A, [HL]         ; palette A from ROM table
    emit([0xFE, 0xFF])        # CP 0xFF            ; garbage filter
    emit_jr(0x28, 'skip_9800')  # JR Z, skip_9800
    emit([0x4F])              # LD C, A            ; save palette in C
    emit([0x3E, 0x01])        # LD A, 0x01
    emit([0xE0, 0x4F])        # LDH [FF4F], A      ; VRAM bank 1
    emit([0x79])              # LD A, C            ; restore palette
    emit([0x12])              # LD [DE], A          ; write 9800 attr (interleaved ~30cy after read)
    emit([0xAF])              # XOR A
    emit([0xE0, 0x4F])        # LDH [FF4F], A      ; VRAM bank 0

    labels['skip_9800'] = len(code)

    # === Toggle to 0x9C00 ===
    emit([0xCB, 0xD2])        # SET 2, D           ; D: 0x98→0x9C (8 cycles, 2 bytes)

    # === 9C00 read-write (interleaved) ===
    emit([0x1A])              # LD A, [DE]         ; read 9C00 tile (VRAM bank 0)
    emit([0x6F])              # LD L, A            ; lookup index
    emit([0x7E])              # LD A, [HL]         ; palette B from ROM table
    emit([0xFE, 0xFF])        # CP 0xFF            ; garbage filter
    emit_jr(0x28, 'skip_9C00')  # JR Z, skip_9C00
    emit([0x4F])              # LD C, A            ; save palette in C
    emit([0x3E, 0x01])        # LD A, 0x01
    emit([0xE0, 0x4F])        # LDH [FF4F], A      ; VRAM bank 1
    emit([0x79])              # LD A, C            ; restore palette
    emit([0x12])              # LD [DE], A          ; write 9C00 attr (interleaved)
    emit([0xAF])              # XOR A
    emit([0xE0, 0x4F])        # LDH [FF4F], A      ; VRAM bank 0

    labels['skip_9C00'] = len(code)

    # === Toggle back to 0x9800 ===
    emit([0xCB, 0x92])        # RES 2, D           ; D: 0x9C→0x98

    # === Advance + wrap ===
    emit([0x13])              # INC DE
    emit([0x7A])              # LD A, D
    emit([0xFE, 0x9C])        # CP 0x9C             ; hardcoded wrap point
    emit_jr(0x20, 'no_wrap')  # JR NZ, no_wrap
    emit([0x16, 0x98])        # LD D, 0x98          ; reset to base
    ...
    if sweep_offset < -128 or sweep_offset > 127:
        raise ValueError(f"Sweep loop JR NZ offset {sweep_offset} out of range (-128..+127)!")
...
def main():
    ...
    print("=== Penta Dragon DX v2.60 ===")
    print("Conditional palette + dual-tilemap BG + 109 tiles/frame + MiSTer speed fix")
    print()
