#!/usr/bin/env python3
"""
v1.90: VRAM write + counter test

- Same as v1.89 but with HRAM counter and warmup logic
- Fixed address (0x9800) - no row calculation
- Tests if counter logic causes Sara corruption
"""
import sys
import yaml
...
def create_bg_row_colorizer_lazy(lookup_table_addr: int) -> bytes:
    """VRAM write + counter test - adds counter logic to v1.89.

    Tests if counter/warmup logic causes Sara corruption.
    """
    HRAM_COUNTER = 0xC4
    WARMUP_FRAMES = 60
    WRAP_VALUE = 188
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v190.gb")
    ...
    print("\n=== v1.90: Counter + write test ===")
    print("  VRAM write + counter/warmup logic, fixed address (0x9800)")
    print("  Tests if counter logic causes Sara corruption")
    print()
