#!/usr/bin/env python3
"""
v2.29: Projectile Colorization (based on v2.28)

NEW in v2.29:
- Projectile sub-range detection within tiles 0x00-0x0F
- Dynamic Palette 0 loading based on Sara form (pink for Witch, green for Dragon)
- Enemy projectiles use distinct color (palette 3 - blue/dark)
- Tile range assumptions (to be verified by testing):
  * 0x00-0x07: Sara projectiles → Palette 0 (dynamic)
...
def create_tile_based_colorizer() -> bytes:
    """Tile-based colorizer with projectile sub-range detection (v2.29)."""
    code = bytearray()
    labels = {}
    jumps_to_fix = []
    ...
    # NEW: Check projectile range (< 0x10)
    code.extend([0xFE, 0x10])
    jumps_to_fix.append((len(code), 'check_projectile_subrange'))
    code.extend([0x38, 0x00])
    ...
    # NEW: Projectile sub-range detection
    labels['check_projectile_subrange'] = len(code)
    code.append(0x79)           # LD A, C (get tile ID)
    code.extend([0xFE, 0x08])   # CP 0x08
    jumps_to_fix.append((len(code), 'sara_projectile'))
    code.extend([0x38, 0x00])   # JR C, sara_projectile (tiles 0x00-0x07)
    # Else: enemy projectile (tiles 0x08-0x0F)
    labels['enemy_projectile'] = len(code)
    code.extend([0x3E, 0x03])   # LD A, 3 (Crow palette - blue/dark)
    jumps_to_fix.append((len(code), 'apply_palette'))
    code.extend([0x18, 0x00])

    labels['sara_projectile'] = len(code)
...
def create_palette_loader(
    ...
    """
    Load CGB palettes with stage, boss, and Sara form detection (v2.29).

    NEW: Dynamic Palette 0 loading based on Sara form
    - Sara W (0xFFBE=0): Pink projectile colors
    - Sara D (0xFFBE≠0): Green projectile colors

    Stage detection: Check 0xFFD0 for jet form palettes
    Boss detection: Check 0xFFBF for miniboss palettes
    """
    code = bytearray()

    # Check Sara form flag and save in C register for Palette 0 logic
    ...
    # Check boss flag and save in E register (read once, reuse for palette 6 and 7)
    ...
    # NEW: Load OBJ palette 0 (Sara projectiles - dynamic based on form)
    obj_data_addr = palette_data_addr + 64
    code.extend([0x3E, 0x80, 0xE0, 0x6A])  # Set OCPS to palette 0
    # Check Sara form in C register
    code.extend([0x79, 0xB7])  # LD A, C; OR A (check if Sara W = 0)
    # If Sara W (C=0), load SaraProjectileWitch palette
    # If Sara D (C≠0), load SaraProjectileDragon palette
...
def main():
    ...
    print("=== Penta Dragon DX v2.29 ===")
    print("Projectile colorization + dynamic Palette 0 (based on v2.28)")
    ...
    # For now, use same as Sara Dragon sprite palette
    ...
    sara_projectile_dragon_addr = 0x68A0  # NEW: Sara D projectile palette
    palette_loader_addr = 0x6900  # ~160 bytes (increased from v2.28)
    shadow_main_addr = 0x69B0     # 52 bytes
    colorizer_addr = 0x69F0       # ~110 bytes (increased from v2.28)
    ...
    print(f"\nProjectile colorization features:")
    print(f"  - Tiles 0x00-0x07: Sara projectiles (Palette 0 - dynamic)")
    print(f"  - Tiles 0x08-0x0F: Enemy projectiles (Palette 3 - blue/dark)")
