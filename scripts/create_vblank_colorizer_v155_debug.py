#!/usr/bin/env python3
"""
v1.55 DEBUG: Force all BG tiles to palette 3 (green) to verify VBK writes work.

If BG turns green, the VBK writes are working and we just need to fix the lookup.
If BG stays blue/purple, VBK writes aren't taking effect.
"""
import sys
import yaml
...
def create_bg_row_colorizer_debug() -> bytes:
    """
    DEBUG VERSION: Force ALL tiles to palette 3 (green)
    This tests if VBK writes are working at all.
    """
    code = bytearray()
    ...
    # Simple loop: write palette 3 to all 32 tiles in this row
    row_loop = len(code)
    code.extend([0x3E, 0x03])           # LD A, 3 (palette 3 - green)
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v155_debug.gb")
    ...
    print("\n=== v1.55 DEBUG: Force ALL BG to palette 3 (green) ===")
    print("  If BG turns green -> VBK writes work, fix lookup logic")
    print("  If BG stays blue/purple -> VBK writes failing")
    print()
    ...
    bg_colorizer = create_bg_row_colorizer_debug()  # DEBUG version
    ...
    print(f"BG row colorizer (DEBUG): {len(bg_colorizer)} bytes - forces palette 3")
    ...
    print("\n=== v1.55 DEBUG Build Complete ===")
    print("\nRun and check: does the background turn GREEN?")
