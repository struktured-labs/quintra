#!/usr/bin/env python3
"""
v2.00: Fixed buffer initialization bug!

BUG FOUND: The fill loop was clobbering A with the loop counter,
causing us to write garbage (0xFF) instead of 0. This set flip bits!

FIX: Use nested loops that preserve A = 0.
"""
import sys
import yaml
...
def create_gdma_bg_colorizer_fixed() -> bytes:
    """GDMA copy with FIXED buffer initialization (no A clobber bug)."""
    ATTR_BUFFER = 0xD000
    TILEMAP_VRAM = 0x9800
    BLOCKS = 64
    HRAM_INIT = 0xC7
    ...
    init_skip = len(code)
    code.extend([0x20, 0x00])             # JR NZ, skip_init (placeholder)

    # === FIXED: Initialize buffer using nested loops ===
    # Outer loop: B = 4 (4 * 256 = 1024 bytes)
    # Inner loop: C = 0 (256 iterations, C wraps 0->255->0)
    code.extend([0x21, 0x00, 0xD0])       # LD HL, 0xD000
    code.extend([0x06, 0x04])             # LD B, 4 (outer counter)
    code.extend([0xAF])                   # XOR A (A = 0, value to write)

    # outer_loop:
    outer_loop = len(code)
    code.extend([0x0E, 0x00])             # LD C, 0 (256 iterations)

    # inner_loop:
    inner_loop = len(code)
    code.append(0x22)                     # LD [HL+], A (write A, inc HL) - opcode 0x22
    code.append(0x0D)                     # DEC C
    inner_offset = inner_loop - len(code) - 2
    code.extend([0x20, inner_offset & 0xFF])  # JR NZ, inner_loop

    code.append(0x05)                     # DEC B
    outer_offset = outer_loop - len(code) - 2
    ...
    code[init_skip + 1] = (skip_init_target - init_skip - 2) & 0xFF
    ...
    # Start GDMA (64 blocks = 1024 bytes)
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v200.gb")
    ...
    print("\n=== v2.00: Fixed buffer init bug (A clobber in loop) ===")
    print("  Uses nested loops with LD [HL+], A to preserve A = 0")
    print()
    ...
    gdma_colorizer = create_gdma_bg_colorizer_fixed()
    combined = create_combined_func(palette_loader_addr, shadow_main_addr, gdma_colorizer_addr)
