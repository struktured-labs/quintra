#!/usr/bin/env bash
# Regression: on deterministic entropy run three, a Skeleton can share
...
# requires the real room-31 lane to advance through the seventh boss.
set -euo pipefail
...
QUINTRA_BALANCE_RUNS=3 QUINTRA_BALANCE_CLASSES=1 \
  QUINTRA_BALANCE_REPS=1 QUINTRA_BALANCE_FRAMES=60000 \
