#!/usr/bin/env python3
"""
v2.05: Debug VRAM Access During VBlank

Test if VRAM is actually accessible during our VBlank hook.
Uses STAT register check before VRAM write.
"""
import sys
import yaml
...
def create_bg_attr_test_with_vblank_wait() -> bytes:
    """
    Test VRAM write WITH explicit VBlank wait.
    Wait for mode 1 (VBlank) before writing to VRAM bank 1.
    """
    code = bytearray()
    ...
    code.extend([0xFE, 0x01])              # CP 1 (mode 1 = VBlank)
    offset = wait_vblank - len(code) - 2
    code.extend([0x20, offset & 0xFF])     # JR NZ, wait_vblank

    # Now in VBlank - switch to VRAM bank 1
    code.extend([0x3E, 0x01])              # LD A, 1
    code.extend([0xE0, 0x4F])              # LDH [VBK], A

    # Write a simple pattern to first 32 bytes (first row)
    # Just to test if VRAM writes work
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v205.gb")
    ...
    print("\n=== v2.05: VRAM Write with VBlank Wait ===")
    print("  Explicit VBlank wait before VRAM bank 1 write")
    print("  First row of background should be GOLD if working")
    print()
    ...
    bg_attr_test = create_bg_attr_test_with_vblank_wait()
