...
def main():
    ...
    # Enter the town through the ordinary room transition, with no debug
    # reveal written to run state. Room 19 is the first regional arrival.
    pb.memory[run_state + 1] = 18
    clear_hostiles()
    place(72, 0)
    tick(45)
    assert pb.memory[run_state + 1] == 19 and pb.memory[run_state + 19] == 0
    ...
    place(72, 0)
    tick(45)
    assert pb.memory[run_state + 1] == 20, "town north gate did not enter dungeon"
    # The entrance itself is also marked by normal fog-of-war; room 20 is
    # compass cell 2 on this deterministic setup. Require the Chartwright's
    # two promised bits without rejecting that ordinary discovery mark.
