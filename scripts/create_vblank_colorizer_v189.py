#!/usr/bin/env python3
"""
v1.89: VRAM write test - write to bank 1

- Full loop: read tiles, lookup palette, write to VRAM bank 1
- Tests if VRAM writing to bank 1 causes Sara corruption
"""
import sys
import yaml
...
def create_bg_row_colorizer_lazy(lookup_table_addr: int) -> bytes:
    """VRAM write test - full loop with write to bank 1.
    ...
    code.append(0x77)                   # LD [HL], A  <-- THE WRITE
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v189.gb")
    ...
    print("\n=== v1.89: VRAM write test ===")
    print("  Full loop: read tile, lookup, WRITE to VRAM bank 1")
    print("  Tests if VRAM writing causes Sara corruption")
    print()
