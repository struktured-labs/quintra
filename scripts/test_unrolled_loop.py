#!/usr/bin/env python3
"""
Test: Fully unrolled palette loader - no JR instruction.
64 copies of: LD A, [HL+]; LDH [OCPD], A
"""
import sys
import yaml
...
def create_unrolled_loader(rom_addr: int) -> bytes:
    """Fully unrolled - no loop, just 64 reads and writes."""
    code = bytearray()

    # Point HL to ROM data
    code.extend([0x21, rom_addr & 0xFF, (rom_addr >> 8) & 0xFF])  # LD HL, rom_addr

    # Set OCPS to 0x80 (auto-increment)
    code.extend([0x3E, 0x80])  # LD A, 0x80
    code.extend([0xE0, 0x6A])  # LDH [OCPS], A

    # 64 unrolled iterations
    for _ in range(64):
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_UNROLLED.gb")
    palette_path = Path("palettes/penta_palettes_v097.yaml")

    print("=== Fully unrolled palette loader (no JR) ===")
    ...
    rom_data_addr = 0x6800
    palette_loader_addr = 0x6850
    colorizer_addr = 0x6A00  # After the larger unrolled loader
    shadow_main_addr = 0x6A20
    combined_addr = 0x6A50

    palette_loader = create_unrolled_loader(rom_data_addr)
    ...
    print(f"Palette loader: {len(palette_loader)} bytes (unrolled)")
