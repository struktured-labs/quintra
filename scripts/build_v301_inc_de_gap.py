#!/usr/bin/env python3
"""2 tile_loops with gap via INC DE × 8 (no JR NC, no ADD).

If PASS → the JR NC, +1 (carry-conditional jump in DI) was the freezing trigger.
If FREEZE → unwritten bank-2 gap addresses are the trigger regardless of how
            the gap is created.
"""
import sys
...
def create_inc_de_gap(bg_table_addr):
    ...
    # Gap: INC DE × 8 (no JR NC, just plain INCs)
    code.extend([0x13] * 8)                       # 8× INC DE
...
def build():
    ...
    rom[0x14D] = chk
    out = Path("rom/working/penta_dragon_dx_v301_inc_de_gap.gb")
    out.write_bytes(rom)
    print(f"Wrote {out}")
