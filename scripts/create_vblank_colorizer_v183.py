#!/usr/bin/env python3
"""
v1.83: BG Colorizer - row scanning with direct VBK switching

- 8 tiles per frame, direct VBK switch per tile
- Proper row scanning: different tiles each frame based on counter
- Fixes v1.82 flickering (was always colorizing same 8 tiles)
"""
import sys
import yaml
...
def create_bg_row_colorizer_lazy(lookup_table_addr: int) -> bytes:
    """BG colorizer - 8 tiles/frame with row scanning, direct VBK switching.

    Each frame colorizes 8 tiles at different positions:
    - step = counter - WARMUP (0-127)
    - row = step >> 2 (0-31, which row of tilemap)
    - chunk = step & 3 (0-3, which 8-tile chunk within row)
    - VRAM addr = 0x9800 + row * 32 + chunk * 8

    Full tilemap (32x32 = 1024 tiles) colorized in 128 frames.
    """
    HRAM_COUNTER = 0xC4
    WARMUP_FRAMES = 60
    TOTAL_STEPS = 128
    ...
    code.append(0x47)                   # LD B, A (save counter for later)

    # If counter < warmup, skip to end (just increment counter)
    ...
    # VRAM addr = 0x9800 + row * 32 + chunk_offset
    # row * 32: shift D left 5 times, but D only has 5 bits (0-31)
    # So: L = (row << 5) + chunk_offset, H = 0x98 + carry
    #
    # row * 32: for row 0-7, result fits in L (0-224)
    #           for row 8-31, we overflow into H
    #
    # Simpler approach: L = low byte, H = 0x98/0x99/0x9A/0x9B
    # row 0-7:   0x9800-0x98FF
    # row 8-15:  0x9900-0x99FF
    # row 16-23: 0x9A00-0x9AFF
    # row 24-31: 0x9B00-0x9BFF
    ...
    code.extend([0xCB, 0x27])           # SLA A (x32) - carries into C flag for rows >= 8
    code.append(0x83)                   # ADD E (add chunk_offset)
    code.append(0x6F)                   # LD L, A (L = low byte of offset)
    ...
    # D = loop counter (reuse D since we're done with row)
    ...
    code.append(0x7E)                   # LD A, [HL]
    # C is free now (we saved step but don't need it anymore)
    code.append(0x4F)                   # LD C, A (save tile ID, though not needed)
    ...
    # VBK = 0 (restore for next tile read)
    code.extend([0xAF])                 # XOR A
    code.extend([0xE0, 0x4F])           # LDH [VBK], A
    ...
    # end: increment counter and wrap
    end_label = len(code)
    code[skip_to_end + 1] = (end_label - skip_to_end - 2) & 0xFF

    code.append(0x78)                   # LD A, B (original counter)
    code.append(0x3C)                   # INC A
    wrap_val = WARMUP_FRAMES + TOTAL_STEPS  # 60 + 128 = 188
    code.extend([0xFE, wrap_val])       # CP 188
    code.extend([0x38, 0x02])           # JR C, +2 (no wrap needed)
    code.extend([0x3E, WARMUP_FRAMES])  # LD A, 60 (reset to warmup)
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v183.gb")
    ...
    print("\n=== v1.83: Row scanning with direct VBK switching ===")
    print("  8 tiles/frame, scans different rows each frame")
    print("  Full tilemap colorized in 128 frames")
    print()
