#!/usr/bin/env python3
"""
v2.08: v1.09 + Simple VBK Attribute Test

Combines:
- Full v1.09 working colorization (sprites + palettes)
- Simple VBK test: write palette 1 to ALL BG tiles

If VBK works, entire background should be GOLD (palette 1).
If VBK doesn't work, we should see normal v1.09 colors.
"""
import sys
import yaml
...
def create_simple_vbk_fill() -> bytes:
    """
    Simple test: fill ALL BG attributes with palette 1 (gold).
    """
    code = bytearray()
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v208.gb")
    ...
    print("\n=== v2.08: v1.09 + VBK Fill Test ===")
    print("  Full v1.09 sprite colorization")
    print("  Plus: fill ALL BG attributes with palette 1 (gold)")
    print("  Expected: GOLD background if VBK works")
    print()
