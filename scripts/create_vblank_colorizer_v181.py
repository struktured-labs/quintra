#!/usr/bin/env python3
"""
v1.81: BG Colorizer - full pipeline 1 tile test

- Tests complete pipeline: read tile -> lookup -> WRAM -> write attr
- But only for 1 tile (no loop)
"""
import sys
import yaml
...
def create_bg_row_colorizer_lazy(lookup_table_addr: int) -> bytes:
    """BG colorizer - test full pipeline but only 1 tile."""
    HRAM_COUNTER = 0xC4
    WRAM_BUFFER = 0xD900
    WARMUP_FRAMES = 60
    TOTAL_STEPS = 128
    ...
    # After warmup, do full pipeline for 1 tile
    ...
    # Set VBK=0 for reading tile
    code.extend([0xAF])                 # XOR A (A = 0)
    code.extend([0xE0, 0x4F])           # LDH [VBK], A

    # Read 1 tile from VRAM
    code.extend([0x21, 0x00, 0x98])     # LD HL, 0x9800
    code.append(0x7E)                   # LD A, [HL] - read tile ID

    # Lookup palette (using lookup table)
    code.append(0x6F)                   # LD L, A (tile ID)
    code.extend([0x26, (lookup_table_addr >> 8) & 0xFF])  # LD H, high byte
    code.append(0x7E)                   # LD A, [HL] - read palette

    # Store to WRAM buffer
    code.extend([0x21, WRAM_BUFFER & 0xFF, (WRAM_BUFFER >> 8) & 0xFF])
    code.append(0x77)                   # LD [HL], A

    # Switch to VBK=1 for writing attribute
    code.extend([0x3E, 0x01])           # LD A, 1
    code.extend([0xE0, 0x4F])           # LDH [VBK], A

    # Write 1 attribute to VRAM
    # Load from WRAM buffer
    code.extend([0x21, WRAM_BUFFER & 0xFF, (WRAM_BUFFER >> 8) & 0xFF])
    code.append(0x7E)                   # LD A, [HL]
    ...
    # Restore VBK
    code.append(0x79)                   # LD A, C (restore VBK)
    code.extend([0xE0, 0x4F])           # LDH [VBK], A
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v181.gb")
    ...
    print("\n=== v1.81: Full pipeline - 1 tile ===")
    print("  Read -> Lookup -> WRAM -> Write (no loop)")
    print()
