#!/usr/bin/env python3
"""
v1.06: Miniboss detection - all enemies use palette 7 when miniboss present

Two-pass approach:
1. Scan all sprites for tiles in 0x70-0x7F range (miniboss)
2. If miniboss found, ALL non-Sara sprites get palette 7

This fixes the alternating colors on miniboss (which uses tiles from multiple ranges).
"""
import sys
import yaml
...
def create_tile_based_colorizer() -> bytes:
    """
    Tile-based colorizer with miniboss override.

    Input: HL = pointer to flags byte, D = Sara palette, E = miniboss flag (0=normal, 7=miniboss mode)

    Logic:
    - Slots 0-3: Sara (palette D)
    - Tile < 0x10: Projectile (palette 0)
    - If E=7 (miniboss mode): ALL other sprites get palette 7
    ...
    # Check miniboss mode (E register)
    code.append(0x7B)                # LD A, E
    code.append(0xB7)                # OR A (set flags)
    jumps_to_fix.append((len(code), 'miniboss_override'))
    code.extend([0x20, 0x00])        # JR NZ, miniboss_override (E != 0)

    # Normal tile-based coloring
    ...
    # miniboss_override: E != 0, use palette 7 for all non-Sara/non-projectile
    labels['miniboss_override'] = len(code)
...
def create_shadow_colorizer_main(colorizer_addr: int) -> bytes:
    """Colorizes BOTH shadow buffers with miniboss detection.

    First scans for miniboss tiles (0x70-0x7F), then colorizes both buffers.
    E = 7 if miniboss detected, E = 0 otherwise.
    """
    code = bytearray()
    ...
    # === MINIBOSS DETECTION via ENTITY DATA ===
    # Check entity array at 0xC200+ for entity type 0x1D (miniboss)
    # Entity structure: 24 bytes each, type at offset 3
    # E = 0 (no miniboss), E = 7 (miniboss detected)
    code.extend([0x1E, 0x00])        # LD E, 0 (assume no miniboss)
    code.extend([0x21, 0x03, 0xC2])  # LD HL, 0xC203 (entity 0, type offset)
    code.extend([0x06, 0x0A])        # LD B, 10 (10 entities)

    # scan_loop:
    scan_loop_pos = len(code)
    code.append(0x7E)                # LD A, [HL] (entity type)
    code.extend([0xFE, 0x1D])        # CP 0x1D (miniboss type)
    code.extend([0x20, 0x02])        # JR NZ, +2 (not miniboss, skip)
    code.extend([0x1E, 0x07])        # LD E, 7 (miniboss mode!)
    # skip: advance to next entity (HL += 24)
    # Manual add to preserve B (loop counter) and E (miniboss flag)
    code.append(0x7D)                # LD A, L
    code.extend([0xC6, 0x18])        # ADD A, 24
    code.append(0x6F)                # LD L, A
    code.extend([0x30, 0x01])        # JR NC, +1 (no carry)
    code.append(0x24)                # INC H
    code.append(0x05)                # DEC B
    loop_offset = scan_loop_pos - len(code) - 2
    code.extend([0x20, loop_offset & 0xFF])  # JR NZ, scan_loop
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v106.gb")
    ...
    print("\n=== v1.06: Miniboss Detection Fix ===")
    print("  Two-pass: scan for miniboss tiles, then colorize")
    print("  If ANY sprite has tile 0x70-0x7F → ALL enemies get palette 7")
    print("  Normal mode: tile-based coloring per monster type")
