#!/usr/bin/env python3
"""
v1.78: BG Colorizer - VRAM READ only test

- Tests if VRAM reading causes freeze
- No VBK switch, no write
"""
import sys
import yaml
...
def create_bg_row_colorizer_lazy(lookup_table_addr: int) -> bytes:
    """BG colorizer - test VRAM READ only (no VBK switch, no write)."""
    HRAM_COUNTER = 0xC4
    WARMUP_FRAMES = 60
    TOTAL_STEPS = 128
    TILES_PER_FRAME = 8
    ...
    # After warmup, do VRAM read
    code.extend([0xFE, WARMUP_FRAMES])
    skip_to_end = len(code)
    code.extend([0x38, 0x00])           # JR C, end (placeholder)

    # Read 8 tiles from VRAM (no VBK switch needed for read, just test access)
    code.extend([0x21, 0x00, 0x98])     # LD HL, 0x9800
    code.extend([0x06, TILES_PER_FRAME])  # LD B, 8
    read_loop = len(code)
    code.append(0x7E)                   # LD A, [HL] - READ from VRAM
    code.append(0x23)                   # INC HL
    code.append(0x05)                   # DEC B
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v178.gb")
    ...
    print("\n=== v1.78: BG Colorizer - VRAM READ only ===")
    print("  No VBK switch, no write")
    print()
