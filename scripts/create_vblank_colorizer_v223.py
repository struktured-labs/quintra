...
def create_combined_with_dma(palette_loader_addr: int, obj_main_addr: int, bg_colorizer_addr: int) -> bytes:
    """Combined function: load palettes, colorize OBJ, colorize BG (one-shot), run DMA."""
    code = bytearray()
...
def main():
    ...
    output_rom = project_root / "rom/working/penta_dragon_dx_v223.gb"
    ...
    print("\n=== v2.23: Fix Input Handler - Proper Timing ===")
    print("  CRITICAL FIX: Input handler now matches original:")
    print("    - 8 delay reads for d-pad (was 2)")
    print("    - Returns input value in A register")
    print("  Plus: Palette loader runs only once (not every frame)")
    print("  OBJ: Tile-based colorization | BG: One-shot colorization")
    print()
