#!/usr/bin/env python3
"""
v2.48: Fast BG Colorizer (ROM Table + 192 Tiles + Active Tilemap)

Key improvements:
- ROM LOOKUP TABLE: 256-byte table at 0x6E00 replaces CALL subroutine.
  Table read = 7M vs CALL chain = 25M → 35% faster per tile.
- 192 TILES/FRAME: Affordable due to table speedup. Full tilemap
  coverage in ~5.3 sweeps. With mode 3 filter, ~8 frames = 0.13s.
- SINGLE TILEMAP: Only processes the LCDC-bit-3-active tilemap.
  All writes go to the visible map.
- INTERLEAVED read→write: write immediately after read (~30M gap).
- 0xFF filter: skip writes from garbage VRAM reads during mode 3.
- FFA9 stores tilemap base for dynamic wrap check.
...
def create_bg_tile_table() -> bytes:
    ...
    Same classification as the tile_to_palette subroutine.
    """
    table = bytearray(256)
...
def create_bg_colorizer(bg_table_addr: int) -> bytes:
    """BG colorizer: active tilemap + ROM table lookup + 192 tiles.

    Uses a 256-byte ROM lookup table instead of CALL subroutine.
    Table read = 7M vs CALL chain = 25M → 35% faster per tile.
    192 tiles/frame at ~38M/tile = ~7,296M (42% of frame).
    Full tilemap coverage in ~5.3 frames = 0.09s (before mode 3 filter).

    Only processes the LCDC-bit-3-active tilemap.
    HRAM: FF91=counter low, FFA5=counter high, FFA9=tilemap base.
    """
    TILES_PER_VBLANK = 128
    ...
    # Set H = table high byte (persists through loop, L changes per tile)
    ...
    code.extend([0xE6, 0x03])        # AND 0x03  (works for both bases)
...
def main():
    ...
    print("=== Penta Dragon DX v2.48 ===")
    print("Fast BG Colorizer (ROM Table + 192 Tiles + Active Tilemap)")
    print()
    print("KEY CHANGES:")
    print("  1. ROM lookup table: 256-byte table replaces CALL subroutine")
    print("     Table read = 7M vs CALL chain = 25M -> 35% faster per tile")
    print("  2. 192 tiles/frame: full tilemap in ~5.3 sweeps (~0.13s ideal)")
    print("  3. Single active tilemap: only processes displayed map (LCDC bit 3)")
    print("  4. Interleaved read->write with 0xFF garbage filter")
    print()
