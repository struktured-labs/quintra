...
local function normalize_compact_source()
    -- Synthetic post-boss routing rewrites the logical source room before
    -- crossing its real threshold. A scrolling stage entry can otherwise
    -- lend that compact defeated-boss fixture 248x248 bounds, moving its
    -- south exit beyond the authored row-16 door. Restore the source role;
    -- the destination is still generated by the cartridge transition.
    emu:write8(LR, 0)
    emu:write8(WW, 160)
    emu:write8(WH, 136)
    emu:write8(0xFF43, 0)
...
local function settle(normalize_scroll)
    ...
    error(string.format(
        "room never reached a save-safe display state"
            .. " room=%d screen=%d lcd=%02X scroll=%d,%d"
            .. " normalize=%s large=%d world=%dx%d bad_tile=%d",
        emu:read8(RS + 1), emu:read8(LS), emu:read8(0xFF40),
        emu:read8(0xFF43), emu:read8(0xFF42),
        tostring(normalize_scroll), emu:read8(LR),
        emu:read8(WW), emu:read8(WH), last_bad))
end
...
local function enter_dungeon(target, stage, qualified, normalize_scroll)
    ...
    if normalize_scroll == nil then
        ...
        -- transaction, just like a court or moving Colossus arena. Decide
        -- from the destination role because the public room counter commits
        -- one beat before wide-room metadata during the transition.
        local local_room = target - STAGE_START[stage + 1]
        normalize_scroll = local_room ~= 0
            and target ~= STAGE_BOSS[stage + 1]
    end
