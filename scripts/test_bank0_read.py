#!/usr/bin/env python3
"""
Test: Read palette data from bank 0 (always mapped) instead of bank 13.
Put data near 0x0200 where there's usually free space.
"""
import sys
import yaml
...
def create_bank0_loader(bank0_addr: int) -> bytes:
    """Read from bank 0 address."""
    code = bytearray()

    # Point HL to bank 0 data
    code.extend([0x21, bank0_addr & 0xFF, (bank0_addr >> 8) & 0xFF])
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_BANK0.gb")
    palette_path = Path("palettes/penta_palettes_v097.yaml")

    print("=== Read palette from bank 0 (always accessible) ===")
    ...
    # Put palette data in bank 0 at 0x0200
    bank0_data_addr = 0x0200
    rom[bank0_data_addr:bank0_data_addr + len(obj_data)] = obj_data
    print(f"Put palette data at bank 0 address 0x{bank0_data_addr:04X}")

    # Code stays in bank 13
    palette_loader_addr = 0x6850
    colorizer_addr = 0x6880
    shadow_main_addr = 0x68A0
    combined_addr = 0x68D0

    palette_loader = create_bank0_loader(bank0_data_addr)
