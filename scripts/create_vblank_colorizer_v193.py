#!/usr/bin/env python3
"""
v1.93: BG colorizer called FIRST (before sprites)

- Call order: BG colorizer -> palettes -> sprites -> DMA
- Tests if order affects Sara corruption
- 4 tiles/frame retained
"""
import sys
import yaml
...
def create_bg_row_colorizer_lazy(lookup_table_addr: int) -> bytes:
    """v1.92: Reduced to 4 tiles per frame to reduce VBlank time.

    Tests if fewer tiles fixes Sara corruption.
    """
    HRAM_COUNTER = 0xC4
    WARMUP_FRAMES = 60
    WRAP_VALUE = 188  # Same wrap, just process fewer tiles per step
    TILES_PER_FRAME = 4  # Reduced from 8 to 4
    code = bytearray()
    ...
    code.extend([0xCB, 0x27])           # SLA A (x8) -> A = 0/8/16/24
    ...
    # just_increment: increment counter with wrap
    ...
    code.extend([0xFE, WRAP_VALUE])     # CP 188
    code.extend([0x38, 0x02])           # JR C, +2 (no wrap needed)
    code.extend([0x3E, WARMUP_FRAMES])  # LD A, 60 (wrap)
...
def create_combined_with_dma_and_bg(palette_loader_addr: int, shadow_main_addr: int, bg_colorizer_addr: int) -> bytes:
    """Combined: BG colorize FIRST, then palettes, sprite colorize, DMA.

    v1.93: Changed order - BG first to isolate from sprite processing.
    """
    code = bytearray()
    # BG colorizer FIRST (isolated from sprite processing)
    code.extend([0xCD, bg_colorizer_addr & 0xFF, bg_colorizer_addr >> 8])
    # Ensure VBK=0 after BG colorizer
    code.extend([0xAF])                 # XOR A
    code.extend([0xE0, 0x4F])           # LDH [VBK], A
    # Then normal sprite processing
    code.extend([0xCD, palette_loader_addr & 0xFF, palette_loader_addr >> 8])
    code.extend([0xCD, shadow_main_addr & 0xFF, shadow_main_addr >> 8])
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v193.gb")
    ...
    print("\n=== v1.93: BG colorizer FIRST ===")
    print("  Order: BG -> VBK=0 -> palettes -> sprites -> DMA")
    print("  Tests if call order affects Sara corruption")
    print()
