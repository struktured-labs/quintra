#!/usr/bin/env python3
"""
v1.41: HDMA BG Colorization - Fixed Item Tile Range

Based on VRAM analysis of actual item tiles (extra life = 0xAE, 0xAF, 0xBE, 0xBF):
...
- 0x80-0x9F: Hazards/spikes (palette 3 - green)  [FIXED: was 0x80-0xBF]
- 0xA0-0xDF: Items/pickups (palette 1 - GOLD!)   [FIXED: was 0xC0-0xDF]
- 0xE0-0xFF: Wall edges/borders (palette 2)

Items ARE BG tiles, NOT sprites. Confirmed via OAM dump and VRAM read.
"""
import sys
import yaml
...
def create_tile_palette_lookup() -> bytes:
    """
    Create 256-byte lookup table with CORRECTED item tile range.

    Based on actual Level 1 tile usage + VRAM analysis:
    - Floor: 0x00-0x12 (basic checkerboard, transitions)
    - Wall borders: 0x13-0x1F
    - Wall corners: 0x20-0x29
    - Floor chains/deco: 0x2A-0x3F -> palette 0
    - Extended walls: 0x40-0x7F -> palette 2
    - Hazards: 0x80-0x9F -> palette 3 (FIXED: was 0x80-0xBF)
    - Items: 0xA0-0xDF -> palette 1 (GOLD!) (FIXED: was 0xC0-0xDF)
    - Wall edges: 0xE0-0xFF -> palette 2

    EVIDENCE: Extra life item uses tiles 0xAE, 0xAF, 0xBE, 0xBF
    which are in range 0xA0-0xBF, NOT 0xC0-0xDF!
    """
    ...
    for i in range(256):
        if i <= 0x0F:
            ...
            # Hazards (spikes, etc) - ONLY 0x80-0x9F now!
            lookup[i] = 3
        elif i <= 0xDF:
            # Items - GOLD! Now includes 0xA0-0xDF (extra life, powerups, etc)
            lookup[i] = 1
        else:
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v141.gb")
    ...
    print("\n=== v1.41: HDMA with FIXED Item Tile Range ===")
    print("  FIX: Items now 0xA0-0xDF (was 0xC0-0xDF)")
    print("  FIX: Hazards now 0x80-0x9F (was 0x80-0xBF)")
    print("  Extra life tiles 0xAE,0xAF,0xBE,0xBF now get GOLD palette!")
    print()
