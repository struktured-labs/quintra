#!/usr/bin/env python3
"""
v2.62: STAT-safe BG colorizer for flicker-free scrolling on MiSTer FPGA

Building on v2.61's single-tilemap + conditional palette + optimized OBJ,
this version adds STAT mode checking before each VRAM access to eliminate
the scroll flicker caused by PPU mode 3 write drops on cycle-accurate hardware.

Root cause of scroll flicker: On MiSTer/real CGB hardware, VRAM reads during
PPU mode 3 return 0xFF and writes are silently dropped. Without STAT waits,
~50% of BG colorizer operations fail. During scrolling, tiles shift but
attributes become stale → visible flicker.

Fix: Wait for LCD mode 0 (HBlank) or 1 (VBlank) before each tile access.
STAT bit 1 = 0 guarantees safe VRAM window. Single STAT wait covers both
the read and write since mode 0 → mode 2 transition takes ≥87T and our
read→write sequence is ≤72T. Mode 2 is also VRAM-safe on CGB.

Since STAT waits guarantee valid reads, the 0xFF anti-flicker filter is
removed (all tiles including 0xFF now map to valid palettes).

Key features:
- STAT-safe VRAM access (AND 0x02 on FF41 before each tile)
- Single active tilemap via LCDC bit 3 detection
- No 0xFF filter needed (reads always valid)
- 89 tiles/frame (prime, coprime with 1024, 0.19s settling)
...
def create_bg_tile_table() -> bytes:
    ...
    With STAT-safe reads, all tiles map to valid palettes (no 0xFF skip marker).
...
def create_bg_colorizer(bg_table_addr: int) -> bytes:
    """BG colorizer: STAT-safe single active tilemap for MiSTer FPGA.

    Waits for LCD mode 0/1 (STAT bit 1 = 0) before each tile access.
    This guarantees VRAM reads return real data and writes land, eliminating
    the scroll flicker caused by mode 3 drops on cycle-accurate hardware.

    Timing safety proof: After STAT check passes (mode 0 or 1), the read→write
    sequence takes ≤72T. Mode 0 (HBlank) lasts ≥87T, and mode 2 (next mode)
    is also VRAM-safe on CGB. So even if we spill into mode 2, writes succeed.

    Safe HRAM: FF91=counter_lo, FFA5=counter_hi
    """
    TILES_PER_VBLANK = 71  # prime, coprime with 1024
    # 71 STAT-safe tiles > 55 effective tiles in v2.61 (109 × 50% success on MiSTer)
    # Settling: 1024/71 = 14.4 frames = 0.24s (FASTER than v2.61's 0.31s on MiSTer)
    # Budget: 71 × 229T = 16,259T BG + 8,500T OBJ = ~24,759T = 35.3% of frame
    ...
    # STAT WAIT: wait for mode 0 (HBlank) or mode 1 (VBlank)
    # Mode 0: STAT & 0x02 = 0 → pass
    # Mode 1: STAT & 0x02 = 0 → pass
    # Mode 2: STAT & 0x02 = 2 → wait (OAM scan, blocks OAM but not VRAM - wait anyway for mode 3 race safety)
    # Mode 3: STAT & 0x02 = 2 → wait (drawing, blocks VRAM)
    labels['stat_wait'] = len(code)
    emit([0xF0, 0x41])        # LDH A, [FF41]      ; 12T
    emit([0xE6, 0x02])        # AND 0x02            ; 8T
    stat_jr_pos = len(code)
    emit([0x20, 0x00])        # JR NZ, stat_wait    ; 12T taken / 8T not
    # Patch backward jump
    code[stat_jr_pos + 1] = (labels['stat_wait'] - (stat_jr_pos + 2)) & 0xFF

    # Read tile (VBK=0) - guaranteed safe after STAT wait
    emit([0x1A])              # LD A, [DE]          ; 8T  VRAM read
    emit([0x6F])              # LD L, A             ; 4T  lookup index
    emit([0x7E])              # LD A, [HL]          ; 8T  ROM table → palette

    # Write attr (VBK=1) - guaranteed safe (≤72T from STAT check, within mode 0/2 window)
...
def main():
    ...
    print("=== Penta Dragon DX v2.62 ===")
    print("STAT-safe BG colorizer + single tilemap + conditional palette + optimized OBJ")
    print()
