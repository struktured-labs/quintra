#!/usr/bin/env python3
"""
v1.70: BG Colorizer - 8 tiles per frame

Only processes 8 tiles per frame (was 32) to stay within VBlank.
Takes 128 frames to fully colorize (vs 32 frames before).
"""
import sys
import yaml
...
def create_bg_row_colorizer_lazy(lookup_table_addr: int) -> bytes:
    """BG colorizer - only 8 tiles per frame to stay within VBlank."""
    WRAM_BUFFER = 0xDFF0
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v170.gb")
    ...
    print("\n=== v1.70: BG Colorizer (8 tiles/frame) ===")
    print("  Only 8 tiles per frame to fit in VBlank")
    print()
