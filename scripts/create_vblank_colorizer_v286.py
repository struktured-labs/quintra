#!/usr/bin/env python3
"""
v2.86: Audio Fix — Palette Caching to Reduce VBlank Time
...
Fix: Restore hash-based palette caching using FFA6 (separate from FFA9
...
def create_conditional_palette_cached(palette_loader_addr: int) -> bytes:
    """Hash-cached palette wrapper using FFA6 (not FFA9, which is scroll state).

    v2.86: Computes hash from FFBE (form) ^ FFBF (boss) ^ FFC0 (powerup) ^
    FFD0 (tilemap hi) + 1. Compares with FFA6. If unchanged, skips palette
    loading entirely → saves ~550 M-cycles per VBlank frame.

    Palettes only need reloading when room/form/boss/tilemap transitions occur.
    """
    code = bytearray()
    ...
    code.extend([
        ...
        0xF0, 0xA6,  # LDH A, [FFA6]  ; previous hash
        0xB8,        # CP B            ; same?
        0xC8,        # RET Z           ; skip if unchanged
        0x78,        # LD A, B         ; new hash
        0xE0, 0xA6,  # LDH [FFA6], A  ; save
...
def build_v286():
    ...
    output_path = Path("rom/working/penta_dragon_dx_v286.gb")
    ...
    print(f"Cond palette (hash-cached FFA6): {len(cond_pal)} bytes at 0x{cond_pal_addr:04X}")
