#!/usr/bin/env python3
"""
v2.09: v1.09 + Fixed VBK Fill Bug

The v2.08 VBK fill loop had a bug where A was clobbered by the BC zero check.
Fixed by using register E to hold the fill value.

Combines:
- Full v1.09 working colorization (sprites + palettes)
- FIXED VBK fill: write palette 1 to ALL BG tiles
"""
import sys
import yaml
...
def create_simple_vbk_fill() -> bytes:
    """
    FIXED: Fill ALL BG attributes with palette 1 (gold).

    Bug in v2.08: The BC zero check clobbered A.
    Fix: Use register E for the fill value, A is free for the check.
    """
    code = bytearray()
    ...
    code.extend([0x1E, 0x01])              # LD E, 1 (palette 1 - preserved across loop!)

    # fill_loop:
    fill_loop = len(code)
    code.append(0x73)                      # LD [HL], E  (write E, not A!)
    code.append(0x23)                      # INC HL
    code.append(0x0B)                      # DEC BC
    code.append(0x78)                      # LD A, B
    code.append(0xB1)                      # OR C
    code.extend([0x20, (fill_loop - len(code) - 2) & 0xFF])  # JR NZ
...
def create_combined_with_dma(
    ...
    """v1.09 combined + VBK fill test."""
    code = bytearray()
    code.extend([0xCD, palette_loader_addr & 0xFF, palette_loader_addr >> 8])
    code.extend([0xCD, shadow_main_addr & 0xFF, shadow_main_addr >> 8])
    code.extend([0xCD, vbk_fill_addr & 0xFF, vbk_fill_addr >> 8])
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v209.gb")
    ...
    print("\n=== v2.09: v1.09 + FIXED VBK Fill ===")
    print("  BUG FIX: v2.08 loop clobbered A register")
    print("  Now uses E register for fill value")
    print("  Expected: GOLD background (all palette 1)")
    print()
    ...
    vbk_fill = create_simple_vbk_fill()
    combined = create_combined_with_dma(palette_loader_addr, shadow_main_addr, vbk_fill_addr)
    vblank_hook = create_vblank_hook_with_input(combined_addr)

    print(f"VBK fill: {len(vbk_fill)} bytes at 0x{vbk_fill_addr:04X}")
    ...
    rom[bank_offset(vbk_fill_addr):bank_offset(vbk_fill_addr) + len(vbk_fill)] = vbk_fill
