#!/usr/bin/env python3
"""
v1.86: BG Colorizer DISABLED - diagnostic version

- BG colorizer is just RET (no-op) to confirm it causes Sara corruption
- If Sara renders correctly, the issue is in BG colorizer code
- If Sara still corrupted, issue is elsewhere
"""
import sys
import yaml
...
def create_bg_row_colorizer_lazy(lookup_table_addr: int) -> bytes:
    """BG colorizer - DISABLED for diagnostic.

    Just returns immediately to confirm BG colorizer causes Sara corruption.
    """
    # Just RET - do nothing
    return bytes([0xC9])
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v186.gb")
    ...
    print("\n=== v1.86: BG Colorizer DISABLED (diagnostic) ===")
    print("  BG colorizer is just RET - no BG colorization")
    print("  If Sara renders correctly, BG colorizer causes corruption")
    print()
