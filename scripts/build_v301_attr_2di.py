#!/usr/bin/env python3
"""v3.01 attr_comp: 2 rows in 2 SEPARATE DI windows (EI gap between).

If this PASSES → one long DI window for 2 rows is the issue (~4000T was too much).
If this FREEZES → the issue isn't the DI window length, it's something else
about doing 2 rows of writes.
"""
import sys
...
def create_attr_2di(bg_table_addr):
    """2 rows of 24 tiles each, in 2 separate DI/EI windows."""
...
def build():
    ...
    w(A['cz'], bytes(code))
    print(f"  attr 2di: {len(create_attr_2di(A['bt']))} bytes")
    ...
    rom[0x14D] = chk
    out = Path("rom/working/penta_dragon_dx_v301_attr_2di.gb")
    out.write_bytes(rom)
    print(f"Wrote {out}")
