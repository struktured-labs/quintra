#!/usr/bin/env python3
"""
EXACT copy of test_overwrite.py but using YAML palette data.
If this works, we know the issue is with grayscale init.
"""
import sys
import yaml
...
def load_palette4_from_yaml(yaml_path: Path) -> bytes:
    ...
    return pal_to_bytes(["0000", "03FF", "00DF", "0000"])


# EXACT copy from test_overwrite.py
def create_overwrite_loader(data_addr: int) -> bytes:
    code = bytearray()

    # BG palettes - zeros (EXACT copy from OVERWRITE)
    ...
    # OBJ palettes - BLUE (EXACT copy from OVERWRITE)
    code.extend([0x3E, 0x80])
    ...
    # Loop overwrite of palette 4 (EXACT same loop structure)
    code.extend([0x21, data_addr & 0xFF, (data_addr >> 8) & 0xFF])
    code.extend([0x3E, 0xA0])  # OCPS = 0xA0 (palette 4)
    code.extend([0xE0, 0x6A])
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_OVERWRITE_YAML.gb")
    palette_path = Path("palettes/penta_palettes_v097.yaml")

    print("=== EXACT OVERWRITE clone with YAML data ===")

    pal4_data = load_palette4_from_yaml(palette_path)
    print(f"Palette 4 (Hornets) from YAML: {pal4_data.hex()}")

    rom = bytearray(input_rom.read_bytes())
    apply_all_display_patches(rom)

    # Put palette data at 0x0200 (same as OVERWRITE)
    data_addr = 0x0200
    rom[data_addr:data_addr + len(pal4_data)] = pal4_data

    # Use SAME addresses as OVERWRITE
    palette_loader_addr = 0x6800
    colorizer_addr = 0x6C00
    colorizer_main_addr = 0x6C20
    combined_addr = 0x6C50

    palette_loader = create_overwrite_loader(data_addr)
