#!/usr/bin/env python3
"""
v1.77: BG Colorizer - LCD + counter only (no VRAM)

- Tests if counter logic alone causes freeze
- No VRAM reads or writes
"""
import sys
import yaml
...
def create_bg_row_colorizer_lazy(lookup_table_addr: int) -> bytes:
    """BG colorizer - minimal: LCD check + counter only (no VRAM)."""
    HRAM_COUNTER = 0xC4
    WARMUP_FRAMES = 60
    TOTAL_STEPS = 128
    code = bytearray()
    ...
    # Skip VRAM operations, just increment counter
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v177.gb")
    ...
    print("\n=== v1.77: BG Colorizer - LCD + counter only ===")
    print("  No VRAM operations")
    print()
