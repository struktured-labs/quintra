...
LUNGE_PANIC_RANGES="${QUINTRA_POLICY_LUNGE_PANIC_RANGES:-0}"
...
mkdir -p "$OUTDIR"
printf 'class,policy,lunge_panic_range,runs,bosses,wins,max_room,min_hp,combat_stalls,route_stalls\n'

for class in $CLASSES; do
  for policy in $POLICIES; do
    for lunge_panic_range in $LUNGE_PANIC_RANGES; do
    out="$OUTDIR/class-$class-$policy-lunge-$lunge_panic_range.csv"
    ...
    save_dir="$(mktemp -d "$OUTDIR/.save-$class-$policy-lunge-$lunge_panic_range.XXXXXX")"
