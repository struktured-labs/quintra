#!/usr/bin/env python3
"""
v2.04: BG Colorization with Manual VRAM Copy (Debug GDMA)

Same as v2.03 but uses manual loop instead of GDMA to copy attributes to VRAM.
This helps debug whether GDMA is the issue.
"""
import sys
import yaml
...
def create_simple_bg_attr_test() -> bytes:
    """
    SIMPLE TEST: Just set all visible BG tiles to palette 1 (gold).
    This tests if manual VRAM writes to VBK 1 work at all.

    If this works and we see gold background, VRAM bank switching works.
    If not, there's a fundamental issue with our VRAM access.
    """
    code = bytearray()
    ...
    # Fill 0x9800-0x9BFF (1024 bytes) with palette 1
    # This covers the entire 32x32 tilemap
    code.extend([0x21, 0x00, 0x98])        # LD HL, 0x9800
    code.extend([0x3E, 0x01])              # LD A, 1 (palette 1 = gold)
    ...
    code.extend([0x3E, 0x01])              # LD A, 1 (reload palette)
...
def create_combined_with_dma(
    ...
    bg_attr_test_addr: int
) -> bytes:
    """Combined function: load palettes, colorize sprites, test BG attrs, run sprite DMA."""
    code = bytearray()
    code.extend([0xCD, palette_loader_addr & 0xFF, palette_loader_addr >> 8])
    code.extend([0xCD, shadow_main_addr & 0xFF, shadow_main_addr >> 8])
    code.extend([0xCD, bg_attr_test_addr & 0xFF, bg_attr_test_addr >> 8])
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v204.gb")
    ...
    print("\n=== v2.04: Simple BG Attr Test (All Gold) ===")
    print("  Testing manual VRAM bank 1 writes")
    print("  ALL background tiles should appear GOLD if working")
    print()
    ...
    bg_attr_test = create_simple_bg_attr_test()
    combined = create_combined_with_dma(palette_loader_addr, shadow_main_addr, bg_attr_test_addr)
    ...
    print(f"BG attr test: {len(bg_attr_test)} bytes at 0x{bg_attr_test_addr:04X}")
    ...
    rom[bank_offset(bg_attr_test_addr):bank_offset(bg_attr_test_addr) + len(bg_attr_test)] = bg_attr_test
    ...
    print("\n=== v2.04 Build Complete ===")
    print("\nExpected result: ENTIRE BACKGROUND should be GOLD (palette 1)")
