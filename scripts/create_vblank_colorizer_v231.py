#!/usr/bin/env python3
"""
v2.31: Fixed jump offsets in palette loader (fixes v2.30 flickering)

CRITICAL FIX:
- v2.29/v2.30 had wrong jump offsets in dynamic Palette 0 loading
- JR NZ, +3 should be JR NZ, +5 (skip 3-byte LD HL + 2-byte JR)
- JR +2 should be JR +3 (skip 3-byte LD HL)
- This caused random code execution and palette flickering

APPROACH (same as v2.30):
- ALL projectiles (0x00-0x0F) use Palette 0 (dynamic)
- Palette 0 changes based on Sara form (pink for W, green for D)
- No sub-range detection (avoids direction-dependent colors)
...
def create_tile_based_colorizer() -> bytes:
    """Tile-based colorizer with simplified projectile detection (same as v2.30)."""
    code = bytearray()
    labels = {}
    jumps_to_fix = []
    ...
    # All projectiles (< 0x10) use Palette 0 (dynamic)
...
def create_palette_loader(
    ...
    """
    Load CGB palettes with FIXED jump offsets (v2.31).

    CRITICAL FIX: Jump offsets were wrong in v2.29/v2.30:
    - JR NZ, +3 should be JR NZ, +5 (skip LD HL + JR)
    - JR +2 should be JR +3 (skip LD HL)
    """
    code = bytearray()
    ...
    # FIXED: Read Sara form inline, correct jump offsets
    obj_data_addr = palette_data_addr + 64
    code.extend([0x3E, 0x80, 0xE0, 0x6A])  # Set OCPS to palette 0
    code.extend([0xF0, 0xBE])              # LDH A, [0xFFBE] - read Sara form
    code.extend([0xB7])                    # OR A (check if zero = Sara W)
    # FIXED: JR NZ, +5 (skip 3-byte LD HL + 2-byte JR = 5 bytes)
    code.extend([0x20, 0x05])              # JR NZ, +5
    code.extend([0x21, obj_data_addr & 0xFF, (obj_data_addr >> 8) & 0xFF])  # Sara W palette (3 bytes)
    # FIXED: JR +3 (skip 3-byte LD HL)
    code.extend([0x18, 0x03])              # JR +3
    code.extend([0x21, sara_projectile_dragon_addr & 0xFF, (sara_projectile_dragon_addr >> 8) & 0xFF])  # Sara D palette (3 bytes)
...
def main():
    ...
    print("=== Penta Dragon DX v2.31 ===")
    print("Fixed jump offsets in palette loader (fixes flickering)")
    ...
    print(f"\nFIXES in v2.31:")
    print(f"  - JR NZ, +3 → JR NZ, +5 (correct offset to skip LD HL + JR)")
    print(f"  - JR +2 → JR +3 (correct offset to skip LD HL)")
    print(f"  - This should fix random palette flickering")
