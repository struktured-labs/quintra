#!/usr/bin/env python3
"""
v2.61: Single-tilemap BG + conditional palette + optimized OBJ for MiSTer speed

Building on v2.60's conditional palette and OBJ optimizations, this version
switches the BG colorizer from dual-tilemap to single active tilemap, halving
the number of VRAM accesses per frame.

Key optimization: reads LCDC bit 3 to determine which tilemap (0x9800 or
0x9C00) is active, and only processes that one. This cuts BG VRAM accesses
from ~436/frame (dual) to ~218/frame (single), saving ~10,000 T-cycles.

Combined optimizations vs v2.59:
1. Conditional palette: skip palette load when state unchanged (~4,700T saved)
2. OBJ hidden sprite skip: early exit for tile 0x00 (~3,000T saved)
3. Single active tilemap: half the VRAM reads/writes (~10,000T saved)
Total: ~17,700T saved → handler drops from 74% to ~26% of frame

Key features:
- Single active tilemap via LCDC bit 3 detection
- Interleaved read→write (read tile → write attr immediately)
- 0xFF anti-flicker filter
- 109 tiles/frame (prime, coprime with 1024, 0.16s settling)
...
def create_bg_colorizer(bg_table_addr: int) -> bytes:
    """BG colorizer: SINGLE ACTIVE TILEMAP for MiSTer speed.

    Reads LCDC bit 3 to determine which tilemap is active (0x9800 or 0x9C00)
    and only processes that one. This halves VRAM accesses vs dual-tilemap.

    Interleaved read→write: read tile → write attr immediately (~30T gap).
    0xFF anti-flicker filter prevents palette corruption from mode 3 reads.
    Safe HRAM: FF91=counter_lo, FFA5=counter_hi
    """
    TILES_PER_VBLANK = 109  # prime, coprime with 1024, 0.16s settling
    ...
    emit([0x7E])              # LD A, [HL]         ; ROM table → palette
    emit([0xFE, 0xFF])        # CP 0xFF            ; garbage filter
    emit_jr(0x28, 'skip_write')  # JR Z, skip_write
    ...
    emit([0x12])              # LD [DE], A          ; write attr (~30T after read)
    emit([0xAF])              # XOR A
    ...
    emit_jr(0x20, 'no_wrap')  # JR NZ, no_wrap     ; 12T (not taken, 99.6%)

    # Wrap: E went 0xFF→0x00, fix D
    emit([0x14])              # INC D
    emit([0x7A])              # LD A, D
    emit([0xE6, 0x03])        # AND 0x03           ; strip to 0-3
    emit([0xC6, 0x98])        # ADD A, 0x98        ; base 0x98-0x9B
    emit([0x57])              # LD D, A            ; temp
...
def main():
    ...
    print("=== Penta Dragon DX v2.61 ===")
    print("Single-tilemap BG + conditional palette + optimized OBJ + MiSTer speed fix")
    print()
