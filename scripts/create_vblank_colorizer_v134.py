#!/usr/bin/env python3
"""
v1.34: BG tile colorization via incremental VRAM attribute updates

Adds BG tile colorization to v1.33's stable sprite colorization:
- Processes 2 rows (64 tiles) per frame during VBlank
- Uses tile→palette lookup table for consistent coloring
- Full tilemap updated every ~16 frames

Tile ranges (from analysis):
...
def create_bg_colorizer(lookup_table_addr: int, row_counter_addr: int) -> bytes:
    """
    BG tile colorizer - processes 2 rows (64 tiles) per frame.
    Uses row counter at row_counter_addr (HRAM) to track progress.

    Simplified approach: Use B for lookup table high byte, keep HL as tilemap pointer.
    """
    code = bytearray()
    ...
    # Read row counter (0-15) from HRAM
    code.extend([0xF0, row_counter_addr & 0xFF])  # LDH A, [row_counter]
    code.extend([0xE6, 0x0F])                      # AND 0x0F (wrap to 0-15)

    # Calculate tilemap offset: row * 64 (2 rows * 32 tiles each)
    # Use D to accumulate high byte
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v134.gb")
    ...
    print("\n=== v1.34: BG Tile Colorization ===")
    print("  Adds incremental BG tile coloring (2 rows/frame)")
    print()
    ...
    bg_colorizer_addr = 0x6960     # ~66 bytes, ends at ~0x69A2
    shadow_main_addr = 0x69B0      # 52 bytes, ends at 0x69E4
    combined_addr = 0x69F0         # 13 bytes, ends at 0x69FD
