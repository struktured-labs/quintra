#!/usr/bin/env python3
"""
v2.53: Safe HRAM + Anti-Flicker Filter

FIXES:
1. Safe HRAM (from v2.52): FF91/FFA5 instead of FFEA/FFEB
   - FFEA/FFEB had 13+ game references causing mutual state corruption
   - FF91/FFA5 have ZERO references in the original ROM
2. 0xFF Anti-Flicker Filter (new):
   - PPU mode 3 VRAM reads return 0xFF garbage
   - Without filter: 0xFF → palette 0 (wrong), overwrites correct palette → FLICKER
   - With filter: 0xFF → skip write, preserve existing correct palette → NO FLICKER
   - Accuracy monotonically increases: tiles only get better, never worse

Base: v2.36 (known stable) with safe HRAM + anti-flicker.
"""
import sys
import yaml
...
def create_bg_colorizer_continuous() -> bytes:
    ...
    # Uses FF91/FFA5 - verified ZERO references in original ROM
    # (FFEA/FFEB were dangerous: 4+13 and 9+6 game references!)
    code.extend([0xF0, 0x91])  # LDH A, [FF91]
    ...
    # 0xFF anti-flicker filter: skip if tile is garbage from mode 3
...
def main():
    ...
    print("=== Penta Dragon DX v2.53 ===")
    print("Safe HRAM + Anti-Flicker Filter")
    print()
    print("FIXES in v2.53:")
    print("  1. BG counter: FF91/FFA5 (was FFEA/FFEB - game conflicts!)")
    print("  2. 0xFF anti-flicker: skip writes when VRAM returns garbage")
    print()
