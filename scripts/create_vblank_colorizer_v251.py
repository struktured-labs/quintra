#!/usr/bin/env python3
"""
v2.51: Input Fix + Anti-Flicker (minimal diff from v2.36 STABLE)

CHANGES over v2.36:
1. INPUT: 2 d-pad reads (was 1) - fixes phantom forward movement
   - Original game uses 2 reads for d-pad line debounce
   - 1 read caused electrical noise → phantom Right presses ("ship mode")
   - Fits in 47-byte hook via A=0 bank restore trick (DMA returns A=0,
     MBC1 maps bank 0 writes to bank 1, saves 2 bytes)
2. 0xFF ANTI-FLICKER FILTER: Skip VRAM writes when tile reads return 0xFF
   - PPU mode 3 returns 0xFF garbage; without filter this writes palette 0
     over correct attributes causing visible flicker
   - With filter, garbage reads are skipped (attribute preserved)

UNCHANGED from v2.36:
- 32 tiles/frame BG colorizer (same CPU cost, no sluggishness)
- Inline tile→palette comparison (no ROM table - those fail during VBlank)
- Shadow colorizer: both buffers (C000 + C100)
...
def _emit_tile_to_palette(code: bytearray) -> int:
...
def create_bg_colorizer_continuous() -> bytes:
    """
    Continuous BG colorizer - v2.50: 48 tiles/frame + 0xFF anti-flicker filter.

    KEY CHANGES (v2.50 over v2.36):
    1. 48 tiles per frame (was 32): Full tilemap refresh every ~21 frames (~0.35s)
       48 tiles x ~75 M = 3600 M + DMA 160 M = 3760 M (under 4560 M VBlank budget)
    2. 0xFF anti-flicker filter: If tile read is 0xFF (PPU mode 3 garbage),
       skip the inline comparison chain entirely. This prevents writing
       palette 0 over correct attributes when VRAM returns garbage.

    INHERITED from v2.36/v2.35:
    - Uses FFEA/FFEB for position counter (game overwrites FFE0/FFE1!)
    - Reads tiles from EACH tilemap independently (they can differ)
    - INLINE tile->palette logic - eliminates ROM lookup table read that
      was returning wrong values due to MBC1 bank state during VBlank
    - Processes both 0x9800 and 0x9C00 tilemaps at each position
    """
    code = bytearray()
    ...
    # B = 48 positions per frame (v2.50: increased from 32)
    # BG colorizer runs FIRST in VBlank - has ~4560 M-cycles available.
    # 48 tiles x ~75 M-cycles avg = 3600 M + DMA 160 M = 3760 M < 4560 M.
    ...
    code.extend([0x06, 0x20])  # LD B, 32 (same as v2.36 - no sluggishness)
    ...
    # v2.50: 0xFF anti-flicker filter for 9800 tilemap
    # If tile is 0xFF (PPU mode 3 garbage), skip palette comparison + write
    code.extend([0x7A])         # LD A, D
    code.extend([0xFE, 0xFF])   # CP 0xFF
    # JR Z needs to skip over: _emit_tile_to_palette (29 bytes) + LD A,1 (2) + LDH (2) + LD [HL],E (1) = 34 bytes
    skip_9800_jr_pos = len(code)
    code.extend([0x28, 0x00])   # JR Z, skip_9800 (placeholder, will be patched)

    # Inline tile->palette (D -> E), no ROM read needed
    palette_size_9800 = _emit_tile_to_palette(code)
    ...
    # Patch skip_9800 JR Z offset: from JR+2 to here
    skip_9800_target = len(code)
    code[skip_9800_jr_pos + 1] = (skip_9800_target - (skip_9800_jr_pos + 2)) & 0xFF
    ...
    # v2.50: 0xFF anti-flicker filter for 9C00 tilemap
    code.extend([0x7A])         # LD A, D
    code.extend([0xFE, 0xFF])   # CP 0xFF
    # JR Z needs to skip over: _emit_tile_to_palette (29 bytes) + LD A,1 (2) + LDH (2) + LD [HL],E (1) = 34 bytes
    skip_9c00_jr_pos = len(code)
    code.extend([0x28, 0x00])   # JR Z, skip_9C00 (placeholder, will be patched)

    # Inline tile->palette (D -> E), no ROM read needed
    palette_size_9c00 = _emit_tile_to_palette(code)

    code.extend([0x3E, 0x01])   # LD A, 1
    code.extend([0xE0, 0x4F])   # LDH [0xFF4F], A  (VRAM bank 1)
    code.extend([0x73])         # LD [HL], E  (write attr to 0x9C00 bank 1)

    # Patch skip_9C00 JR Z offset: from JR+2 to here
    skip_9c00_target = len(code)
    code[skip_9c00_jr_pos + 1] = (skip_9c00_target - (skip_9c00_jr_pos + 2)) & 0xFF
...
def create_vblank_hook_with_input(combined_func_addr: int) -> bytes:
    ...
    v2.51: 2 d-pad reads for proper debounce (was 1 in v2.36/v2.50).
    Original game uses 2 sequential reads for d-pad line settling.
    With only 1 read, electrical noise registers as phantom Right presses.

    Fits in 47 bytes via A=0 bank restore trick:
    - Combined function's last call is DMA at FF80
    - DMA wait loop: LD A,0x28; .wait: DEC A; JR NZ,.wait; RET → A=0
    - MBC1 maps bank 0 writes to bank 1, so LD [0x2000],A with A=0 = bank 1
    - Saves 2 bytes (removes LD A,0x01), making room for 2nd d-pad read
    """
    # Must fit within 47 bytes total (0x0824-0x0852)
    # 35 bytes joypad + 12 bytes hook = 47 total
    joypad_input = bytearray([
        # D-pad (2 reads for debounce - fixes phantom input)
        0x3E, 0x20,        # LD A, 0x20        ; select d-pad
        0xE0, 0x00,        # LDH [FF00], A
        0xF0, 0x00,        # LDH A, [FF00]     ; read 1 (settle, discard)
        0xF0, 0x00,        # LDH A, [FF00]     ; read 2 (use this one)
        0x2F,              # CPL
        0xE6, 0x0F,        # AND 0x0F
    ...
    hook_code = bytearray([
        0x3E, 0x0D, 0xEA, 0x00, 0x20,  # LD A,0x0D; LD [0x2000],A  ; bank 13
        0xCD, combined_func_addr & 0xFF, combined_func_addr >> 8,  # CALL combined
        0xEA, 0x00, 0x20,              # LD [0x2000],A  ; A=0 from DMA → bank 1 (MBC1)
        0xC9,
    ])  # 12 bytes hook
...
def main():
    ...
    print("=== Penta Dragon DX v2.51 ===")
    print("Input Fix + Anti-Flicker (minimal diff from v2.36)")
    print()
    print("CHANGES from v2.36:")
    print("  1. INPUT: 2 d-pad reads (fixes phantom forward movement)")
    print("  2. 0xFF anti-flicker filter: skip garbage VRAM reads")
    print()
    print("UNCHANGED from v2.36:")
    print("  - 32 tiles/frame BG colorizer (same speed, no sluggishness)")
    print("  - Shadow colorizer: both buffers (fixes inverted FFCB logic)")
    print()
