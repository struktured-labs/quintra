#!/usr/bin/env python3
"""
v1.85: BG Colorizer - continuous colorization with VBK save/restore

- 8 tiles per frame, direct VBK switch per tile
- Continuous colorization (wraps counter) to handle game tilemap updates
- VBK save/restore retained to prevent Sara corruption
"""
import sys
import yaml
...
def create_bg_row_colorizer_lazy(lookup_table_addr: int) -> bytes:
    """BG colorizer - continuous colorization with VBK save/restore.

    Counter cycles 0-187:
    - 0-59: warmup (skip colorization)
    - 60-187: colorize 8 tiles per frame
    - 188+: wrap back to 60 (continuous)

    VBK is saved/restored to prevent Sara corruption.
    """
    HRAM_COUNTER = 0xC4
    WARMUP_FRAMES = 60
    TOTAL_STEPS = 128
    WRAP_VALUE = WARMUP_FRAMES + TOTAL_STEPS  # 188 = wrap point
    TILES_PER_FRAME = 8
    code = bytearray()
    ...
    code.extend([0x3E, WARMUP_FRAMES])  # LD A, 60 (wrap back to start of colorization)
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v185.gb")
    ...
    print("\n=== v1.85: Continuous colorization with VBK save/restore ===")
    print("  8 tiles/frame, continuous (wraps) to handle game tilemap updates")
    print("  VBK save/restore retained to prevent Sara corruption")
    print()
