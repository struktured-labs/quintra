#!/usr/bin/env python3
"""
v2.01: Fixed buffer init + tile-based building + GDMA

Combines:
- v2.00's fixed buffer initialization (no A clobber bug)
- v1.98's tile-based buffer building (32 tiles/frame)
- GDMA for safe, instant copy to VRAM bank 1
"""
import sys
import yaml
...
def create_tile_based_colorizer() -> bytes:
    """Sprite colorizer (unchanged from working v1.09)."""
    code = bytearray()
    labels = {}
    jumps_to_fix = []
...
def create_bg_tile_lookup_table() -> bytes:
    ...
    for i in range(256):
        ...
        # Walls (specific tiles) -> palette 2 (purple)
        elif i in [0x03, 0x04, 0x13, 0x14, 0x23, 0x24, 0x33, 0x34]:
            table[i] = 0x02

        # Platforms/structural (0x30-0x3F) -> palette 3 (cyan)
        elif 0x30 <= i < 0x40:
            table[i] = 0x03
...
def create_buffer_initializer() -> bytes:
    """One-time buffer initialization to all zeros (FIXED - no A clobber)."""
    ATTR_BUFFER = 0xD000
    HRAM_INIT = 0xC7
    ...
    code.extend([0xF0, HRAM_INIT])        # LDH A, [0xFFC7]
    code.append(0xB7)                     # OR A
    code.append(0xC0)                     # RET NZ (already initialized)

    # Initialize buffer using nested loops (FIXED)
    code.extend([0xF5, 0xC5])             # PUSH AF, BC
    code.extend([0x21, 0x00, 0xD0])       # LD HL, 0xD000
    code.extend([0x06, 0x04])             # LD B, 4 (outer)
    code.extend([0xAF])                   # XOR A

    outer_loop = len(code)
    code.extend([0x0E, 0x00])             # LD C, 0 (256 inner)
    ...
    code.extend([0x20, (outer_loop - len(code) - 2) & 0xFF])

    code.extend([0x3E, 0x01])             # LD A, 1
    code.extend([0xE0, HRAM_INIT])        # LDH [0xFFC7], A
...
def create_buffer_builder(lookup_table_addr: int) -> bytes:
    """
    Builds attribute buffer incrementally (32 tiles/frame).

    Reads tiles from VRAM, looks up palette, writes to buffer.
    ...
    # Read row counter
    code.extend([0xF0, HRAM_ROW])
    code.append(0x47)                      # LD B, A

    # If row < WARMUP, skip work
    code.extend([0xFE, WARMUP])
    skip_to_end = len(code)
    code.extend([0x38, 0x00])              # JR C, placeholder

    # Calculate row = (counter - WARMUP) mod 32
    code.extend([0xD6, WARMUP])            # SUB WARMUP
    code.extend([0xE6, 0x1F])              # AND 0x1F (mod 32)
    code.append(0x4F)                      # LD C, A (row 0-31)
    ...
    code.append(0x0D)                      # DEC C
    code.extend([0x20, (build_loop - len(code) - 2) & 0xFF])
...
def create_gdma_copy() -> bytes:
    """GDMA copy: buffer (0xD000) -> VRAM bank 1 (0x9800)."""
    ATTR_BUFFER = 0xD000
    TILEMAP_VRAM = 0x9800
    BLOCKS = 64
...
def create_combined_func(palette_loader_addr: int, shadow_main_addr: int,
                         buffer_init_addr: int, buffer_builder_addr: int,
                         gdma_copy_addr: int) -> bytes:
    code = bytearray()
    code.extend([0xCD, palette_loader_addr & 0xFF, palette_loader_addr >> 8])
    code.extend([0xCD, shadow_main_addr & 0xFF, shadow_main_addr >> 8])
    code.extend([0xCD, buffer_init_addr & 0xFF, buffer_init_addr >> 8])
    code.extend([0xCD, buffer_builder_addr & 0xFF, buffer_builder_addr >> 8])
    code.extend([0xCD, gdma_copy_addr & 0xFF, gdma_copy_addr >> 8])
...
def main():
    ...
    output_rom = Path("rom/working/penta_dragon_dx_v201.gb")
    ...
    print("\n=== v2.01: Fixed init + tile-based building + GDMA ===")
    print()
    ...
    buffer_init_addr = 0x6A80
    buffer_builder_addr = 0x6AB0
    gdma_copy_addr = 0x6B20
    ...
    print(f"Buffer init: {len(buffer_init)} bytes at 0x{buffer_init_addr:04X}")
    print(f"Buffer builder: {len(buffer_builder)} bytes at 0x{buffer_builder_addr:04X}")
    print(f"GDMA copy: {len(gdma_copy)} bytes at 0x{gdma_copy_addr:04X}")
    ...
    rom[bank13_offset + (buffer_init_addr - 0x4000):bank13_offset + (buffer_init_addr - 0x4000) + len(buffer_init)] = buffer_init
    rom[bank13_offset + (buffer_builder_addr - 0x4000):bank13_offset + (buffer_builder_addr - 0x4000) + len(buffer_builder)] = buffer_builder
    rom[bank13_offset + (gdma_copy_addr - 0x4000):bank13_offset + (gdma_copy_addr - 0x4000) + len(gdma_copy)] = gdma_copy
