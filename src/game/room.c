...
static void room_load_dynamic_fx_identity(void) {
    ...
    if (!run_state.world_mode && !room_state_has_shop_wares()) {
        ...
        // Slot 79 is a phase-safe multiplex: Cinder Kite is Ember-only,
        // Sunwheel is Temple-only, and Dusk Midge starts in Bloodmoon. No
        // stage pool combines them, so one OBJ tile carries all three art
        // identities without growing the fixed CGB atlas.
