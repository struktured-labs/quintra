...
static const u8 bgt_wild_climate[6][16] = {
    // Mud: broad dark pools with a few reflective bubbles.
    {0x00,0x00, 0x18,0x00, 0x00,0x24, 0x42,0x00,
     0x00,0x18, 0x24,0x00, 0x00,0x42, 0x18,0x00},
    // Sand: wind-combed dunes, deliberately quieter than dungeon floors.
    {0x00,0x00, 0x00,0x1F, 0x3E,0x00, 0x00,0x00,
     0x00,0xF8, 0x7C,0x00, 0x00,0x00, 0x00,0x00},
    // Snow: sparse crystalline flecks over a bright open field.
    {0x00,0x00, 0x10,0x10, 0x44,0x44, 0x00,0x00,
     0x01,0x01, 0x28,0x28, 0x00,0x00, 0x82,0x82},
    // Hole: a broken rim around an unmistakably empty black centre.
    {0x3C,0x3C, 0x7E,0x42, 0xE7,0x81, 0xC3,0x81,
     0xC3,0x81, 0xE7,0x81, 0x7E,0x42, 0x3C,0x3C},
    // Mountain: stepped peak, snow/shine cap, and a dense rock foot.
    {0x18,0x18, 0x3C,0x24, 0x7E,0x42, 0xFF,0x81,
     0xFF,0x99, 0xFF,0xA5, 0xFF,0xC3, 0xFF,0xFF},
    // Reeds: three tall wetland stalks with hooked seed heads.
    {0x24,0x24, 0x6C,0x48, 0x24,0x24, 0x25,0x24,
     0x24,0x24, 0xA4,0x24, 0x24,0x24, 0x7E,0x24},
};
...
static const u8 bgt_gate_tiles[5][16] = {
    ...
    {0x3C,0x3C, 0x7E,0x42, 0xFF,0x81, 0xE7,0x99,
     0xE7,0x99, 0xFF,0x81, 0x7E,0x42, 0x3C,0x3C},
    // Deep water: three heavy currents, distinct from harmless pond trim.
    {0x00,0x3C, 0x7E,0xC3, 0x00,0x00, 0x3C,0x7E,
     0xC3,0x00, 0x00,0x3C, 0x7E,0xC3, 0x00,0x00},
    // Chasm: broken pale ledges around a visibly empty center.
    {0xFF,0x81, 0x81,0x81, 0x81,0x81, 0x81,0x81,
     0x81,0x81, 0x81,0x81, 0x81,0x81, 0xFF,0x81},
    // Thorn lattice: crossed hooked vines, not generic spikes.
    {0x81,0x81, 0x42,0xC3, 0xA5,0x66, 0x5A,0x3C,
     0x5A,0x3C, 0xA5,0x66, 0x42,0xC3, 0x81,0x81},
    // Hive vent: narrow breathing slits only Vespine can enter.
...
void tiles_load_dungeon_bg(void) BANKED {
    ...
    set_bkg_data(BGT_GATE_BOULDER, 5, bgt_gate_tiles[0]);
    set_bkg_data(BGT_WILD_MUD, 6, bgt_wild_climate[0]);
}

// Colossus rooms need a different texture hierarchy from ordinary dungeon
// exploration. A repeated crack/pebble field competes with the giant BG body,
// its moving weak point, and dense projectile tells. Keep the broad plane one
// quiet colour, then let procgen place only a few low-contrast border inlays.
// All three slots retain the floor palette, so none of this can masquerade as
// a hazard or interactive object.
static const u8 bgt_arena_quiet[16] = {
    0x00,0xFF, 0x00,0xFF, 0x00,0xFF, 0x00,0xFF,
    0x00,0xFF, 0x00,0xFF, 0x00,0xFF, 0x00,0xFF
};
static const u8 bgt_arena_inlay[16] = {
    0xFF,0xFF, 0x81,0xFF, 0x81,0xFF, 0x81,0xFF,
    0x81,0xFF, 0x81,0xFF, 0x81,0xFF, 0xFF,0xFF
};
static const u8 bgt_arena_seal[16] = {
    0x00,0xFF, 0x18,0xFF, 0x24,0xFF, 0x42,0xFF,
    0x42,0xFF, 0x24,0xFF, 0x18,0xFF, 0x00,0xFF
};

void tiles_load_arena_floor(void) BANKED {
    set_bkg_data(BGT_FLOOR,  1, bgt_arena_quiet);
    set_bkg_data(BGT_FLOOR2, 1, bgt_arena_inlay);
    set_bkg_data(BGT_FLOOR3, 1, bgt_arena_seal);
}

void tiles_load_standard_floor(void) BANKED {
    set_bkg_data(BGT_FLOOR,  1, bgt_floor_plain);
    set_bkg_data(BGT_FLOOR2, 1, bgt_floor_crack);
    set_bkg_data(BGT_FLOOR3, 1, bgt_floor_pebble);
}
