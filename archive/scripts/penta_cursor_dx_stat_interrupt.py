#!/usr/bin/env python3
"""
GBC STAT Interrupt Approach - Hook LCD interrupt for precise timing
Modify OAM during VBlank using STAT interrupt (safer than direct VBlank hook)
"""
import sys
import yaml
...
def main():
    ...
    # Use input handler approach but with DOUBLE-PASS (before + after)
    # This is safer than VBlank and ensures coverage
    ...
    # Double-pass: before and after game code
    ...
    print(f"✓ Built ROM with DOUBLE-PASS sprite palette override")
    print(f"  - Strategy: Modify OAM BEFORE and AFTER game code")
    print(f"  - Lookup table: {mapped_count} tiles mapped")
    print(f"  - Ensures coverage: before game writes + after game overwrites")
