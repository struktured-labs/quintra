#!/usr/bin/env python3
"""
Preemptive colorizer - runs sprite loop BEFORE original input handler.
Also modifies actual OAM which persists until next frame's DMA.
"""
import sys
import yaml
...
def create_oam_only_loop(lookup_table_addr: int) -> bytes:
    """Only modify actual OAM (0xFE00) - runs after DMA so changes persist."""
    lo = lookup_table_addr & 0xFF
    hi = (lookup_table_addr >> 8) & 0xFF

    code = bytearray()
    code.extend([0xF5, 0xC5, 0xD5, 0xE5])  # PUSH

    # Only actual OAM - DMA already happened, our changes persist until next DMA
...
def main():
    ...
    sprite_loop = create_oam_only_loop(0x6E00)

    # Order: palette load → sprite loop → original input
    # Sprite loop runs FIRST (after palette load) to catch OAM right after DMA
    combined = bytes([
    ...
    print(f"  OAM-only, runs before input handler")
