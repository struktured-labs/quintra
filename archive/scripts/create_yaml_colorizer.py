#!/usr/bin/env python3
"""
Stable tile-based palette colorizer that reads palettes from YAML file.
"""
import sys
import yaml
...
def load_palettes_from_yaml(yaml_path: Path) -> tuple[bytes, bytes]:
    ...
    # BG palettes (8 x 4 colors = 64 bytes)
    ...
    # OBJ palettes (8 x 4 colors = 64 bytes)
...
def create_lookup_table() -> bytes:
    ...
    # Monster mappings based on OAM analysis:
    # SARA D (tiles 0-7): Palette 0 (RED)
    for tile in range(0, 8):
        table[tile] = 0

    # SARA W (tiles 8-15): Palette 1 (GREEN)
    for tile in range(8, 16):
        table[tile] = 1

    # DRAGONFLY (tiles 32-47): Palette 2 (BLUE)
    for tile in range(32, 48):
        table[tile] = 2

    # Additional ranges for other sprites
...
def create_stable_sprite_loop(lookup_table_addr: int) -> bytes:
    """Create sprite loop that modifies actual OAM + active shadow buffer."""
    ...
    code.extend([0x23, 0x23])  # INC HL x2
    ...
    # Read active shadow buffer from FFCB
    code.extend([0xF0, 0xCB, 0xE6, 0x01, 0xC6, 0xC0, 0x67, 0x2E, 0x00])
    ...
    skip2_mod_tgt = len(code)
    code[skip2_mod + 1] = (skip2_mod_tgt - skip2_mod - 2) & 0xFF
    code.append(0x23)
    jr2_dec = len(code)
    code.extend([0x18, 0x00])

    next2_spr = len(code)
    code[skip2_jrz + 1] = (next2_spr - skip2_jrz - 2) & 0xFF
    code[skip2_jrnc + 1] = (next2_spr - skip2_jrnc - 2) & 0xFF
    code.extend([0x23, 0x23, 0x23, 0x23])

    dec2_b = len(code)
    code[jr2_dec + 1] = (dec2_b - jr2_dec - 2) & 0xFF
    code.append(0x05)
...
def main():
    ...
    print(f"  Palettes loaded from YAML")
    print(f"  Sprite loop: {len(sprite_loop)} bytes")
