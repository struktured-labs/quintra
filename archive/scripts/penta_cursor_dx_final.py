#!/usr/bin/env python3
"""
FINAL APPROACH: Double-pass with optimized timing
Modify OAM before AND after game code, with minimal overhead
This is the most reliable approach that balances coverage and stability
"""
import sys
import yaml
...
def main():
    ...
    # Generate lookup table - focus on Sara W (4-7) and Dragonfly (0-3)
    ...
    # Ultra-optimized sprite loop (minimal instructions)
    ...
    # Double-pass: before + after game code
    temp_combined = bytes([
        # Load OBJ palettes (minimal - only what's needed)
    ...
    print(f"✓ Built ROM with FINAL double-pass approach")
    print(f"  - Strategy: Modify OAM BEFORE and AFTER game code")
    print(f"  - Lookup table: {mapped_count} tiles mapped")
    print(f"  - Sara W (tiles 4-7): Palette 1")
    print(f"  - Dragonfly (tiles 0-3): Palette 0")
    print(f"  - Total overhead: {len(combined_bank13)} bytes")
