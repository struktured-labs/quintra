#!/usr/bin/env python3
"""
GBC Post-Game OAM Modification - Modify OAM AFTER game code completes
This ensures our palette assignments happen after all game OAM writes
"""
import sys
import yaml
...
def main():
    ...
    # Strategy: Run sprite loop AFTER original input handler completes
    # This ensures we modify OAM after all game code has run
    temp_combined = bytes([
        # Load OBJ palettes
        0x3E, 0x80, 0xE0, 0x6A, 0x0E, 0x40, 0x2A, 0xE0, 0x6B, 0x0D, 0x20, 0xFA,
        # Original input handler runs FIRST
    ]) + original_input + bytes([
        # NOW run sprite loop AFTER game code (this is the key!)
    ]) + temp_loop + bytes([
        0xC9,  # RET
    ])
    ...
    # Final: Original input handler FIRST, then sprite modification
    combined_bank13 = bytes([
        # Load OBJ palettes
        0x3E, 0x80, 0xE0, 0x6A, 0x0E, 0x40, 0x2A, 0xE0, 0x6B, 0x0D, 0x20, 0xFA,
        # Original input handler (game code runs FIRST)
    ]) + original_input + bytes([
        # Sprite loop runs AFTER game code completes
    ...
    print(f"✓ Built ROM with POST-GAME sprite palette override")
    print(f"  - Strategy: Modify OAM AFTER game code completes")
    print(f"  - Lookup table: {mapped_count} tiles mapped")
    print(f"  - This ensures our changes happen after all game OAM writes")
