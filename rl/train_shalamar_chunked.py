"""Train Shalamar in CHUNKS — each chunk = 5 epochs with fresh process.

Workaround for torch+PyBoy intermittent hangs. Each chunk runs as a separate
process, saves checkpoint, then exits. Wrapper retries on hang.
"""
import os, sys, json, time
sys.path.insert(0, "/home/struktured/projects/penta-dragon-dx-claude/rl")

# Import env stuff at function level to avoid early torch
...
def run_chunk(epochs, steps_per_epoch, label, resume_path=None):
    """Run one chunk of training. Save final checkpoint. Exit."""
...
if __name__ == "__main__":
    ...
    label = sys.argv[3] if len(sys.argv) > 3 else "shalamar_ck"
