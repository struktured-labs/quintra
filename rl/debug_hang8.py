"""Debug 8: extend debug_hang7 with buf.store and FFBA check."""
import sys, time
...
for t in range(2000):
    ...
    if done:
        ...
        obs = obs2
    if t % 200 == 0:
        print(f"  t={t} ({time.time()-t0:.2f}s, {n_done} done)", flush=True)
print(f"DONE {time.time()-t0:.2f}s {n_done} eps", flush=True)
env.close()
