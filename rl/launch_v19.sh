#!/bin/bash
# v19: continue v18 setup but max_steps=15000 (try to reach stage boss after 2 mini-boss kills)
...
RESUME = '/home/struktured/projects/penta-dragon-dx-claude/rl/ppo_v18_bc_v4r_final.pt'

device = 'cuda' if torch.cuda.is_available() else 'cpu'
n_envs, epochs, steps_per_epoch, max_steps = 2, 1500, 512, 15000
...
cfg = PPOConfig(epochs=epochs, steps_per_epoch=steps_per_epoch * n_envs,
                ...
                hidden=256, n_layers=3, gamma=0.99, pi_lr=5e-5)  # even lower LR to preserve v18
agent = PPOAgent(obs_dim, N_ACTIONS, cfg, device=device)
state = torch.load(RESUME, map_location=device, weights_only=False)
agent.net.load_state_dict(state['model'])
print(f'Resumed from v18 ({RESUME}); chasing stage boss with max_steps=15000')

metrics, completed_returns, completed_bosses, multi_kill_eps = [], [], [], []
arena_eps = []  # episodes that reached stage boss arena
obs = venv.reset()
...
for ep in range(epochs):
    ...
    for t in range(steps_per_epoch):
        ...
        for i in range(n_envs):
            ...
            ep_rewards[i] += rews[i]
            # Check events for stage_boss progress
            for ev in infos[i].get('events', []):
                if isinstance(ev, tuple) and ev and 'STAGE_ARENA' in str(ev[0]):
                    arena_eps.append({'ep_global': len(completed_returns)+1, 'epoch': ep+1})
                if isinstance(ev, tuple) and ev and 'STAGE_BOSS_KILL' in str(ev[0]):
                    print(f'  *** STAGE BOSS KILL *** ep={ep+1}, level={ev[1] if len(ev)>1 else \"?\"}')
            if dones[i]:
