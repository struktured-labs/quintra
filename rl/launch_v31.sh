#!/bin/bash
# v18: BC pretrain → PPO finetune with REWARD v4 (fire_in_combat=0, b_button=0)
# Real ROM, gargoyle.state, max_steps=8000, entropy=0.005
...
cfg = PPOConfig(epochs=epochs, steps_per_epoch=steps_per_epoch * n_envs,
                ...
                hidden=512, n_layers=4, gamma=0.99, pi_lr=1e-4)
agent = PPOAgent(obs_dim, N_ACTIONS, cfg, device=device)
# BC weights are 256-hidden, can't load into 512-hidden network
# fresh init for bigger net
print(f'v31: Bigger network 512x4, FRESH init')
