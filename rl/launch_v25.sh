#!/bin/bash
# v25: RESUME v19 ep200 (combat policy) but train on gameplay_start.state.
# Hypothesis: combat skill is preserved; nav skill emerges from corridor experience.
...
n_envs, epochs, steps_per_epoch, max_steps = 2, 1000, 512, 18000
...
cfg = PPOConfig(epochs=epochs, steps_per_epoch=steps_per_epoch * n_envs,
                train_iters=10, entropy_coef=0.01,  # higher to explore corridors
                hidden=256, n_layers=3, gamma=0.99, pi_lr=5e-5)  # very low LR — preserve combat skill
agent = PPOAgent(obs_dim, N_ACTIONS, cfg, device=device)
state = torch.load(RESUME, map_location=device, weights_only=False)
agent.net.load_state_dict(state['model'])
print(f'v25: Resumed v19 ep200, training on gameplay_start.state to merge combat+nav')
