#!/bin/bash
# v27: BC + PPO from gargoyle.state, ckpt every 10 epochs to catch narrow peaks like v19 ep200.
# Same as v18 setup but more granular checkpoint sampling.
...
SAVE_DIR = '/home/struktured/projects/penta-dragon-dx-claude/rl/v27_ckpts'
...
print(f'v27: BC + PPO, ckpt every 10 epochs into {SAVE_DIR}')
...
for ep in range(epochs):
    ...
    if time.time() - last_print >= 5 or ep == 0 or ep == epochs - 1:
        ...
        last_print = time.time()
    # GRANULAR ckpts every 10 epochs
    if (ep + 1) % 10 == 0:
        ckpt = f'{SAVE_DIR}/ep{ep+1:04d}.pt'
        torch.save({'model': agent.net.state_dict(), 'metrics_summary': m,
                    'multi_kill_episodes_count': len(multi_kill_eps)}, ckpt)

final = '/home/struktured/projects/penta-dragon-dx-claude/rl/ppo_v27_granular_final.pt'
