"""Combat-focused trainer using save-state for instant respawn into mini-boss.

Each episode starts with FFBF != 0 (boss already spawned). Goal: reduce DCBB to 0.
Episodes end on death (D880=0x17), boss kill, or 1500 steps (~100s gameplay).
"""
...
def main(epochs: int = 200, steps_per_epoch: int = 512, n_envs: int = 4,
         max_steps: int = 1500, save_dir: str = SAVE_DIR, resume: str = None,
         label: str = "combat",
    ...
    cfg = PPOConfig(epochs=epochs, steps_per_epoch=steps_per_epoch * n_envs,
                    train_iters=10, entropy_coef=0.05)  # high entropy for exploration
                ...
                if dones[i]:
                    ...
                    if nb > 0:
                        boss_kill_episodes.append({
                            "ep_global": len(completed_returns), "epoch": ep + 1,
                            "n_bosses": nb, "reward": float(ep_rewards[i]),
    ...
    print(f"\nFinal: {final}")
    print(f"Total: {len(completed_returns)} episodes, {sum(completed_bosses)} cum kills, "
...
if __name__ == "__main__":
    ...
    label = sys.argv[4] if len(sys.argv) > 4 else "combat"
