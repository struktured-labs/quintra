#!/bin/bash
# v22: REPLICATE v18 setup (BC pretrain + PPO) but save ckpts every 25 eps to find peaks.
# Tests reproducibility of v19 ep200 result with different env stochasticity.
...
import numpy as np, time, json, os, random

# Different seed — should give different stochastic env transitions
# no seed — match v18
...
print(f'v22: Replicate v18 setup from BC (seed 42)')
...
for ep in range(epochs):
    ...
    if time.time() - last_print >= 5 or ep == 0 or ep == epochs - 1:
        ...
        last_print = time.time()
    # FREQUENT ckpts every 25 epochs
    if (ep + 1) % 25 == 0:
        ckpt = f'{SAVE_DIR}/ppo_v22_repro_ep{ep+1}.pt'
        torch.save({'model': agent.net.state_dict(), 'metrics': metrics,
                    'multi_kill_episodes': multi_kill_eps}, ckpt)

final = f'{SAVE_DIR}/ppo_v22_repro_final.pt'
torch.save({'model': agent.net.state_dict(), 'metrics': metrics,
