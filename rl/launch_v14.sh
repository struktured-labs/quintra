#!/bin/bash
# v14: PPO FRESH, REAL ROM, gargoyle.state, FIXED reward, SHORT eps (3000), LOW entropy
# Hypothesis: v13 regression came from too-high entropy (2.3, near-uniform) + post-kill noise
# in long episodes. Force tight combat focus.
...
n_envs, epochs, steps_per_epoch, max_steps = 2, 2000, 512, 3000
...
cfg = PPOConfig(epochs=epochs, steps_per_epoch=steps_per_epoch * n_envs,
                train_iters=10, entropy_coef=0.02,  # LOWER
                hidden=256, n_layers=3, gamma=0.99)
...
print(f'Total: {len(completed_returns)} eps, {sum(completed_bosses)} cum kills, '
      f'{len(boss_kill_episodes)} kill-eps')
venv.close()
" 2>&1
