#!/bin/bash
# v32: RESUME v31 ep1500 (big net), train with v19's setup (max_steps=15000, low entropy).
# Bigger net + v19 finetune protocol — chase 100% det multi-kill replicate.
...
RESUME = '/home/struktured/projects/penta-dragon-dx-claude/rl/ppo_v31_bignet_final.pt'

device = 'cuda' if torch.cuda.is_available() else 'cpu'
n_envs, epochs, steps_per_epoch, max_steps = 2, 500, 512, 15000
print(f'Device: {device}, big net 512x4, max_steps={max_steps}, epochs={epochs}')
...
cfg = PPOConfig(epochs=epochs, steps_per_epoch=steps_per_epoch * n_envs,
                ...
                hidden=512, n_layers=4, gamma=0.99, pi_lr=5e-5)
agent = PPOAgent(obs_dim, N_ACTIONS, cfg, device=device)
state = torch.load(RESUME, map_location=device, weights_only=False)
agent.net.load_state_dict(state['model'])
print(f'v32: Resumed v31 final, big net 512x4, max_steps=15000')
...
for ep in range(epochs):
    ...
    if time.time() - last_print >= 5 or ep == 0 or ep == epochs - 1:
        ...
        last_print = time.time()
    if (ep + 1) % 20 == 0:
        ckpt = f'{SAVE_DIR}/ppo_v32_bignet_resume_ep{ep+1}.pt'
        torch.save({'model': agent.net.state_dict(), 'metrics_summary': m,
                    'multi_kill_count': len(multi_kill_eps)}, ckpt)

final = f'{SAVE_DIR}/ppo_v32_bignet_resume_final.pt'
